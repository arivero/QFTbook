...
\paragraph{Observable outputs before formal sewing.}
The formal data should be read through the boundary observables they predict.
Three output coordinates recur throughout the chapter:
\begin{equation}
\begin{aligned}
  Z_{ab}^{\rm op}(q)
  &=
  \operatorname{Tr}_{\Hilb_{ab}}q^{L_0-c/24},                         \\
  \langle\Phi_i(x+\ii y,x-\ii y)\rangle_{\mathbb H,a}
  &=
  U_i^a(2y)^{-2h_i},
  \qquad \Phi_i\ \hbox{spinless},                                      \\
  \mathcal G_a[\varphi]
  &=
  \frac12
  \int_0^L\!\dd\tau_1
  \int_0^L\!\dd\tau_2\,
  K_L(\tau_1-\tau_2)
  \langle\varphi(\tau_1)\varphi(\tau_2)\rangle^{\rm ren}_{c,a}.
\end{aligned}
  \label{eq:bcft-observable-output-coordinates}
\end{equation}
The first is the open-channel annulus spectrum, the second is the disk
one-point response to a bulk source near the boundary, and the third is the
renormalized boundary susceptibility which enters the boundary-entropy
gradient formula after the Ward and contact-term hypotheses below are supplied.
Agreement in the annulus coordinate alone is therefore not an observable-level
BCFT comparison.  Two candidates can have the same graded open Hilbert spaces
while their disk one-point coefficients, boundary two-point pairings, or
gradient susceptibilities differ.  The finite rational and nonrational cells
below are useful only insofar as they keep these observable coordinates tied to
one common sewing datum.
...
\begin{proposition}[Annulus nimrep spectral resolution]
\label{prop:bcft-annulus-nimrep-spectral-resolution}
Let \(I\) be the set of simple chiral labels of a unitary rational theory,
with fusion coefficients \(N_{ij}{}^k\) and modular matrix \(S\).  Let
\(\mathcal B\) be a finite set of elementary boundary labels, and suppose the
open annulus multiplicities form matrices
\[
  (n_i)_a{}^b=n_{ab}{}^i,
...
\begin{equation}
  n_0=\mathbf 1,\qquad
  n_i n_j=\sum_k N_{ij}{}^k n_k .
  \label{eq:bcft-nimrep-ring-representation}
\end{equation}
Assume also that these matrices are normal and mutually commuting in the
unitary boundary two-point pairing.  Then there is a unitary matrix
\(\Psi_{a e}\), whose columns are indexed by a finite multiset
\(\mathcal E\) of exponents \(e\), and a chiral label \(\alpha(e)\in I\) for
each exponent, such that
\begin{equation}
  (n_i)_a{}^b
  =
  \sum_{e\in\mathcal E}
  \Psi_{a e}\,
  \frac{S_{i,\alpha(e)}}{S_{0,\alpha(e)}}\,
  \overline{\Psi_{b e}} .
  \label{eq:bcft-annulus-nimrep-spectral-resolution}
\end{equation}
The number of copies of a fixed chiral label in \(\mathcal E\) is the
dimension of the corresponding common eigenspace of the annulus matrices.
If the closed channel contains one Ishibashi copy \(|e\rangle\!\rangle\) for
each exponent copy and the boundary-state coefficients are
\begin{equation}
  B_a{}^e
  =
  \frac{\Psi_{a e}}{\sqrt{S_{0,\alpha(e)}}},
  \label{eq:bcft-nimrep-boundary-reflection-coefficients}
\end{equation}
then the closed-channel annulus formula reproduces precisely the open-channel
matrices \(n_i\).
\end{proposition}

\begin{proof}
The matrices \(n_i\) are simultaneously unitarily diagonalizable because they
are normal and mutually commute.  On a common eigenline \(e\), the eigenvalues
\(\lambda_i(e)\) obey
\[
  \lambda_i(e)\lambda_j(e)
  =
  \sum_k N_{ij}{}^k\lambda_k(e),
  \qquad
  \lambda_0(e)=1,
\]
so \(i\mapsto\lambda_i(e)\) is a one-dimensional representation of the
Verlinde fusion algebra.  Since \(S\) diagonalizes this semisimple algebra,
every such character has the form
\[
  \lambda_i(e)=\frac{S_{i,\alpha(e)}}{S_{0,\alpha(e)}}
\]
for a chiral label \(\alpha(e)\), with repetitions allowed when the common
eigenspace has multiplicity.  Writing the spectral projectors in an
orthonormal eigenbasis gives
\eqref{eq:bcft-annulus-nimrep-spectral-resolution}.  Finally,
\eqref{eq:cardy-annulus-integrality-equation} with
\eqref{eq:bcft-nimrep-boundary-reflection-coefficients} gives
\[
  \sum_{e\in\mathcal E}
  \overline{B_b{}^e}\,B_a{}^e\,S_{\alpha(e)i}
  =
  \sum_{e\in\mathcal E}
  \Psi_{a e}\,
  \frac{S_{i,\alpha(e)}}{S_{0,\alpha(e)}}\,
  \overline{\Psi_{b e}},
\]
using the symmetry of \(S\), which is the displayed open annulus matrix.
\end{proof}

This proposition is the annulus part of the open--closed sewing problem, not a
replacement for the Cardy--Lewellen equations.  It says that a non-diagonal
rational boundary theory is not specified by arbitrary nonnegative integer
open spectra: the annulus matrices must form a fusion-ring representation, and
their common eigenvectors are the closed-channel reflection coefficients of
the boundary states.  The later boundary OPE, disk classifying algebra, and
two-point pairing data still have to explain how these reflection coefficients
participate in local operator products and all bordered-surface sewings.  In
the diagonal Cardy solution this spectral resolution is the regular
representation diagonalized by \(S\); in non-diagonal examples the exponent
multiset records which Ishibashi sectors, and with what multiplicities, are
visible to the chosen boundary theory.
...
normalizations of boundary two-point functions and, when a chiral fusion space
has dimension larger than one, on a separate choice of chiral-intertwiner
basis.  A change of those bases conjugates the coefficient tensors, while the
sewing equations below are invariant statements.
...
Cardy--Lewellen boundary sewing equation has the following scalar-coordinate
form when the four chiral fusion spaces in the cell are multiplicity-free:
\begin{align}
  &\sum_{p,\mu}
...
\end{align}
The displayed formula is a coordinate expression for an equality of chiral
blocks, not a scalar identity independent of bases.  The indices \(\mu\) and
\(\eta\) are boundary-field multiplicity labels: the sum over them is the
composition of boundary OPE tensors, with the boundary two-point pairing used
to identify the intermediate field space with its dual.  In a nonorthonormal
boundary basis, the displayed repeated-index convention inserts the inverse
two-point metric \(g^{(ac,p)}_{\mu\nu}\), and similarly in the \(q\)-channel.
The multiplicity-free chiral block coordinates are instead basis lines
\(e_p\) and \(f_q\), and the fusing matrix is the coordinate matrix of the
associator
\[
  F:\bigoplus_{p}\operatorname{span}\{e_p\}
  \longrightarrow
  \bigoplus_{q}\operatorname{span}\{f_q\}.
\]
The left side of \eqref{eq:cardy-lewellen-boundary-sewing} first contracts the
two boundary-OPE tensors through the boundary two-point duality and then
applies this chiral associator; the right side gives the same vector's
coordinates after the other OPE ordering.  If the chiral fusion has
multiplicity, the label \(p\) must be refined to
\((p,r,s)\), with
\(r\in\operatorname{Hom}(i\otimes j,p)\) and
\(s\in\operatorname{Hom}(p\otimes k,\ell)\), and similarly
\(q\) to \((q,t,u)\).  Then \(F_{pq}\) is replaced by the full matrix
\[
  F_{(p,r,s),(q,t,u)}
  \!\begin{bmatrix} i&j\\ k&\ell \end{bmatrix},
\]
and the OPE tensors carry these chiral-intertwiner indices separately from the
boundary-field labels \(\alpha,\beta,\gamma,\delta,\mu,\eta\).  The scalar
formula \eqref{eq:cardy-lewellen-boundary-sewing} is the
multiplicity-free specialization used by the diagonal Ising cell below; the
general semisimple mechanism is the module-associativity formulation developed
...
\paragraph{Sewing-move map for the examples.}
The examples up to this point are local coordinates on one sewing problem,
not independent substitutes for a BCFT construction.  The useful way to read
them is to ask which elementary move they test and what still has to be
supplied before the move can be used on arbitrary bordered surfaces:
\begin{center}
\small
\begin{tabular}{
...
\hline
local cell & sewing move tested & input still outside the cell \\
\hline
Diagonal Cardy annulus, compact-boson boundaries, and Ising spectra
& open/closed annulus equality after boundary-state and character
normalizations have been fixed
& construction of the open Hilbert spaces, positivity of the multiplicity
measure, and compatibility with boundary fields \\
Boundary OPE equation and the finite Ising four-field cell
& genus-zero boundary associativity for two decompositions of one disk
correlator
& common domains, convergent conformal blocks, two-point metrics, and the
higher move relations generated by further cuts \\
Bulk-boundary normalization, disk classifying coordinates, and direct sums
& identity-channel disk sewing and the distinction between elementary
boundaries and Chan--Paton completions
& the bulk two-point metric, local-operator map, boundary identity
normalization, and any chosen trace on a non-simple boundary algebra \\
Liouville Plancherel and pole-crossing cells
& direct-integral annulus pairing, quotient by reflection of boundary labels,
and residue summands when contours move
& nuclear test spaces, positive open spectral measures, boundary-field
operator domains, and a channel-independent contour/residue prescription \\
Finite move-defect and anomaly-line models
& path independence in a generated move graph and removal or retention of
projective sewing phases
& proof that the elementary moves generate the relevant decompositions and
that determinant lines are trivialized compatibly with all gluing maps \\
\hline
\end{tabular}
\end{center}
The rational Frobenius/module formalism below is therefore not another
annulus formula.  It is the algebraic mechanism that makes the annulus
matrices, boundary OPE, bulk-boundary classifying action, and two-point
pairings shadows of one module-category structure.  The nonrational paragraphs
above identify the analogous analytic burden when finite sums are replaced by
direct integrals and contour prescriptions.
...
\paragraph{Annulus shadows versus sewing data.}
In the pointed \(G/H\) laboratory, the endpoint annulus matrices do not
determine the endpoint-preserving boundary algebra, its classifying idempotents,
or its two-point sewing pairing.  More precisely, the annulus shadow
\[
  n_{(0,0)}=n_{(1,0)}=\mathbf 1
\]
is compatible with the semisimple stabilizer algebra
\[
  E_{\rm ss}=\mathbb C\{1,\tau\},\qquad \tau^2=1,
\]
which has primitive idempotents
\[
  e_\pm=\frac12(1\pm\tau),
\]
but the same endpoint-dimension shadow is also compatible, as a graded vector
space, with the nilpotent algebra
\[
  E_{\rm nil}=\mathbb C\{1,\epsilon\},\qquad \epsilon^2=0.
\]
The latter has no nontrivial idempotent decomposition and the
coefficient-of-\(1\) two-point pairing is degenerate on the \(\epsilon\)
sector.  Hence the annulus endpoint spectrum by itself is not enough data for
Cardy--Lewellen sewing; the boundary OPE multiplication, the classifying
idempotents, and the Frobenius two-point functional are additional structure.

The finite verification is direct.  Both algebras have a one-dimensional
vacuum-labeled endomorphism sector and a one-dimensional stabilizer-labeled
endomorphism sector at the same boundary
endpoint.  Therefore the annulus matrix which records only endpoint
multiplicity is the identity matrix for both labels \(1\) and the nontrivial
stabilizer label.
...
\]
With \(\varepsilon_{\rm ss}(1)=1\) and \(\varepsilon_{\rm ss}(\tau)=0\), the
pairing \((x,y)\mapsto\varepsilon_{\rm ss}(xy)\) satisfies
\[
  \varepsilon_{\rm ss}(e_\pm e_\pm)=\frac12,\qquad
  \varepsilon_{\rm ss}(e_+e_-)=0.
\]
This is the finite classifying-sector splitting used above.

In \(E_{\rm nil}\), an idempotent \(p=a+b\epsilon\) satisfies
\[
  a^2=a,\qquad 2ab=b.
\]
The first equation gives \(a=0\) or \(a=1\), and then the second gives
\(b=0\).  Thus the only idempotents are \(0\) and \(1\).  With the same
annulus-normalized coefficient-of-\(1\) functional
\(\varepsilon_{\rm nil}(1)=1\), \(\varepsilon_{\rm nil}(\epsilon)=0\), the
pairing obeys
\[
  \varepsilon_{\rm nil}(\epsilon\epsilon)=0,
\]
so the stabilizer sector has no inverse two-point partner in that pairing.
The two finite systems therefore have the same endpoint annulus shadow but
different boundary OPE and classifying-sector data.  The semisimple rational
construction excludes the nilpotent alternative by its Frobenius-module
identities, while positivity supplies the corresponding two-point
nondegeneracy.  The example shows why these inputs are not recoverable from
annulus spectra alone.

\begin{controlledapproximation}[Boundary observable vector from one sewing datum]
\label{ca:bcft-boundary-observable-vector}
A BCFT construction claim is an assertion about a vector of boundary
observables, not only about an annulus spectrum.  For fixed chiral input and
boundary labels, assemble the comparison vector
\[
  \mathcal V_{\partial}
  =
  \bigl(
  ...
  \bigr),
\]
where the entries denote, respectively, annulus amplitudes, disk one-point
coefficients, bulk-boundary maps, boundary OPE tensors, boundary two-point
pairings, defect-endpoint data, and the closed-loop sewing transport.  A
single boundary datum \(\mathcal D_\partial\) determines
\(\mathcal V_{\partial}(\mathcal D_\partial)\) only after the open Hilbert
spaces or spectral measures, operator domains, two-point metrics, classifying
coordinates, local move maps, and anomaly-line trivializations have been
chosen in the same normalization.

The finite pointed laboratory above is a small exact model of this dependence.
The annulus shadow
\[
  n_{(0,0)}=n_{(1,0)}=\mathbf 1
\]
does not determine the stabilizer product or pairing.  In the semisimple
choice \(\tau^2=1\), the stabilizer field has an inverse channel and
idempotents \(e_\pm=(1\pm\tau)/2\) with nondegenerate two-point pairing.  In
the nilpotent choice \(\epsilon^2=0\), the same endpoint annulus shadow has no
nontrivial idempotent split and the stabilizer field has zero identity-channel
two-point partner.  These two choices give different
\(\mathcal V_{\partial}\) even though they agree on the annulus component.

For a controlled construction, the comparison with the exact boundary theory
therefore has the form
\begin{equation}
  \mathcal V_{\partial}^{\rm exact}
  -
  \mathcal V_{\partial}(\mathcal D_\partial)
  =
  E_{\rm ch}
  +E_{\rm open}
  +E_{\rm loc}
  ...
  +E_{\rm move}
  +E_{\rm anom},
  \label{eq:bcft-boundary-observable-vector-residual}
\end{equation}
...
\begin{equation}
  ...
  +B_{\rm move}
  +B_{\rm anom}.
  \label{eq:bcft-boundary-observable-vector-bound}
\end{equation}
Here \(E_{\rm ch}\) records errors in the chiral modular/fusing input,
\(E_{\rm open}\) errors in open Hilbert spaces or spectral measures,
\(E_{\rm loc}\) local annulus, disk, bulk-boundary, and OPE mismatches,
\(E_{\rm pair}\) two-point and Frobenius-pairing mismatches,
\(E_{\rm move}\) generated sewing-move defects, and \(E_{\rm anom}\)
determinant-line or projective-transport errors.  Omitting the pair or move
terms turns the construction into a statement about selected shadows, not a
BCFT observable claim.  This is why the diagonal Cardy annulus, the pointed
\(G/H\) cell, and the Liouville contour identities are useful local tests but
not interchangeable proofs of all bordered-surface amplitudes.
...
\paragraph{Boundary observables and construction dependencies.}
A boundary condition becomes physical through the observables it assigns to
intervals, disks, annuli, and boundary operator insertions.  For boundary
conditions \(a,b\), the open-channel annulus
\[
  Z_{ab}(q)
  =
  \operatorname{Tr}_{\Hilb_{ab}}
  q^{L_0-c/24}
\]
records the spectrum seen by a cylinder, but it is only one projection of the
boundary theory.  The disk one-point coefficients \(U^a_i\), the
bulk-boundary coefficients \(R^a_{i\beta}\), the boundary OPE tensors
\[
  C\!\begin{bmatrix} a&b&c\\ i&j&k \end{bmatrix},
\]
the boundary two-point pairing, the endpoint data for defect lines, and the
renormalized entropy \(s(\lambda,L)\) are different physical outputs.  They
are tied together only when they are produced from the same boundary Hilbert
spaces, operator domains, normalizations, and sewing maps.  Thus the chapter's
finite computations should be read as a dependency chain, not as a collection
of unrelated BCFT cells and not as interchangeable tests of the same claim.
A genuine construction of a boundary conformal field theory has to pass
...
\begin{enumerate}
  ...
  \item Local observable coordinates must be compatible: annulus
  multiplicities or spectral measures, disk one-point and bulk-boundary
  coefficients, boundary OPE products, classifying coordinates, defect
  endpoints, and two-point pairings must be projections of the same boundary
  structure.
...
\end{enumerate}
This order prevents several common overreadings.  The diagonal Ising example
closes the first few rational finite checks for a specific modular tensor
category and boundary basis; it does not determine the general
Cardy--Lewellen construction.  The pointed \(G/H\) laboratory goes further by
showing that annulus matrices, quotient Fourier data, stabilizer
idempotents, boundary OPEs, and two-point pairings are compatible shadows of
one module-category mechanism.  It also shows why the annulus shadow alone is
too small: a boundary sector can have the same endpoint spectrum but a
different multiplication, idempotent decomposition, or two-point pairing, and
therefore different disk and boundary-correlator observables.  The Liouville
block closes only local nonrational coordinates: hyperbolic FZZT/ZZ
identities, the even-parameter Plancherel quotient, and pole-crossing residue
bookkeeping.  These are indispensable coordinates for boundary amplitudes,
but they still require common test spaces, positive open spectral measures,
and channel-independent contour prescriptions before they define a
nonrational BCFT.  Conversely, the boundary-entropy gradient result uses
renormalized boundary RG and positivity data once a unitary boundary QFT
exists; it orders possible endpoints but does not construct the sewing maps,
identify the infrared boundary condition, or supply the missing boundary OPE
and bulk-boundary coefficients.
