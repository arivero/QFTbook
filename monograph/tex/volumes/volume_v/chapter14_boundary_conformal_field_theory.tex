...
\]
The scalar \(\lambda_\ell\) on a closed loop \(\ell\) is the finite
projective sewing anomaly.  The anomaly cocycle is trivial on a connected
move graph if there are nonzero vertex scalars \(\eta_v\) such that
\begin{equation}
...
trivialization exists on the component containing that loop; a nonzero scalar
amplitude can be decomposition-independent only as a section of the anomaly
line, not after forgetting the line.
\end{proposition}
...
come from vertex rescalings.  Transporting a nonzero scalar amplitude around
that loop multiplies it by \(\lambda_\ell\), contradicting scalar
decomposition independence unless the anomaly line is retained.
\end{proof}
...
of the move; it does not by itself cancel a closed-loop anomaly.
