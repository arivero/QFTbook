...
Cardy--Lewellen boundary sewing equation, written in the chiral-block
coordinate convention described below, is
\begin{align}
  &\sum_{p,\mu}
...
blocks, not a scalar identity independent of bases.  For each intermediate
label \(p\), choose a basis \(e_{p,\mu}\) of the \(s\)-channel block space
and the dual basis \(e^{p,\mu}\) of its dual; similarly choose
\(t\)-channel bases \(f_{q,\eta}\).  The fusing matrix is the coordinate
matrix of the associator
\[
  F:\bigoplus_{p}\operatorname{span}\{e_{p,\mu}\}
  \longrightarrow
  \bigoplus_{q}\operatorname{span}\{f_{q,\eta}\}.
\]
The left side of \eqref{eq:cardy-lewellen-boundary-sewing} first contracts the
two boundary-OPE tensors against the \(s\)-channel dual basis and then applies
this associator; the right side gives the same vector's coordinates in the
\(t\)-channel basis after the other OPE ordering.  If the chiral-block bases
are changed, the \(C\)-tensors and \(F\)-matrix transform by the corresponding
source and target change-of-basis matrices, so the equality is really the
statement that boundary OPE coefficients form an associative multiplication
