...
\begin{proposition}[Degenerate BPZ blocks]
...
\end{equation}
where \(\mathcal R_b\) is the explicit product of gamma functions obtained
from the connection matrix and the OPE normalization
\[
  V_{-b/2}V_{\alpha_1}
  \sim
  ...
  +C_-(\alpha_1)V_{\alpha_1+b/2}.
\]
The exact expression is equivalent to the \(b\)-shift of
\eqref{eq:dozz-structure-constant} under the \(\Upsilon_b\) relation
\eqref{eq:upsilon-b-shift-b}.
\end{proposition}
...
\eqref{eq:liouville-degenerate-shift-ratio}.  Substituting the
\(\Upsilon_b\) shift relation into the ratio of the two DOZZ constants gives
the same gamma-function product, which is the convention-sensitive check
recorded here.
\end{proof}
