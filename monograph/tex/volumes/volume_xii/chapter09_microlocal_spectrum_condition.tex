...
\begin{proposition}[Transport equation for \(U\)]
\label{prop:microlocal-u-transport}
The coefficient multiplying \(\sigma_\epsilon^{-1}\) in the four-dimensional
Hadamard parametrix is
\[
  U(x,y)=\Delta(x,y)^{1/2}.
...
\]
where \(U\) is fixed by Proposition~\ref{prop:microlocal-u-transport} and the
...
\]
Together with Proposition~\ref{prop:microlocal-u-transport}, the remaining
...
\begin{proposition}[Hadamard form of a quasifree two-point function]
\label{prop:microlocal-hadamard-smooth-remainder}
