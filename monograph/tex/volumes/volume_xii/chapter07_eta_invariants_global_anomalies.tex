...
\begin{lemma}[Integer jump at a simple crossing]
\label{lem:eta-integer-jump}
...
\(+1\).  The reverse crossing is the same calculation with signs reversed.
\end{proof}
...
Lemma~\ref{lem:eta-integer-jump} is the elementary reason that global
...
\(\operatorname{Det}D_A\) with \(\operatorname{Det}D_{A^g}\).  Gauge
equivariance identifies \(\operatorname{Det}D_{A^g}\) with
\(\operatorname{Det}D_A\).  The composition is transport around a closed loop
in the quotient background groupoid.  The geometric representative of this
loop is precisely the mapping torus \(Y_g\), and the holonomy is therefore
...
\begin{proof}[Proof of deformation invariance]
Eigenvalues away from zero occur in pairs \(\lambda,-\lambda\) for the real
...
\begin{proposition}[Parity criterion for \(SU(2)\) representations]
\label{prop:su2-global-anomaly-parity-criterion}
...
\begin{proposition}[Local descent as the infinitesimal limit of inflow]
\label{prop:eta-inflow-local-descent}
...
additional global information not determined by the local polynomial alone.
\end{proposition}
