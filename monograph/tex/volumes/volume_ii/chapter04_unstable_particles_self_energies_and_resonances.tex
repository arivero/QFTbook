...
same channel as the exchanged \(\phi_2\) line. The pole suggested by the
tree graph is reorganized by the exact two-point function of \(\phi_2\).
...
The partial-wave normalization used below is
\[
  \mathcal M(s,z)
  =
  ...
  \qquad z=\cos\theta,
\]
and, in an elastic one-channel interval,
\[
...
on the physical upper boundary. Elastic unitarity gives
\[
  |S_\ell(s)|=1 .
\]
Thus the exact physical-axis partial wave is finite. A resonance is the
corresponding analytic structure: a nearby pole of the continuation of
\(S_\ell(s)\), or equivalently of \(\mathcal M_\ell(s)\), through the
threshold cut.
...
direction, so it is not a normalizable Hilbert-space eigenvector; it is the
singular vector associated with the pole of the analytically continued
scattering function.
...
\[
  ...
  \langle\Omega|T\phi_2(x)\phi_2(0)|\Omega\rangle .
\]
...
\[
  ...
  \frac{1}{
    \left[\ell_E^2+k^2x(1-x)+m_1^2-\ii0\right]^2}.
\]

In four dimensions, with a Euclidean cutoff \(|\ell_E|<\Lambda\),
\[
  \Sigma_1(k)
  =
  \frac{g^2}{32\pi^2}
  ...
  \left[
    \log
    \frac{\Lambda^2}{k^2x(1-x)+m_1^2-\ii0}
    -1+O(\Lambda^{-2})
  \right].
\]
...
Near a narrow resonance, this continuation places the pole at
\[
  s_\ast
  =
