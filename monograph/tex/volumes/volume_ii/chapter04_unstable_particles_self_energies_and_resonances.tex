...
that subspace.  A resonance pole \(s_\ast=M_R^2-\ii M_R\Gamma_R+\cdots\) has
...
the width scale \(M_R\Gamma_R\) must be small compared with the squared-mass
distance to thresholds or singularities whose variation is being neglected,
and the nonresonant background must be slowly varying on the same scale.  If
the pole lies close to a threshold, or if several channels produce nearby
singularities, the simple Breit--Wigner or complex-mass-shell description is
replaced by the corresponding multi-channel analytic line shape.  The
external states remain the stable states in all cases.
