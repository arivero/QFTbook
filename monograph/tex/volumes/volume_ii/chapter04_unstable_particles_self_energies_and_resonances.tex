...
the complex conjugate of the denominator.  Hence
\[
  |S_0^{\rm BW}(s)|=1
\]
exactly in this elastic model.  The physical partial wave differs from
\(S_0^{\rm BW}\) by the omitted background, crossed-channel, inelastic, and
higher-order corrections; these are corrections to the model, not violations
of the algebraic unitarity of the Breit--Wigner factor itself.  The resonance
is expressed as rapid phase motion. If
\[
...
connected stable-channel scattering
kernels.  After the identity term and the overall momentum-conservation delta
function have been removed, let
\[
  \mathcal M_{ab}^{\rm I}(s)
\]
denote a connected invariant amplitude or partial-wave coordinate between
stable channels \(a\) and \(b\) on the first sheet.  Its physical value on the
real two-particle interval is the upper-edge boundary value
\(\mathcal M_{ab}^{\rm I}(s+\ii0)\).  A resonance is not a pole of this
physical boundary value and is not an additional normalizable asymptotic state.
It is a pole of an analytically continued connected \(S\)-matrix element.
...
Feynman boundary prescription in the preceding section.  The second sheet
relative to the \(\phi_1\phi_1\) channel is the adjacent analytic germ obtained
by continuing \(F_{\rm I}\), or equivalently \(\mathcal M_s^{\rm I}\), through
the threshold cut.  Across this cut the square-root branch changes sign.
Equivalently, the logarithm in \(F_{\rm I}(s)\) has the discontinuity
\[
...
sheet; the notation \(F_{\rm II}\) below denotes precisely this continued
denominator.  Near \(s=M_R^2\), the branch inherited by \(F_{\rm II}\) has
\[
  F_{\rm II}(s)
  =
...
The first-sheet denominator has a useful sign property. Continue \(F_{\rm I}(s)\)
...
\[
  ...
  \operatorname{Im}(-s)<0,
\]
for \(0<x<1\). Hence \(\operatorname{Im}F_{\rm I}(s)<0\) on the upper half of the
first sheet, so \(F_{\rm I}(s)\) has no zero there. The lower half-plane gives the
