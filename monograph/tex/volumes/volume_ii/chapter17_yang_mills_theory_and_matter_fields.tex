...
field is represented by a Lie-algebra-valued one-form
\[
  A=A_\mu(x)\,\dd x^\mu,
  \qquad
...
basis of the Lie algebra \(\mathfrak g=\operatorname{Lie}(G)\).  We use
Hermitian generators \(t^a\), so
\begin{equation}
  [t^a,t^b]=\ii f^{ab}{}_{c}t^c .
  \label{eq:ym-lie-algebra-conventions}
\end{equation}
The real constants \(f^{ab}{}_{c}\) are the structure constants in this basis.
...
the local gauge algebra is a compact reductive Lie algebra: a direct sum of an
abelian part and compact simple factors.  For the simple compact factors one
may choose a basis in which \(\kappa^{ab}\) is positive diagonal.
...
\(\mathfrak{su}(N)\) is represented by traceless Hermitian \(N\times N\)
matrices, and
\[
...
scalar of power-counting dimension four:
\begin{equation}
...
is a total derivative, so perturbation theory around the trivial gauge-field
sector has no local Feynman vertex associated with
\eqref{eq:ym-theta-density}.  Finite-action gauge fields may approach
nontrivial pure gauges at spatial infinity, and then the spacetime integral of
\eqref{eq:ym-theta-density} can be nonzero.  Thus \(\theta\) enters through
global topological sectors of the gauge field.
...
\text{adjoint} & \mathfrak{su}(N)\simeq\mathbb R^{N^2-1} &
  \widetilde\rho_{\mathrm{adj}}(t^a)(t^b)=\ii f^{ab}{}_{c}t^c
\end{array}
\]
The adjoint representation is conjugation on the algebra:
\[
  v\mapsto gvg^{-1},
  \qquad
  \widetilde\rho_{\mathrm{adj}}(t^a)v=[t^a,v].
\]
...
For chiral notation, choose the \(\gamma_5\) convention
\[
