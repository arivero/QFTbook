...
\begin{align}
  ...
  &\hspace{2.0cm}\times
  \exp\left[
    -\int\dd^Dx\,J(x)\bigl(\phi'(x)+\widehat\phi(x)\bigr)
  \right].
...
\(\int\dd^Dx\,J(x)\widehat\phi(x)=0\) for the shell field.  With a smooth cutoff
profile one enforces this by choosing the source momenta in a slightly smaller
region on which the shell covariance vanishes, or by using a profile with a
plateau.  Thus the source term in
...
renormalization convention requires the usual finite redefinition between
Wilsonian coordinates and the renormalized coupling used to label Green
functions; the structural fact is that the irrelevant coordinate has been
eliminated by solving its own flow equation.
