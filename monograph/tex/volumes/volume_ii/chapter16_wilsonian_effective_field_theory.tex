...
\begin{equation}
  ...
  =
  \frac12\int_k
  \widehat\phi_{\mathrm{sh}}(-k)\,
  \frac{k^2+m^2}{\chi_\Lambda(k)-\chi_{\Lambda'}(k)}\,
  \widehat\phi_{\mathrm{sh}}(k).
  \label{eq:shell-kinetic-term}
\end{equation}
The shell field is denoted \(\phi_{\mathrm{sh}}\).  A hat continues to denote
Fourier transformation; thus \(\widehat\phi_{\mathrm{sh}}(k)\) is the shell
field's Fourier mode, while \(\widehat\phi(k)\) elsewhere is the Fourier mode
