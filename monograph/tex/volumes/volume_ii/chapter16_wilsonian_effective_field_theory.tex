...
\begin{equation}
  ...
  F(\phi'+\widehat\phi)\,
  \dd\mu_{\widehat C_{\Lambda,\Lambda'}}(\widehat\phi)\,
...
\end{equation}
The number \(y_\alpha\) is the RG eigenvalue of the corresponding scaling
operator.  In the convention \(t=\log\Lambda\), decreasing the cutoff moves
toward the infrared.  A coordinate with \(y_\alpha>0\) grows toward the
ultraviolet and must be fixed by a renormalization condition to define a
continuum limit; it is relevant.  A coordinate with \(y_\alpha<0\) is
irrelevant: its value at a fixed physical scale is attracted to a function of
the relevant and marginal coordinates along a renormalized trajectory.
...
normalizations have been matched.  The exponent is not a matter of
microscopic aesthetics; it is the RG eigenvalue of the first omitted operator.
