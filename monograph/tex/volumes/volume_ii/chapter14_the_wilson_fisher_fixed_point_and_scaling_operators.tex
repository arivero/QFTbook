...
renormalized operator algebra, the equation of motion gives the descendant
relation
\[
  O_3(x)=\phi^3(x)=\kappa_*(\mu)\,\Box\phi(x),
\]
where \(\kappa_*(\mu)\) is a nonzero normalization depending on the convention
for the renormalized quartic interaction and on the normalization of
\([O_3]_\mu\).  The statement invariant under such choices is that \(O_3\) is
an equation-of-motion descendant of \(O_1=\phi\).  Its dimension is
\[
  \Delta_{\phi^3}
  =
  \Delta_\phi+2.
