...
The LSZ formula for the connected kernel is
\[
\boxed{
\begin{aligned}
&S^{\mathrm c}(q_1,\ldots,q_m|p_1,\ldots,p_n)
...
\]
Then cluster decomposition is the identity
\[
  S(\beta|\alpha)
  =
  \sum_{\Pi}
  \prod_{B\in\Pi}S_B^{\mathrm c}.
\]
This equation is also a recursive definition of the connected kernels. The
one-particle identity kernel is included as the connected kernel for a block
with one incoming and one outgoing label:
\[
  S^{\mathrm c}(q|p)
  =
  (2\pi)^d2\omega_{\vec p}\delta^d(\vec q-\vec p)
