...
vacuum, a mass gap above the vacuum, local commutativity, and clustering of
vacuum Wightman functions for spacelike separated operator groups.  Let
\(\Psi_A^{\rm in/out}\) and \(\Psi_B^{\rm in/out}\) be two groups of
...
\[
  \left\langle
    \Psi_A^{\rm out}\otimes U(a)\Psi_B^{\rm out},
    S\,
    \Psi_A^{\rm in}\otimes U(a)\Psi_B^{\rm in}
  \right\rangle
  \longrightarrow
  \left\langle\Psi_A^{\rm out},S\Psi_A^{\rm in}\right\rangle
  \left\langle\Psi_B^{\rm out},S\Psi_B^{\rm in}\right\rangle .
\]
...
\begin{proof}
Haag--Ruelle scattering states are obtained as large-time limits of products
of almost-local creation operators with separated velocity supports.  After a
large spacelike relative translation, every local operator belonging to group
\(A\) is spacelike separated from every local operator belonging to group
\(B\) throughout the time ranges used in the scattering limit.  Local
commutativity orders the two groups without extra commutator terms, and vacuum
clustering factors the Wightman functions into the product of the two
subprocess Wightman functions.  The mass gap gives the uniform estimates
needed to pass this factorization through the Haag--Ruelle limits.  Applying
the cumulant recursion of Proposition~\ref{prop:connected-scattering-kernels}
then removes all products of proper subprocesses from \(S^{\mathrm c}\).  A
separate momentum-conserving delta function for a proper subprocess would
survive precisely as such a product term, so it is absent from the connected
kernel.
\end{proof}
