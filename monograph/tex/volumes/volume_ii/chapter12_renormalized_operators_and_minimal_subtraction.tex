...
\[
  ...
  [O_I]_\mu .
\]
This is the cotangent transformation law.  It is the local-operator version of
the statement that beta functions are components of a vector field.
...
The operator mixing equation is the insertion version of the Callan--Symanzik
equation.  Let \(G_I^{(n)}\) denote a renormalized connected correlation
function with one insertion of \([O_I]_\mu\) and \(n\) elementary renormalized
fields:
\[
  G_I^{(n)}
  =
...
\]
Away from coincident points, the scale independence of the corresponding bare
correlation function gives
\begin{equation}
  \left(
...
\end{equation}
Summation over repeated coupling and operator labels is understood.  Contact
terms must be added when \(x\) collides with one of the other insertion
points; those terms are local and are governed by the same renormalized
operator basis.
