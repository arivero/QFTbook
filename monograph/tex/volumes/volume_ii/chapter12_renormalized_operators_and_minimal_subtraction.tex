...
Figure~\ref{fig:source-extended-wilsonian-operator-bridge} organizes the
source-extended comparison: source coordinates define insertion kernels in the
regulated Wilsonian functional, and the same source chart determines the 1PI
representative after Legendre transformation.
...
Figure~\ref{fig:mass-insertion-source-wilsonian-matching} displays this mass
insertion example as a finite source-coordinate matching problem between a
BPHZ normal product and the Wilsonian operator chart.
...
Figure~\ref{fig:multi-insertion-contact-source-jets} identifies the origin of
multi-insertion contact conventions: nonlinear source jets and the subtraction
of one-low-field reducible pieces in the projected 1PI transform.
...
Figure~\ref{fig:volume-ii-ms-coordinate-structure} fixes the MS coordinate
convention used below: the finite coordinate is kept separate from the pole
part, and all scheme information resides in the pole coefficients
\(K_I^{(n)}\).
...
\begin{figure}[H]
  ...
  \caption[Minimal-subtraction coordinate and pole data]{Minimal subtraction defines dimensionless running coordinates by
  expressing regulator-dependent couplings as a finite coordinate plus pole
  parts in dimensional regularization.  The pole coefficients are constrained
  by the independence of the regulated couplings from the auxiliary scale
  \(\mu\).}
...
Figure~\ref{fig:volume-ii-ms-recursion} records the recursive separation used in
the pole equations: positive powers in the beta function determine the pole
coefficients, while the engineering term contributes the universal
\(-\epsilon\lambda\) part.
