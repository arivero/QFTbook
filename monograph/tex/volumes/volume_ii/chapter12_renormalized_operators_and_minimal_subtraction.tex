...
finite-rank source-coordinate system and its transition functions have been specified.
\end{definition}
...
treats this finite
normal-product choice as part of the local source-coordinate system: changing a
...
assignment in the chosen low-momentum source-coordinate system.  Projecting at
...
matrices \(M_{\alpha\beta}(g,t)\) between source-coordinate systems are \(C^1\) along the
...
\end{equation}
for local representative columns \(v_\alpha\) in a source-coordinate system \(\alpha\).
Under a finite scale-dependent change of source-coordinate system with
...
the full Taylor expansion of the source-coordinate system.  At finite regulator let
\(\mathcal S_\Lambda\) denote the space of regulated source functions
\(\eta_\Lambda^A(x)\).  A local source-coordinate transformation is a smooth map
\[
...
source-extended Wilsonian action in the chosen source-coordinate system.
...
source-coordinate system and the Legendre-transform projection.
...
\paragraph{Insertion Callan--Symanzik equation.}
...
\begin{lemma}[Existence and uniqueness of the MS pole recursion]
\label{lem:ms-pole-recursion-existence-uniqueness}
