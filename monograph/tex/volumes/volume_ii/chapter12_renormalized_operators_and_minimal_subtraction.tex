...
\[
  ...
  \bigg|_{\eta_\Lambda=0}.
\]
The chain rule gives the finite-regulator partition formula
\begin{equation}
  \begin{aligned}
  G^\Lambda_{A_1\cdots A_r}(x_1,\ldots,x_r;J)
  &=
...
\]
The sign in \eqref{eq:multi-insertion-source-partition-formula} follows after
two pieces are combined: each operator insertion is represented by
\(-\delta/\delta\eta\), and the connected functional derivatives obey the
Fa\`a di Bruno partition rule for the composite source map
\(\eta\mapsto\eta^0(\eta)\).  For a partition with \(|\pi|\) blocks, the
connected functional contributes one source derivative for each block, whereas
the left-hand side has \(r\) insertion derivatives; the relative sign is
therefore \((-1)^{r-|\pi|}\).
...
\begin{proposition}[Noncoincident insertions and contact coordinates]
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\scriptsize}]
    \tikzset{
      box/.style={draw,rounded corners=3pt,align=center,
        minimum width=2.75cm,minimum height=0.78cm,line width=0.8pt},
      arr/.style={-{Latex[length=2mm]},line width=0.85pt}
    }
    \node[box] (source) at (0,1.5)
      {local source chart\\\(\eta_0=\mathcal Z_\Lambda(\eta_\Lambda)\)};
    \node[box] (linear) at (4.0,2.55)
      {linear jet\\\(D\mathcal Z_\Lambda\)};
    \node[box] (higher) at (4.0,0.45)
      {higher jets\\\(D^r\mathcal Z_\Lambda,\ r\ge2\)};
    \node[box] (sep) at (8.05,2.55)
      {noncoincident\\insertions};
    \node[box] (diag) at (8.05,0.45)
      {collision\\diagonals};
    \node[box] (dist) at (12.0,1.5)
      {renormalized\\inserted distribution};

    \draw[arr] (source)--(linear);
    \draw[arr] (source)--(higher);
    \draw[arr] (linear)--(sep);
    \draw[arr] (higher)--(diag);
    \draw[arr] (sep)--(dist);
    \draw[arr] (diag)--(dist);

    \node[blue!70!black,align=center] at (6.1,3.05)
      {operator mixing};
    \node[red!70!black,align=center] at (6.1,-0.10)
      {contact convention};
  \end{tikzpicture}
  }
  \caption{A full source coordinate chart has a linear jet and higher local
  jets.  The linear jet determines operator mixing at separated insertion
  points.  The higher jets are supported on collision diagonals and determine
  the contact-term convention for the full distribution.}
