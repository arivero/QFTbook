...
\begin{quotedtheorem}[Current-sector bosonization]
\label{qthm:schwinger-current-sector-bosonization}
...
\end{quotedtheorem}
  ...
  The normalization check for the current sector, which is the part used in the
  gauge theory, is as follows.  The equal-time commutator of the Dirac current,
  defined by
  ...
  The remaining content of the quoted theorem is that this representation gives
  all current correlators,
  ...
  \eqref{eq:schwinger-current-generating-functional}.  A complete proof also
  constructs the compact charge lattice and the vertex-operator sectors; only
  the gauge-invariant current subalgebra is used in the local Schwinger-model
  calculations below.
...
\begin{proposition}[Exact bosonized local gauge sector]
\label{prop:schwinger-exact-bosonized-gauge-sector}
...
\begin{figure}[H]
  ...
  from a linearly growing Maxwell kernel to the screened form proved in
  Proposition~\ref{prop:schwinger-screened-static-potential}.}
...
\begin{proposition}[Screened static potential in the massless model]
\label{prop:schwinger-screened-static-potential}
