...
physical scattering convention.
...
\]
on the first sheet.  The first sheet is the branch reached from the Euclidean
or Feynman domain by the \(+\ii0\) prescription without analytically
continuing through a unitarity cut.  In the fixed-\(t\) \(s\)-plane it has the
right-hand \(s\)-channel cut
\[
  C_s=[4m^2,\infty),
\]
...
positive Feynman parameters.  This condition is what distinguishes a
...
\[
  ...
  \lambda_i\ge0 ,
\]
after choosing orientations of the on-shell momenta \(q_i\).  For a
first-sheet physical singularity these oriented momenta must be compatible
with future-directed propagation through the spacetime diagram.  The factor
...
Gram determinant of the on-shell internal momenta degenerates in a way
compatible with positive Feynman parameters.
...
\(P_3^2=-M_3^2\).  A solution requiring reversed time ordering or a negative
proper-time segment lies on a continued sheet, even if it solves the same
algebraic Landau equations.
...
not by the endpoint opening of an ordinary multiparticle channel.  Its local
data are the masses, momentum routing, and positive Feynman parameters of the
Landau solution.  Coleman--Thun singularities in two-dimensional factorized
scattering are a standard nonperturbative setting where this mechanism
appears.
