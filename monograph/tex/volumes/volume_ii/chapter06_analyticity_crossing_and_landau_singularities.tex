...
whose mixed lines separate the graph into the two subgraphs carrying the
external labels \(\alpha\to X\) and \(X\to\beta\).  The mixed propagator on
each separating line is exactly
\[
  \frac{\dd^D q_i}{(2\pi)^D}
  (2\pi)\theta(q_i^0)\delta(q_i^2+m_i^2),
\]
