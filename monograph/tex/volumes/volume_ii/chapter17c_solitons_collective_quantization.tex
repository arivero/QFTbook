...
\begin{proof}
The detailed regulator calculation is
Section~\ref{sec:sine-gordon-dhn-soliton-mass}.  The vacuum and soliton
continuum modes are paired by mode number,
\[
  k_nL=\pi n,\qquad q_nL+\delta(q_n)=\pi n,
\]
after combining the two parity channels into the total positive-momentum
phase shift and using the branch \(\delta(0^+)=\pi\), \(\delta(\infty)=0\).
Levinson's theorem accounts for the single translation zero mode; after that
mode is removed from the determinant and integrated as the collective
coordinate \(X\), the oscillator comparison contributes \((0-m)/2\).  The
term \(2m/k\) in \eqref{eq:soliton-chapter-dhn-mass-shift} is the first Born
phase shift of the fluctuation potential, and the no-tadpole counterterm
cancels exactly that Born contribution.  Thus the finite \(-m/\pi\) is
produced by the Born-subtracted determinant, not inserted into the
counterterm.
\end{proof}
