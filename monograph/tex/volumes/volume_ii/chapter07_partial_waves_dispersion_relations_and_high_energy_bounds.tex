...
interval is the degenerate ellipse \(R=1\).
...
\]
Thus angular analyticity implies convergence of the partial-wave expansion
throughout every smaller ellipse inside the Lehmann ellipse.  More
quantitatively, if \(\mathcal A(s,z)\) is bounded by \(B_s(R)\) on
\(|\zeta(z)|=R\), then the corresponding Legendre coefficients are bounded
by a constant times \(B_s(R)R^{-\ell}\).  This is the analytic input later
...
\]
with constants independent of \(s\) at sufficiently large energy, for fixed
\(\delta\) and fixed \(t\) away from the singularity.
...
\[
  ...
  \operatorname{Im}\mathcal A(s,0)
\]
for the absorptive partial-wave sum.  Consequently
\[
  \lim_{s\to\infty}
...
\(P_\ell(1+2t/(s-4m^2))\) at \(t=0\) are nonnegative.  It allows the Taylor
expansion in \(t\) to commute with the dispersive integral inside its
convergence disk.  With two subtractions, \(N=2\) in the previous
