...
\begin{proposition}[Exact center symmetry of pure thermal lattice Yang--Mills]
\label{prop:lattice-center-symmetry}
...
In the ideal finite-basis case one obtains an exact algebraic extraction of
the energies.  Assume that, for \(n=1,\ldots,m\), the overlap matrix
\(Z_i^{(n)}\) is invertible and that the operator subspace has zero overlap
with all higher states.  Then the generalized eigenvalue problem
\begin{equation}
  C(t)v_n(t,t_0)
  =
  \lambda_n(t,t_0)C(t_0)v_n(t,t_0),
  \qquad t>t_0,
  \label{eq:lattice-glueball-gevp}
\end{equation}
has eigenvalues
\begin{equation}
  \lambda_n(t,t_0)=\ee^{-aE_n(t-t_0)}.
  \label{eq:lattice-glueball-gevp-eigenvalues}
\end{equation}
Indeed, under the stated finite-basis hypothesis,
\[
  C(t)=Z\,D(t)\,Z^\dagger,\qquad
  D(t)=\operatorname{diag}(\ee^{-aE_1t},\ldots,\ee^{-aE_mt}).
\]
Since \(Z\) is invertible,
\[
  C(t_0)^{-1}C(t)
  =
  (Z^\dagger)^{-1}D(t_0)^{-1}D(t)Z^\dagger .
\]
This matrix is similar to \(D(t_0)^{-1}D(t)\), hence has eigenvalues
\(\ee^{-aE_n(t-t_0)}\).

When higher states have nonzero overlap with the chosen operator basis, the
GEVP is no longer exactly diagonalized by the first \(m\) spectral terms.
One then writes \(C(t)=Z_mD_m(t)Z_m^\dagger+R_m(t)\), where \(R_m(t)\) is a
positive spectral remainder.  A quantitative extraction theorem requires an
explicit operator norm bound on \(C(t_0)^{-1/2}R_m(t)C(t_0)^{-1/2}\) and the
condition number of the retained overlap matrix; with such data, ordinary
finite-dimensional perturbation estimates give corrections suppressed by the
first omitted energy gap.  The present proposition states only the exact
...
\begin{enumerate}
  ...
  \item finite-volume energies extracted from
  \eqref{eq:lattice-glueball-gevp};
...
\begin{proposition}[Large-\(N\) glueball width counting]
\label{prop:lattice-glueball-large-n-width}
Normalize single-trace glueball operators by
\[
  \mathcal O_N(x)=\frac1{N}\operatorname{tr}\mathcal P(F,D)(x),
\]
so that connected two-point functions are \(O(N^0)\) in the confining
large-\(N\) scaling regime.  If the large-\(N\) expansion has isolated
