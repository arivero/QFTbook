...
\end{equation}
With higher-state overlap present, the same formula is the leading term in a
large-\((t-t_0)\) expansion whose error is controlled by the first omitted
energy gap and by the conditioning of the chosen operator basis.
\end{proposition}
...
\(\ee^{-aE_n(t-t_0)}\).  If higher states are present, write the correlation
matrix as this finite-rank part plus a positive remainder bounded by the first
omitted exponential in the chosen channel.  Standard perturbation estimates
for generalized eigenvalues then give the stated controlled leading behavior
whenever \(C(t_0)\) is well conditioned.
\end{proof}
