...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[>=Stealth,x=1cm,y=1cm,every node/.style={font=\small}]
    ...
    \begin{scope}[shift={(6.4,0)}]
      ...
      \draw[green!55!black,line width=0.95pt,dash pattern=on 5pt off 2.5pt]
        (-1.2,-0.25)--(1.65,1.22);
...
\]
This is a local differentiable Legendre transform in the chosen source-coordinate system.
...
\begin{lemma}[1PI source equation and inverse two-point kernel]
\label{lem:1pi-source-equation-inverse-kernel}
...
\paragraph{Shifted representation and tadpole condition.}
...
\begin{remark}[One-particle-reducible graphs are Legendre trees]
\label{rem:opr-graphs-legendre-trees}
...
In gauge theories there is a further qualification.  A gauge-fixed
perturbative effective potential is a coordinate on a gauge chart, not an
off-shell observable.  The BRST/BV control mechanism is the Nielsen identity
of Section~\ref{sec:nielsen-identity-gauge-effective-potential}: the value of
the exact 1PI action at an exact stationary point is gauge-parameter
independent under its hypotheses, while the off-shell potential and the field
coordinate of the stationary point generally are not.  Derivative-expanded
saddles, including false-vacuum bounces, require the full functional identity,
not only the zero-derivative potential.
...
\paragraph{Functional Schwinger--Dyson identity.}
...
\begin{lemma}[Euclidean convexity at finite regulator]
\label{lem:volume-ii-euclidean-convexity-finite-regulator}
...
Lemma~\ref{lem:volume-ii-euclidean-convexity-finite-regulator}.  It supplies
the positive-measure finite-regulator construction.  The perturbative 1PI
definition used in dimensional regularization, or in any other regulator whose
``path integral'' is a formal algebraic prescription, is a separate
construction.  The two constructions agree only in a domain where both are
defined and the convex conjugate is differentiable with a locally invertible
source-field map.

The local inverse \(J_{\varphi}\) used above is a convenient notation.
Lemma~\ref{lem:volume-ii-euclidean-convexity-finite-regulator} identifies the
...
Lemma~\ref{lem:volume-ii-euclidean-convexity-finite-regulator}, \(K\) is
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[>=Stealth,x=1cm,y=1cm,every node/.style={font=\small}]
    ...
    \begin{scope}
      ...
      \draw[green!55!black,curve,dash pattern=on 5pt off 2.5pt]
        (-1.35,-0.05)--(0.92,0.63);
      \draw[orange!85!black,curve,densely dotted] (-0.15,0.9)--(1.8,-0.65);
    ...
    \begin{scope}[shift={(6.2,0)}]
      ...
      \draw[gray!60,curve,dash pattern=on 5pt off 2.5pt]
  ...
  effective action \(\Gamma_E\) is convex.  Dashed and dotted support lines
  distinguish two source values, while the dashed nonconvex branches are not the
  exact effective action.  In a finite-dimensional double-well model, the exact
  Legendre transform connects the stable branches by the convex segment.}
