...
the remaining large-\(q\) integral has the schematic form
\[
  \int
  \frac{\dd^6q}{(2\pi)^6}\,
  \frac{\Sigma_R(q)}
...
\end{aligned}
\]
where \(R_3(q,k)\) falls fast enough at large \(q\) to make the corresponding
integral ultraviolet convergent.  The displayed terms are local in the
external momentum through order \(k^2\).  They contribute only to the
\(\phi^2\) and \((\partial\phi)^2\) counterterms.
