...
the descent of a six-form anomaly polynomial.  With the trace convention used
for the fermion representations, its group-theoretic part is
\[
  \mathcal P_6(F)
  \propto
  \sum_i\operatorname{tr}_{R_i}(F^3),
  \qquad
  F=\dd A+A^2 .
\]
Choose a Chern--Simons five-form \(Q_5(A)\) with
\[
  \dd Q_5(A)=\mathcal P_6(F).
\]
Its gauge variation is exact:
\[
  \delta_\zeta Q_5(A)=\dd Q_4^{(1)}(\zeta,A).
\]
The four-form \(Q_4^{(1)}\) gives a representative of
\(\mathcal A(\zeta,A)\), with the overall normalization fixed by the
short-distance calculation or by the index theorem.  The cancellation
condition \eqref{eq:gauge-anomaly-cancellation} is precisely the vanishing of
the six-form polynomial for the gauged symmetry.  If the polynomial is nonzero
for a global symmetry background, it is an anomaly to be matched rather than
an inconsistency.
