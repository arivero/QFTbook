...
The high-energy bounds of the preceding chapter used a massive scattering
setting.  Four-dimensional QED has a massless stable photon, and a charged
...
above.  The same combinatorics gives the product for any finite number of
soft photons.
...
normalization factors.  In scalar QED the one-loop self-energy has the soft
part
\[
  \ii\Sigma(p)
  =
...
\]
contains the same soft logarithm as the same-line eikonal loop.  Combining
the distinct-line soft loops with the infrared parts of the \(Z_n^{1/2}\)
factors gives
\[
  \exp\!\left[
    \frac12\sum_{n\ne m}g_ng_mJ_{nm}
...
The real part of the soft loop is
\[
...
Multiplying the real and virtual factors gives the leading soft result
\[
  P_{\beta|\alpha}(E_T)
  =
...
carried by the charged particle.  Dressed-state and inclusive descriptions use
...
soft cloud into the state space.
