...
\begin{remark}[Scaling limit as continuum QFT]
\label{rem:scaling-limit-continuum-qft}
...
LSMOH constraints &
Require microscopic translation, filling or projective-cell data, plus a
background-twist map into the proposed infrared theory.\\
\hline
\end{tabular}
\end{center}
...
Section~\ref{sec:lsmoh-lattice-qft-constraints}.  Rewriting a fractional-filling
or projective-unit-cell obstruction as an anomaly is powerful only when the
finite lattice translation operator, conserved charge, large background-gauge
transformation, and infrared defect or background-field action have been
matched.  A continuum Lagrangian with the same internal symmetry but no map from
the microscopic unit cell does not by itself inherit the LSMOH conclusion.
...
\subsection{Nonrelativistic Goldstone Counting}

Spontaneously broken internal symmetries also need a non-Poincare validation
map.  In a finite-density or microscopic Hamiltonian system, the relativistic
``one broken generator gives one Goldstone particle'' statement is replaced by
the commutator-density count of
Section~\ref{sec:nonlorentz-goldstone-counting}: broken charges whose
expectation-value commutator has nonzero density form canonical pairs and can
produce type-B modes.  This is a statement about the thermodynamic phase,
current algebra, and low-energy spectral branches, not about the existence of a
Lorentz-invariant UV completion.  The relevant checks are therefore the
charge-density matrix, the stiffness and susceptibility entering the
dispersion, and the separation from broken spacetime symmetries or
inverse-Higgs redundancies.
...
scaling limit satisfying Remark~\ref{rem:scaling-limit-continuum-qft}
constructs the continuum QFT as its limiting object.

The Weinberg--Witten theorem-boundary
Section~\ref{sec:weinberg-witten-boundaries-massless-charges} should be used
here as a diagnostic, not as a slogan.  An emergent gauge field in a lattice
or condensed-matter system may fail exact microscopic Poincare covariance; a
topologically ordered phase may encode charged sectors through line operators
or superselection data rather than point-local particle poles; and an
effective photon may be neutral under the microscopic Noether current whose
local density is available.  The theorem becomes applicable only after the
validation map supplies all of its inputs at once: exact Lorentz covariance,
a physical gauge-invariant local current or stress tensor, standard
massless one-particle states, and the nonzero forward matrix element of that
same operator.
