...
\begin{proof}
Using the odd Jacobi identity with \(F=G=S_{\rm BV}\),
\[
  (S_{\rm BV},(S_{\rm BV},H))
  =
  \frac12((S_{\rm BV},S_{\rm BV}),H)
\]
for every functional \(H\).  The right hand side vanishes by
\eqref{eq:classical-master-equation}.  Hence
\[
  s_{\rm BV}^2H=(S_{\rm BV},(S_{\rm BV},H))=0 .
\]
\end{proof}
...
transformations are nilpotent.  Indeed, the component of
\((S_{\rm min},S_{\rm min})\) with no antifields is the variation of
\(S_{\rm YM}\) under \(sA=Dc\).  The component linear in antifields is
equivalent to \(s^2A=0\) and \(s^2c=0\).  These are precisely the identities
derived in the BRST chapter from gauge invariance and the Jacobi identity.
...
\]
Applying the classical differential \(s_{\rm BV}=(S_0,\cdot)\) and using the
...
descent cocycle whose bottom component gives the consistent anomaly; adding a
local counterterm changes the representative by an \(s_{\rm BV}\)-exact
cocycle, matching the counterterm ambiguity above.
