...
\begin{corollary}[Displacement criterion]
\label{cor:displacement-topological}
...
\begin{proposition}[Higher-form Ward action]
\label{prop:higher-form-ward-action}
For an abelian \(p\)-form symmetry defect \(U_a(N)\) and a charged
\(p\)-operator \(W_q(\Sigma)\),
\[
  U_a(N)\,W_q(\Sigma)
  =
...
\begin{proof}
Because \(U_a\) is topological, move \(N\) through the complement of
