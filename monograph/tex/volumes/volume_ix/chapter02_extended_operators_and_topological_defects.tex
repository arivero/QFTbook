...
\]
At the cochain level the background representative is a cocycle
\(B\in Z^{p+1}(M;A)\), with gauge equivalence
\(B\sim B+\delta\xi\) for an arbitrary degree-\(p\) cochain
\(\xi\in C^p(M;A)\).  If the charged operator is described by a bounding-chain
or trivialization coordinate, Stokes' formula gives the compensating phase
\(q(\langle\xi,\Sigma\rangle)^{-1}\) under this gauge change, with the inverse
fixed by the convention that the combined background coupling and operator
insertion is gauge invariant.  This is different from a global higher-form
symmetry transformation at fixed background: a flat parameter
\(\alpha\in Z^p(M;A)\), modulo coboundaries, has \(\delta\alpha=0\) and acts by
\[
  W_q(\Sigma)\mapsto
  q\!\left(\langle\alpha,[\Sigma]\rangle\right)W_q(\Sigma),
  \qquad B\ \hbox{unchanged}.
\]
