...
There are two standard global replacements.  A finite \(p\)-form background
is represented by a cohomology class
\(\beta_a\in H^{p+1}(M,A)\), or by a relative/Thom class for a symmetry
defect, and its action on \(W_q(\Sigma)\) is the character evaluation
\[
  q\!\left(\langle\beta_a,[\Sigma]\rangle\right).
\]
...
defect is evaluated by the chosen background or torsion-pairing datum above.
