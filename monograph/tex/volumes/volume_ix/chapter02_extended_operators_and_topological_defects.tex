...
\end{tikzpicture}
\caption{Tubular-neighborhood data, fusion of parallel defects, and a
topological linking configuration.}
...
\]
When \(N\) and \(\Sigma\) are disjoint, their linking number is the integer
\[
  \operatorname{Link}(N,\Sigma)\in\mathbb Z
\]
defined as follows.  If \(N=\partial B\) for an oriented
\((D-p)\)-chain \(B\) transverse to \(\Sigma\), then
\[
  \operatorname{Link}(N,\Sigma)=B\cdot\Sigma ,
\]
the signed intersection number.  In general the same integer is the
Alexander-duality pairing between the homology class of \(N\) in
\[
  H_{D-p-1}(M\setminus\Sigma;\mathbb Z)
\]
and the class of \(\Sigma\).

\paragraph{Higher-form Ward action.}
For an abelian \(p\)-form symmetry defect \(U_a(N)\) and a charged
\(p\)-operator \(W_q(\Sigma)\), the Ward action inside correlation functions
whose other insertions are outside the isotopy region is
\[
  U_a(N)\,W_q(\Sigma)
  =
  q(a)^{\operatorname{Link}(N,\Sigma)}
  W_q(\Sigma)
\]
...
\]
This is the topological definition of the action written in linking-number
form.  Because \(U_a\) is topological, move \(N\) through the complement of
\(\Sigma\) without changing the correlator.  The only change occurs when the
swept chain crosses \(\Sigma\).  A single positive crossing implements the
symmetry action on \(W_q\), which is multiplication by the character
\[
  q(a)\in U(1).
\]
...
\(\partial B=N\).  This is the linking number.  Homological invariance of the
intersection pairing extends the formula to the Alexander-duality definition
of linking.  For \(\mathbb Z_N\), the character group is canonically
\(\mathbb Z_N\) after choosing the generator
\[
s(r)=\exp(2\pi i\,rs/N),
\]
