...
\[
  d_T=d-\iota_{\xi_M}
\]
be the Cartan differential at \(\xi\in\mathfrak t\).  Let
\(\alpha(\xi)\in\Omega_T^\bullet(M)\) be \(d_T\)-closed and \(T\)-invariant.
Write
\[
  M^T=\bigsqcup_a F_a
\]
for the fixed components, \(i_a:F_a\hookrightarrow M\) for the inclusions, and
\(N_a\) for their oriented normal bundles.  Assume every \(T\)-weight on
\(N_a\) is nonzero, so that the equivariant Euler class \(e_T(N_a)(\xi)\) is
invertible after localizing the coefficient ring at the nonzero weights.  Then
\begin{equation}
  \int_M\alpha(\xi)
  =
  \sum_a
  \int_{F_a}
  \frac{i_a^*\alpha(\xi)}{e_T(N_a)(\xi)} .
  \label{eq:cartan-localization-fixed-component-formula}
\end{equation}
For a nonisolated component the integral on \(F_a\) means ordinary
integration of the top de Rham degree on \(F_a\).  If a zero-weight normal
direction is present, \(F_a\) was not the full fixed component, or the Euler
class is not invertible; formula
\eqref{eq:cartan-localization-fixed-component-formula} is then not a theorem
without first replacing the fixed locus or adding extra data.
\end{theorem}
...
\[
  x_j+w_j(\xi),\qquad w_j(\xi)\ne0 .
\]
Thus \(e_T(N_a)(\xi)=\prod_j(x_j+w_j(\xi))\) becomes invertible in the
localized Cartan coefficient ring.  The equivariant Thom class
...
to \(M^T\) is injective after the same coefficient localization;
equivalently, outside the fixed set the equivariant contraction with
\(\xi_M\) supplies a homotopy inverse for \(d_T\).  Hence
\[
  \alpha
  =
...
\eqref{eq:cartan-localization-fixed-component-formula}.
\end{proof}
...
\begin{proof}
Set \(u=t^{-1/2}\xi\) and \(\eta=t^{-1/2}\zeta\).  The even Jacobian gives
