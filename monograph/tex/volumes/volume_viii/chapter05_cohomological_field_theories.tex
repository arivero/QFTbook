...
The theorem used by cohomological path integrals is not the assertion that the
zero set of the bosonic body of an arbitrary superfunction automatically
carries an Euler denominator.  The safe finite-dimensional statement is the
Atiyah--Bott--Berline--Vergne localization theorem in the Cartan model
\cite{AtiyahBott1984MomentMap,BerlineVergne1982Localization}.  In that model
the residual supermanifold over a fixed component \(F\) is \(\Pi TF\): its odd
directions are the de Rham variables on \(F\).  The normal odd directions are
\(\Pi N_F\), and the denominator is the equivariant Euler class of the
\(Q^2\)-normal bundle, not a formal symbol attached to a zero set.

\begin{theorem}[Compact Cartan localization]
\label{thm:compact-nondegenerate-cohomological-localization}
Let \(T\) be a compact torus acting smoothly on a compact oriented manifold
\(M\).  Write \(S_T=H_T^\bullet({\rm pt})=\operatorname{Sym}(\mathfrak t^*)\)
and use the Cartan differential
\[
  d_T\alpha(X)=d\alpha(X)-\iota_{X_M}\alpha(X),
  \qquad X\in\mathfrak t .
\]
Let \(\alpha\in\Omega_T^\bullet(M)\) be \(d_T\)-closed.  Write
\[
  M^T=\bigsqcup_a F_a
\]
for the fixed components, \(i_a:F_a\hookrightarrow M\) for the inclusions, and
\(N_a\) for their oriented normal bundles.  Let \(\mathcal S\subset S_T\) be
the multiplicative set generated by all nonzero \(T\)-weights that occur in
the normal bundles \(N_a\).  Then, in \(\mathcal S^{-1}S_T\)-valued
...
\begin{equation}
  ...
  \sum_a
  \int_{F_a}
  \frac{i_a^*\alpha}{e_T(N_a)} .
  \label{eq:cartan-localization-fixed-component-formula}
\end{equation}
For a nonisolated component the integral on \(F_a\) means ordinary
integration of the top de Rham degree on \(F_a\), leaving a rational function
of the equivariant variables.  Evaluation at a numerical
\(\xi\in\mathfrak t\) is a separate operation: it is allowed in this formula
only when every normal weight \(w\) satisfies \(w(\xi)\ne0\).  In that regular
case \(M^\xi=M^T\), \(e_T(N_a)(\xi)\) is invertible, and
\eqref{eq:cartan-localization-fixed-component-formula} can be evaluated at
\(\xi\).  On a weight hyperplane, \(M^\xi\) is larger; the zero-evaluation
normal directions must be absorbed into the fixed component and integrated as
residual collective coordinates, not inverted.
\end{theorem}

\begin{proof}
Choose a \(T\)-invariant tubular neighborhood of each \(F_a\).  The normal
bundle splits, after complexification and passage to the splitting principle,
into weight summands whose equivariant Euler roots have the form
\[
  x_j+w_j,\qquad w_j\in\mathfrak t^*\setminus\{0\}.
\]
Thus \(e_T(N_a)=\prod_j(x_j+w_j)\) becomes invertible after localizing the
Cartan coefficient ring at the nonzero characters \(w_j\).  No numerical
generator \(\xi\) has been chosen at this stage.  The equivariant Thom class
\(\tau_a\) of \(N_a\) satisfies the self-intersection identity
\[
  i_a^*\,i_{a*}\beta=e_T(N_a)\,\beta .
\]
Consequently \(i_{a*}(i_a^*\alpha/e_T(N_a))\) has the same restriction to
\(F_a\) as \(\alpha\).  The Borel localization lemma says that restriction
to \(M^T\) is injective after the same coefficient localization.  Hence
\[
  \alpha
  =
  \sum_a i_{a*}\!\left(\frac{i_a^*\alpha}{e_T(N_a)}\right)
  +d_T\gamma
\]
in localized equivariant cohomology.  Integrating over compact \(M\) kills the
\(d_T\gamma\) term by Stokes and gives
\eqref{eq:cartan-localization-fixed-component-formula}.  If one later chooses
a regular \(\xi\), the evaluated vector field \(\xi_M\) has the same zero set
\(M^T\), and the usual local contraction homotopy on \(M\setminus M^T\) is the
evaluated version of the same localization argument.  If \(w_j(\xi)=0\) for
some normal weight, that homotopy degenerates in the corresponding normal
direction, so the proof localizes to \(M^\xi\), not to \(M^T\).
\end{proof}

\begin{lemma}[Local super-Gaussian form of the inverse Euler factor]
\label{lem:cohomological-normal-gaussian-euler-factor}
Let \(F\subset M\) be one fixed component in
Theorem~\ref{thm:compact-nondegenerate-cohomological-localization}.  Suppose a
\(Q\)-exact deformation gives, in a \(T\)-equivariant tubular neighborhood,
normal even coordinates \(u\in N_{F,y}\), odd normal coordinates
\(\eta\in\Pi N_{F,y}\), and
\begin{equation}
  QV
  =
...
\end{equation}
where \(A_y\) is positive on the chosen real normal contour, \(C_y\) is
skew-nondegenerate on the odd normal variables, \(R_3\) has normal degree at
least three, and \((QV)|_F=0\).  Assume also that the insertion
\(\mathcal I=X\exp(-S)\mu\) has a well-defined restriction
\(\mathcal I|_F\), that all terms with positive normal degree are killed in
the rescaled limit, and that the even normal Gaussian gives a uniform
coefficientwise majorant for the Berezin coefficients.  Then
\[
  \lim_{t\to+\infty}
  \int_{\text{normal fiber}}
  \mathcal I\,\exp(-tQV)
  =
  \mathcal I|_F\,
  (2\pi)^{\dim N_F/2}
...
\]
This local factor represents \(e_T(N_F)^{-1}\) only when the quadratic normal
complex is the Cartan normal complex of the \(T\)-action, with the declared
orientation and square-root convention.  Otherwise it is merely a one-loop
Gaussian density and must not be called an Euler denominator.
\end{lemma}

\begin{proof}
Set \(u=t^{-1/2}\rho\) and \(\eta=t^{-1/2}\zeta\).  The even Jacobian gives
\(t^{-\dim N_F/2}\), while the Berezin change of odd variables gives the
inverse scaling power.  In the balanced normal complex these powers cancel
against the normal Berezin degree of the leading odd polynomial.  The
remainder satisfies
\[
  t\,R_3(y;t^{-1/2}\rho,t^{-1/2}\zeta)=O(t^{-1/2})
\]
coefficient by coefficient on bounded normal sets.  The positive form \(A_y\)
gives a Gaussian majorant for each ordinary coefficient, so the limit can be
taken coefficientwise before Berezin integration.  The remaining normal
integral is
\begin{equation}
  (2\pi)^{\dim N_F/2}
  \frac{\operatorname{Pf}(C_y)}{\det(A_y)^{1/2}} .
...
\end{equation}
Under changes of oriented normal coordinates, the ordinary determinant, the
Pfaffian, and the Berezinian transform in the compensating way.  Thus the
factor glues to a density on \(F\).  It is identified with the inverse
equivariant Euler class precisely when \(C\) and \(A\) are the odd and even
representatives of the equivariant normal complex in the Cartan model.
\end{proof}

\begin{example}[The \(S^2\) fixed-point denominator]
\label{ex:cohomological-s2-fixed-point-denominator}
Let \(S^1\) rotate \(S^2\), use coordinates \(x\in[-1,1]\) and
\(\varphi\in\mathbb R/2\pi\mathbb Z\), and orient by
\(\omega=d\varphi\wedge dx\).  With generator \(\partial_\varphi\), parameter
\(\lambda\), and Hamiltonian \(H=x\), the Cartan-closed form is
\[
  \Omega_\lambda=\omega+\lambda H .
\]
Direct integration gives
\[
  \int_{S^2}e^{\Omega_\lambda}
  =
  2\pi\int_{-1}^{1}e^{\lambda x}\,dx
  =
  2\pi\frac{e^\lambda-e^{-\lambda}}{\lambda}.
\]
The two fixed points have \(H(N)=1\), \(H(S)=-1\), and tangent weights
\(w_N=1\), \(w_S=-1\), so the fixed-point side is
\[
  2\pi
  \left(
    \frac{e^{\lambda H(N)}}{\lambda w_N}
    +
    \frac{e^{\lambda H(S)}}{\lambda w_S}
  \right)
  =
  2\pi\frac{e^\lambda-e^{-\lambda}}{\lambda}.
\]
Here the denominator is the actual equivariant Euler class of the normal
tangent line at a fixed point.
\end{example}

\begin{example}[Zero normal weight obstruction]
\label{ex:cohomological-zero-weight-localization-failure}
Let \(S^1\) rotate the \(S^2\) factor of \(S^2\times S^1\) and act trivially on
the second factor.  The fixed locus is not the two points of the sphere; it is
\[
  (N\times S^1)\sqcup(S\times S^1).
\]
If one incorrectly tries to localize to \(N\times\{\theta_0\}\), the
\(\theta\)-direction is a weight-zero normal direction.  The equivariant Euler
class contains a zero factor and is not invertible.  Analytically, a Gaussian
localizer has no quadratic confinement in the \(\theta\)-direction, so the
large-\(t\) limit still integrates over the whole residual \(S^1\).  This is
the finite-dimensional shadow of the field-theory warning: collective
coordinates, gauge zero modes, and moduli must be integrated over, not hidden
inside a one-loop denominator.
\end{example}

\begin{example}[Evaluation on a weight hyperplane]
\label{ex:cohomological-rank-two-weight-hyperplane}
Let \(T^2\) rotate the two factors of \(S^2\times S^2\), with equivariant
parameters \(\lambda_1,\lambda_2\).  Symbolically, the \(T^2\)-fixed locus has
four points
\[
  (N,N),\quad (N,S),\quad (S,N),\quad (S,S),
\]
and the normal Euler denominators are products of the nonzero characters
\(\pm\lambda_1\) and \(\pm\lambda_2\).
This is a statement over the localized coefficient ring
\(\mathbb R[\lambda_1,\lambda_2][(\lambda_1\lambda_2)^{-1}]\).

Now evaluate at the one-parameter generator \(\xi=(1,0)\).  The character
\(\lambda_2\) is nonzero in \(S_T\), but \(\lambda_2(\xi)=0\).  The vector
field generated by \(\xi\) rotates only the first sphere, so its zero set is
\[
  M^\xi=(N\times S^2)\sqcup(S\times S^2),
\]
not the four \(T^2\)-fixed points.  The second-sphere tangent direction is a
residual tangent direction along \(M^\xi\), and the correct localization for
the evaluated \(S^1_\xi\)-action has normal weights only
\(\pm\lambda_1(\xi)\).  Trying to evaluate the four-point \(T^2\) formula at
\(\lambda_2=0\) is therefore not a harmless specialization; it attempts to
divide by the volume of an unlifted zero mode.
\end{example}

If the integration cycle has boundary or noncompact ends, the
\(t\)-independence step acquires the corresponding boundary flux.  If the
fixed set is noncompact, one needs either compact support, a relative cycle,
or an independent convergence theorem.  If the insertion has normal Taylor
terms that survive the rescaling in
Lemma~\ref{lem:cohomological-normal-gaussian-euler-factor}, the answer is a
stationary-phase expansion with descendant normal contractions rather than the
single leading term \(X|_F/e_T(N_F)\).
