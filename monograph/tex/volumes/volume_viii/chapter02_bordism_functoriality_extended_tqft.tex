...
\begin{theorem}[Cobordism-hypothesis boundary]
\label{thm:cobordism-hypothesis-boundary}
Let \(\mathcal C\) be a symmetric monoidal \((\infty,D)\)-category satisfying
the dualizability and adjointability hypotheses required in the cobordism
hypothesis.  Fully extended framed \(D\)-dimensional topological field
theories with target \(\mathcal C\) are classified by fully dualizable
objects of \(\mathcal C\), with equivalence understood as equivalence of
symmetric monoidal higher functors.
\end{theorem}

This theorem is used here as a boundary statement.  Whenever an example is
claimed to define an extended theory, the target category, dualizability,
anomaly, and boundary data must be supplied.
