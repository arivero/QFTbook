...
\begin{hypothesis}[A-model cohomological sigma-model structure]
\label{hyp:a-model-cohomological-sigma-datum}
A closed A-model cohomological structure is a tuple
\[
  \mathfrak A_X
  =
  \bigl(
...
\end{enumerate}
This is a hypothesis about the cohomological QFT layer.  The
finite-dimensional Gromov--Witten package \(\mathfrak V_X\) is an output
expected from localization once this structure is constructed.  The continuum QFT
construction itself is the separate problem recorded in
Open Problem~\ref{op:topological-sigma-model-continuum-qft}.
\end{hypothesis}

\begin{hypothesis}[Closed B-model cohomological sigma-model structure]
\label{hyp:b-model-cohomological-sigma-datum}
A closed B-model cohomological structure is a tuple
\[
  \mathfrak B_X
  =
  \bigl(
    \Sigma,j_\Sigma; X,J_X,g_X,\omega_X,B_X;
    \mathcal F_{\Sigma,X},Q_B,{\rm PV}^{\bullet,\bullet}(X),
    \mathfrak a_B,\tau_K,\Omega_X,{\rm Tr}_{\Omega_X},\mathfrak c_B
  \bigr)
\]
with the following entries.  The parent two-dimensional
\(\mathcal N=(2,2)\) sigma model has compact K\"ahler target
\((X,J_X,g_X,\omega_X)\) and a closed \(B\)-field \(B_X\).  Torsionful or
generalized-K\"ahler parents are separate models and are not included in this
datum.  The local scalar complex is
\({\rm PV}^{p,q}(X)=\Omega^{0,q}(X,\wedge^pT^{1,0}X)\), with
\(Q_B=\bar\partial\).  The entry \(\mathfrak a_B\) is the chosen
axial-anomaly trivialization for the \(B\)-twist, including possible torsion
and higher-genus determinant-line data.  The entry
\(\tau_K:\mathcal O_X\simeq K_X\) is a holomorphic trivialization of the
canonical bundle, and
\(\Omega_X=\tau_K(1)\) is the chosen nowhere-vanishing holomorphic volume
form.  The trace
\[
  {\rm Tr}_{\Omega_X}(\mu)
  =
...
integrand top degree.  The structure also includes the regulated
\(Q_B\)-invariant integration cycle and the contact-term prescription
\(\mathfrak c_B\).  Its first-order deformation complex is the Dolbeault
polyvector differential graded Lie algebra
\[
...
\begin{definition}[B-model target and anomaly layers]
\label{def:b-model-target-anomaly-layers}
The entries in Hypothesis~\ref{hyp:b-model-cohomological-sigma-datum} live at
different layers.
\begin{enumerate}[label=\textnormal{(\roman*)}]
\item The ordinary Lagrangian \(\mathcal N=(2,2)\) parent uses a K\"ahler
  target and a closed \(B\)-field.  This is the kinetic and supersymmetry
  datum, not the holomorphic trace datum.
\item The scalar local \(B\)-model complex
  \(({\rm PV}^{\bullet,\bullet}(X),\bar\partial)\) uses the complex structure
  of \(X\).  It can be written before a holomorphic volume form is chosen.
\item The perturbative axial \(U(1)\) anomaly is represented by the first
  Chern class of \(T^{1,0}X\), equivalently by the Ricci-form class in de
  Rham cohomology.  Vanishing of this class cancels the local continuous
  anomaly; torsion and global determinant-line phases are separate data in
  \(\mathfrak a_B\).
\item The trace in the displayed formula requires a holomorphic
  trivialization \(K_X\simeq\mathcal O_X\) and a chosen volume form
  \(\Omega_X\).  This is stronger than the de Rham statement that the Ricci
  class vanishes and is not supplied by the local BRST complex.
\item All-genus closed strings, BCOV theory, and open \(B\)-models require
  the relevant determinant or anomaly line to be trivialized over the family
  of worldsheets and backgrounds.  Those global choices are part of
  \(\mathfrak a_B\) and are not consequences of the genus-zero trace alone.
\end{enumerate}
\end{definition}
...
For the A-model structure of
Hypothesis~\ref{hyp:a-model-cohomological-sigma-datum}, the target data are an
...
\[
  ...
  d\omega=0,
\]
and \(J\) is compatible with \(\omega\).  For the B-model structure of
Hypothesis~\ref{hyp:b-model-cohomological-sigma-datum}, the target is a
compact K\"ahler manifold \(X\) for the ordinary \(\mathcal N=(2,2)\)
Lagrangian parent.  The scalar \(B\)-model observables depend on the complex
structure; the trace-level closed model additionally uses the holomorphic
canonical trivialization \(\tau_K\) and volume form \(\Omega_X\).  Thus the
K\"ahler metric, the axial-anomaly condition, and the holomorphic volume form
are related inputs in one QFT, but they are not interchangeable hypotheses.
...
\begin{proposition}[Worldsheet-instanton sector as an amplitude datum]
\label{prop:amodel-worldsheet-instanton-amplitude-datum}
Fix the A-model datum of
Hypothesis~\ref{hyp:a-model-cohomological-sigma-datum}.  For a curve class
\(\beta\), the finite-dimensional expression
\begin{equation}
  Q^\beta
  \int_{[\overline{\mathcal M}_{g,n}(X,\beta)]^{\rm vir}}
  \bigwedge_{a=1}^n{\rm ev}_a^\ast\alpha_a
  \label{eq:amodel-fixed-beta-localized-amplitude}
\end{equation}
is the localized amplitude assigned to the \(\beta\) worldsheet-instanton
sector only after the determinant, orientation, compactification, and
contact-term data in \(\mathfrak V_X\) and \(\mathfrak c_A\) have been fixed.
The bare space of \(J\)-holomorphic maps is not by itself the QFT amplitude.
\end{proposition}

\begin{proof}
The topological term in the action is constant on the component
\(\phi_\ast[\Sigma]=\beta\), giving the factor \(Q^\beta\).  The remaining
\(Q_A\)-exact positive term localizes the bosonic zero locus to
\(\bar\partial_J\phi=0\).  Its linearization is the Fredholm operator
\(D_\phi\).  Even and odd nonzero modes contribute a determinant-line factor
for the virtual index of \(D_\phi\), not an automatic scalar.  Choosing the
orientation, obstruction, gluing, and compactification data in
\(\mathfrak V_X\) identifies this determinant-line datum with the virtual
integration cycle.  Local observables \(\mathcal O_{\alpha_a}\) restrict to
the evaluation pullbacks \({\rm ev}_a^\ast\alpha_a\).  Finally, coincident
marks, nodal boundary strata, and source collisions are interpreted using the
contact prescription \(\mathfrak c_A\).  These steps produce
\eqref{eq:amodel-fixed-beta-localized-amplitude}.
\end{proof}

\begin{proposition}[Zero-mode saturation and the A-model selection rule]
\label{prop:amodel-zero-mode-selection-rule}
Let
\[
  v_{g,n,\beta}
  =
  ...
  \overline{\mathcal M}_{g,n}(X,\beta).
\]
After the nonzero modes have been paired by the cohomological regulator and
the determinant line has been oriented by \(\mathfrak V_X\), the fixed
\(\beta\) primary amplitude is a top-degree extraction on the remaining
zero-mode space.  Therefore
\[
  \left\langle \alpha_1,\ldots,\alpha_n
  \right\rangle_{g,n,\beta}^{X}=0
  \quad\text{unless}\quad
  \sum_{a=1}^n\deg_{\mathbb R}\alpha_a=v_{g,n,\beta}.
\]
...
\begin{equation}
  ...
  \right\rangle_{g,n,\beta}^{X}=0
  \quad\text{unless}\quad
  \sum_{a=1}^n\bigl(\deg_{\mathbb R}\alpha_a+2k_a\bigr)
  =
  v_{g,n,\beta}.
  \label{eq:amodel-descendant-zero-mode-selection}
\end{equation}
Equivalently, in complex degree
\(|\alpha|_{\mathbb C}=\deg_{\mathbb R}\alpha/2\),
\begin{equation}
  \sum_{a=1}^n\bigl(|\alpha_a|_{\mathbb C}+k_a\bigr)
  =
  (1-g)(m-3)+n+\int_\beta c_1(TX).
  \label{eq:amodel-complex-degree-selection}
\end{equation}
\end{proposition}

\begin{proof}
On a regular component of the holomorphic-map locus, the odd scalar zero modes
are identified with differentials on the finite-dimensional zero-mode
manifold.  The insertion \(\mathcal O_{\alpha_a}\) restricts to the form
\({\rm ev}_a^\ast\alpha_a\).  Berezin integration over the odd zero modes is
therefore ordinary top-form extraction: a form of total real degree smaller
than \(v_{g,n,\beta}\) leaves unsaturated odd zero modes, while a form of
larger degree has no component of the required top degree.  On a nonregular
component the same statement is imposed on the virtual tangent-obstruction
complex by the orientation and obstruction data in \(\mathfrak V_X\).  The
classes \(\psi_a^{k_a}\) have real degree \(2k_a\), so descendants simply add
the terms \(2k_a\) in
\eqref{eq:amodel-descendant-zero-mode-selection}.  Substituting the virtual
dimension formula gives
\eqref{eq:amodel-complex-degree-selection}.
\end{proof}

Thus the selection rule is not a slogan about holomorphic maps.  It is the
zero-mode saturation condition for the regulated instanton sector after the
nonzero-mode determinant and virtual orientation have supplied a scalar
measure.  In the primary case,
Proposition~\ref{prop:amodel-zero-mode-selection-rule} specializes to the
familiar dimension rule: the integral vanishes unless
\[
...
\begin{proposition}[Degree-one projective instanton]
\label{prop:projective-degree-one-instanton-relation}
In the Gromov--Witten normalization used above,
\[
  \left\langle H^a,H^b,H^c\right\rangle_{0,3,1}^{\mathbb P^m}
  =
  \begin{cases}
    1, & a+b+c=2m+1,\\
    0, & a+b+c\neq 2m+1
...
\begin{proof}
The expected complex dimension of
...
\]
can be nonzero only when \(a+b+c=m+(m+1)d\).  For \(d=1\), the moduli space can
be described on its open degree-one stratum as a line
\(\ell\subset\mathbb P^m\) together with three marked points on \(\ell\):
\[
  \dim_\mathbb C{\rm Gr}(2,m+1)+3=(2m-2)+3=2m+1.
\]
Equivalently, use the automorphism group of the domain to put the three marked
points at \(0,\infty,1\in\mathbb P^1\).  A degree-one map is represented by
two independent vectors \(A,B\in\mathbb C^{m+1}\), up to common scale, through
\[
  [s:t]\longmapsto [sA+tB].
\]
Generic representatives of \(H^a,H^b,H^c\) are linear subspaces
\(L_a,L_b,L_c\subset\mathbb P^m\) of codimensions \(a,b,c\).  The conditions
\(A\in\widehat L_a\), \(B\in\widehat L_b\), and
\(A+B\in\widehat L_c\) impose \(a+b+c\) independent linear equations on
\((A,B)\in\mathbb C^{m+1}\oplus\mathbb C^{m+1}\).  If
\(a+b+c=2m+1\), the solution space is one-dimensional and therefore gives one
projective degree-one map.  If the codimensions do not sum to \(2m+1\), the
dimension rule gives zero.  This proves the displayed degree-one invariant.

For \(d=0\), the invariant is the classical intersection number and gives
\(H^a\star H^b=H^{a+b}\) when \(a+b\le m\).  If \(a+b=m+1\), write
\[
  H^a\star H^b=\sum_{r=0}^{m} C_r(Q)H^r .
\]
The Poincar\'e pairing on \(\mathbb P^m\) satisfies
\((H^r,H^c)=1\) exactly when \(r+c=m\), so \(C_r(Q)\) is detected by pairing
with \(H^{m-r}\).  The defining formula for \(\star\) gives
\[
  (H^a\star H^b,H^{m-r})
  =
  \sum_{d\ge0}Q^d
  \left\langle H^a,H^b,H^{m-r}\right\rangle_{0,3,d}.
\]
For \(d=0\), the degree rule would require
\[
  a+b+(m-r)=m,
\]
which is impossible because \(a+b=m+1\).  For \(d=1\), the degree rule
requires
\[
  a+b+(m-r)=2m+1,
\]
hence \(r=0\).  For \(d\ge2\), the required degree is larger than
\(a+b+m=2m+1\).  Therefore
\[
  (H^a\star H^b,H^m)=Q,
  \qquad
  (H^a\star H^b,H^{m-r})=0\quad (r>0),
\]
and nondegeneracy of the Poincar\'e pairing gives
\[
  H^a\star H^b=Q\cdot 1 .
\]
Taking \(a=m\) and \(b=1\) gives \(H^{m+1}=Q\).  Associativity then determines
all products.
\end{proof}
...
Let \(X\) be the compact K\"ahler target in
Hypothesis~\ref{hyp:b-model-cohomological-sigma-datum}.  The local scalar
complex is defined from the complex structure alone.  Define the polyvector
Dolbeault complex
\[
  {\rm PV}^{p,q}(X)
  =
...
After the trace-level datum \(\tau_K\) has been chosen, the holomorphic
volume form \(\Omega_X=\tau_K(1)\) converts polyvectors into forms by
contraction,
\[
...
Rescaling \(\Omega_X\mapsto\lambda\Omega_X\) rescales this trace by
\(\lambda^2\).  Thus the trace is part of the normalization data of the
closed \(B\)-model and is fixed by the chosen holomorphic volume form, whereas
the cohomology class \(c_1(T^{1,0}X)\) records the anomaly class.

\begin{example}[\(c_1\) and holomorphic triviality are different tests]
\label{ex:bmodel-c1-volume-form-separation}
On an elliptic curve \(E\), a nontrivial flat holomorphic line bundle
\(L\in{\rm Pic}^0(E)\) has \(c_1(L)=0\) as a topological Chern class, but
\(L\not\simeq\mathcal O_E\) holomorphically and it has no nowhere-vanishing
holomorphic section.  This is the basic line-bundle reason why a Chern-class
statement does not choose a holomorphic trivialization.

For a canonical-bundle example closer to the \(B\)-model, an Enriques surface
is compact K\"ahler and has \(K_X^{\otimes2}\simeq\mathcal O_X\) while
\(K_X\not\simeq\mathcal O_X\).  Its canonical class is torsion, hence
invisible in real de Rham \(c_1\), but \(H^0(X,K_X)=0\).  Therefore there is
no holomorphic two-form \(\Omega_X\) and the trace
\({\rm Tr}_{\Omega_X}\) above is unavailable.  A K3 surface passes the
trace-level test: \(K_X\simeq\mathcal O_X\) and a nonzero holomorphic
two-form can be chosen.
\end{example}
...
\(\mathcal N=(2,2)\) sigma model.  The perturbative anomaly of this symmetry
is measured by \(c_1(T^{1,0}X)\).  Cancellation of the local continuous
anomaly is the condition that the corresponding de Rham class vanish, while
integral torsion and families of worldsheets can still define global
determinant-line data.  The holomorphic trace used in the closed genus-zero
\(B\)-model is a further condition: \(K_X\) must be holomorphically trivial
and a volume form \(\Omega_X\) must be chosen.  Boundary conditions and open
\(B\)-models require additional Chan--Paton and anomaly data.
