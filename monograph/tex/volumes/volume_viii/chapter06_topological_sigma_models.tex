...
\(H^a\star H^b=H^{a+b}\) when \(a+b\le m\).  If \(a+b=m+1\), pairing
\(H^a\star H^b\) with \(1\) receives exactly the degree-one contribution above,
so \(H^a\star H^b=Q\).  Taking \(a=m\) and \(b=1\) gives \(H^{m+1}=Q\).
Associativity then determines all products.
\end{proof}
