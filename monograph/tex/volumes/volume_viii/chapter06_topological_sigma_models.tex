...
For the A-model, the target datum is an almost K\"ahler manifold
...
\[
  ...
  d\omega=0,
\]
and \(J\) is compatible with \(\omega\).  For the B-model, the target datum is
a compact complex manifold \(X\) with trivial canonical bundle and a choice of
...
\begin{proof}
It suffices to compute in a coordinate chart.  Since \(Q_A\phi^i=\psi^i\) and
\(Q_A\psi^i=0\),
\[
...
\begin{lemma}[Pointwise energy identity]
\label{lem:tqft-a-model-energy-identity}
Choose a conformal metric on \(\Sigma\).  With the norm on
\(\Omega^{0,1}(\Sigma,\phi^\ast TX)\) normalized by
\[
  |\bar\partial_J\phi|^2
  =
  \frac12
...
\begin{proposition}[Index of the fixed-domain deformation complex]
\label{prop:tqft-fixed-domain-index}
Let \(m=\dim_\mathbb C X\), \(g\) be the genus of \(\Sigma\), and
\(\beta=\phi_\ast[\Sigma]\).  The real Fredholm index of the deformation
complex
\[
  \Gamma(\Sigma,\phi^\ast TX)
  \xrightarrow{D_\phi}
...
curve moduli space to Proposition~\ref{prop:tqft-fixed-domain-index}; the
unstable cases are defined by quotienting by the automorphism group of the
marked curve.
...
Assume a virtual integration theory satisfying orientation, pullback,
gluing, and deformation-invariance axioms.  For cohomology classes
\(\alpha_a\in H^\bullet(X;\mathbb C)\), define
\[
...
\begin{theorem}[Associativity from the splitting axiom]
\label{thm:tqft-quantum-product-associativity}
Assume the genus-zero virtual fundamental classes satisfy the standard
splitting axiom along boundary divisors of
\(\overline{\mathcal M}_{0,4}(X,\beta)\).  Then \(\star\) is associative.
\end{theorem}
...
The proof shows precisely where the geometry enters: compactness of the
...
\]
In the fixed-worldsheet cohomological field theory, descendants arise from
...
\begin{proposition}[B-model dependence on complex structure]
...
Gromov--Witten theory.  The B-model has a finite-dimensional classical
closed-string sector expressed through polyvector Dolbeault cohomology and,
at higher genus, through the Kodaira--Spencer/BCOV effective theory.  These
...
maps for cutting and sewing worldsheets.  The construction should recover the
