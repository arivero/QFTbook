...
\[
  (\Hilb,\vac,U,\mathcal D,\{\Phi_\alpha\}_{\alpha\in I}).
\]
...
\[
  U(a,\Lambda)^{-1}\Phi_\alpha(f)U(a,\Lambda)
  =
  R(\Lambda)_\alpha{}^\beta\,
  \Phi_\beta(f_{a,\Lambda}),
\]
where
\[
...
\[
  [\Phi_\alpha(f),\Phi_\beta(g)]_{\rm gr}=0
\]
on \(\mathcal D\), where
\[
...
\[
  \operatorname{span}\{
    \Phi_{\alpha_1}(f_1)\cdots\Phi_{\alpha_n}(f_n)\vac
    \mid n\ge0
  \}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\scriptsize}]
    ...
    \node[box] (fields) at (4.2,1.3) {smeared fields\\\(\Phi_\alpha(f)\)};
...
The objects \(\Phi_\alpha(f)\) in the Wightman data are generally unbounded
...
\(\Phi_\alpha(f)\) with \(\operatorname{supp}f\subset\mathcal O\), together
with their adjoints on \(\mathcal D\).  This is an algebra of generally
unbounded operators on a common domain, not a \(C^*\)-algebra.  If the
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\scriptsize}]
    ...
    \node[box] (fields) at (-5.4,0.65)
      {field coordinates\\\(\Phi_\alpha(f)\)};
...
\begin{equation}
  ...
  =
  \bra{\vac}
  \Phi_{\alpha_1}(x_1)\cdots\Phi_{\alpha_n}(x_n)
  \ket{\vac}.
...
\[
  \Phi_\alpha(f)^*|_{\mathcal D}
  =
  \Phi_{\alpha^\dagger}(\overline f)|_{\mathcal D}
\]
...
\item[Covariance.]
The distribution \(W_n\) transforms under \(\mathcal P_+^\uparrow\) by the
...
\[
  \Phi_\alpha(h)[F]=[h_\alpha\otimes F],
\]
...
redundancy are represented in a positive Hilbert-space Wightman framework only
after the construction has specified the physical observable algebra.  Common
constructions introduce additional structure, such as an indefinite inner
product, BRST cohomology, a local algebra of gauge-invariant observables, or a
