...
\[
  U(b,1)=\exp(-\ii b^\mu P_\mu),
  \qquad b\in\mathbb R^D,
\]
in the convention of Chapter \ref{ch:starting-data-local-qft}.  Let \(E_P\) be
the joint spectral measure of \(P=(P_0,\ldots,P_{D-1})\).  The spectrum
condition is
\[
...
\[
  U(b,\Lambda)\Phi_\alpha(f)U(b,\Lambda)^{-1}
  =
  R(\Lambda)_\alpha{}^\beta\,
  \Phi_\beta(f_{b,\Lambda}),
\]
where
\[
  f_{b,\Lambda}(x)=f\bigl(\Lambda^{-1}(x-b)\bigr).
\]
