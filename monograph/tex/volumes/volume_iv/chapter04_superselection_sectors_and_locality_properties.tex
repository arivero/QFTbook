...
Let \(\Obs\) be the quasilocal algebra of a local net and let
\[
  \pi_0:\Obs\to B(\Hilb_0)
\]
...
\[
  \pi:\Obs\to B(\Hilb_\pi)
\]
is a sector when it describes states that cannot be coherently superposed with
states from another representation by observables in \(\Obs\).  Equivalence of
sectors is unitary equivalence of representations:
\[
  \pi_1\simeq\pi_2
...
representation \(\pi\) is localized in a bounded region \(O\) relative to the
vacuum if
\[
  \pi|_{\Obs(O^\perp)}\simeq \pi_0|_{\Obs(O^\perp)} ,
  \label{eq:dhr-localization}
\]
where \(O^\perp\) denotes the spacelike complement.  It is transportable if an
equivalent representation can be localized in any other suitable bounded
region.  A transportable localized sector is a charge that can be moved without
changing its intrinsic superselection class.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\scriptsize}]
    ...
    \node[region] (o) at (-1.85,0.2) {\(O\)};
    \node at (-1.85,-0.75) {charge localized};
    \node[outside] (comp) at (2.25,0.65) {outside \(O\)\\\(\pi\simeq\pi_0\)};
...
\(\pi|_{\Obs(O^\perp)}\) with \(\pi_0|_{\Obs(O^\perp)}\), the action of
\(\pi\) on \(\Obs\) may be represented by an endomorphism
\[
  \rho:\Obs\to\Obs
\]
localized in \(O\), meaning that
\[
  \rho(A)=A,\qquad A\in\Obs(O^\perp).
\]
Composition of localized endomorphisms defines fusion:
\[
  \rho_1\otimes\rho_2=\rho_1\circ\rho_2 .
\]
An intertwiner from \(\rho\) to \(\sigma\) is an observable \(T\in\Obs\) such
that
\[
  T\rho(A)=\sigma(A)T,\qquad A\in\Obs .
\]
...
\[
  d(\rho).
\]
For sectors reconstructed from a compact gauge group, \(d(\rho)\) is the
...
\section{Observable Algebra, Field Algebra, and Gauge Group}

The observable algebra \(\Obs\) contains neutral operators.  Charged fields,
when they are introduced as operators, can be organized in an enlarged algebra
\[
  \mathcal F
\]
with a compact internal gauge group \(G\) acting by automorphisms.  The
observable algebra is the fixed-point algebra
\[
  \Obs=\mathcal F^G .
\]
...
Let \(O_1\Subset O_2\) mean that the closure of \(O_1\) is contained in
\(O_2\) with a positive spacetime collar.  The split property is the existence
...
separation between observables well inside \(O_1\) and observables outside
\(O_2\).  It is a precise algebraic substitute for a finite-dimensional
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\scriptsize}]
    ...
    \node at (3.75,0.72) {\(O_2\)};
    \node at (1.5,0.28) {\(O_1\)};
    \node[alg] (r1) at (-3.45,-1.9) {\(\mathcal R(O_1)\)};
    \node[alg,fill=green!7] (n) at (0,-1.9) {type I factor\\\(\mathcal N\)};
    \node[alg] (r2) at (3.45,-1.9) {\(\mathcal R(O_2)\)};
    \draw[arrow] (r1)--(n);
    \draw[arrow] (n)--(r2);
    \node at (0,-2.75) {\(\mathcal R(O_1)\subset\mathcal N\subset\mathcal R(O_2)\)};
  \end{tikzpicture}
  }
