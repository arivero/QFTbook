...
\(\|F\|_{\rm OS}^2=\langle F,F\rangle_{\rm OS}\).  Contractivity implies
\(R_s\mathcal N\subset\mathcal N\), so \(R_s\) descends to a bounded operator
\(T_E(s)\) on the quotient and then to \(\Hilb_{\rm OS}\).  Equations
