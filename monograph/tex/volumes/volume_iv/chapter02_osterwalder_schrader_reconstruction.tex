...
domain for the reconstructed fields.  If \(h\) is a Euclidean test function
whose support is at positive time and remains ordered to the left of the
supports appearing in \(F\in\mathcal E_+\), define
\[
  \Phi_E(h)[F]
  =
  [h\otimes F],
...
null quotient whenever the supports obey the stated ordering.  Taking limits
of ordered supports gives the Euclidean fields as closable operators on
\(\mathcal D_{\rm OS}\).
...
\end{equation}
The reconstructed operator \(\Phi(t,\mathbf f)\) is the operator-valued
distribution whose vacuum matrix elements are these boundary values and whose
time dependence is implemented by the unitary group
\[
  \ee^{-\ii tH}.
\]
