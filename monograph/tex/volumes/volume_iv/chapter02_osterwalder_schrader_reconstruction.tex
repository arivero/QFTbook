...
enough to produce tempered Lorentzian boundary values.  Then there exists
Wightman field data
\[
...
\(T_E(s)\) on \(\Hilb_{\rm OS}\).  The Hille--Yosida theorem gives a closed
maximal dissipative generator \(A\), and the symmetry obtained from Euclidean
time reflection identifies this contraction semigroup with a self-adjoint
positive generator
\[
  H:=-A,\qquad T_E(s)=e^{-sH},\qquad \sigma(H)\subset[0,\infty).
\]
...
linear-growth estimate \eqref{eq:os-linear-growth-condition} is the analytic
input that permits the Euclidean rotation parameters mixing time and space to
be continued to real Lorentzian boosts as tempered boundary values.  This
...
Wightman distributions.  Permutation symmetry of the Euclidean hierarchy,
together with analytic continuation and the edge-of-the-wedge theorem, gives
local graded commutativity for spacelike separated Lorentzian test supports.
...
original \(S_n\).  The assertion is restricted to noncoincident Euclidean
points because contact extensions are part of the declared Schwinger
hierarchy rather than a consequence of analytic continuation away from the
diagonals.  Finally, every step above is functorial in the OS inner product
and the insertion operations: two reconstructions from the same hierarchy
identify the dense classes \([F]\), preserve all matrix elements, and hence
extend uniquely to a unitary equivalence.
\end{proof}
