...
\begin{proof}
The null space of the reflection-positive form is invariant under
\(T_Y(\tau)\): if \(u\) has zero norm, Cauchy--Schwarz for the positive
semidefinite doubled form gives
\(\langle u,w\rangle_Y=0\) for every positive-side state \(w\).  Taking
\(w=T_Y(2\tau)u\) and using the semigroup law and self-adjointness gives
\[
  \|T_Y(\tau)u\|_Y^2
  =
...
\]
Therefore
\(T_Y(\tau)\) descends to the quotient and then to the Hilbert completion.
The sewing axiom gives the semigroup law on the quotient.  Reflection gives
self-adjointness.  The doubled-cylinder positivity gives positivity.  The
normalization and continuity hypothesis give a strongly continuous
contraction semigroup.  Proposition~\ref{prop:ks-one-dimensional-positive-energy}
then supplies \(H_Y\ge0\) and the unitary boundary values.
\end{proof}
