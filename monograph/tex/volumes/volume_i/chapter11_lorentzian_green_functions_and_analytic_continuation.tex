...
\begin{proposition}[Free scalar Feynman boundary value]
\label{prop:free-scalar-feynman-boundary-value}
...
\begin{proposition}[Source sign under Wick rotation]
\label{prop:source-sign-under-wick-rotation}
...
\begin{proposition}[Two-point Feynman prescription from ordered tubes]
\label{prop:two-point-feynman-prescription-ordered-tubes}
...
the same damping choice as in
Proposition~\ref{prop:free-scalar-feynman-boundary-value}, so the positive
energy pole is passed below the real axis.  If \(x^0<0\), the operator order is
reversed and the contour closure selects the negative-energy pole above the
axis.  The two cases are precisely the two boundary values of the single
...
\begin{proposition}[Tadpole contour rotation and self-energy matching]
\label{prop:tadpole-contour-rotation-self-energy-matching}
...
not to the denominator self-energy scalar itself.  This proves
...
has imaginary value, so the propagator poles in each loop-energy variable
remain separated from the deformation from the real cycle to the imaginary
cycle.  A deformation theorem for meromorphic integrands then identifies the
Lorentzian integral with the Euclidean integral on that first-sheet domain.
...
\]
when \(k^0\ge3m\) in the rest frame.  At such a point poles approach the
integration cycle from opposite sides in the two loop-energy variables, so
the cycle is pinched and cannot be deformed without crossing a singularity.
The Lorentzian value above threshold is therefore a boundary value on a cut of
the first-sheet analytic function, not an ordinary Euclidean rotation.
\end{proof}
...
\begin{proposition}[Spectral thresholds and first-sheet cuts]
\label{prop:spectral-thresholds-first-sheet-cuts}
...
\begin{proof}
The K\"all\'en--Lehmann representation writes the connected two-point
