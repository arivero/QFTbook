...
studied.
With a sharp Fourier cutoff this finite-dimensional statement may be displayed
schematically as
\[
  \int[\dd\phi]_\Lambda
  \quad\leadsto\quad
...
\]
up to the usual choice of independent real coordinates and normalization
constants. The symbol \([\dd\phi]\) is never used here as an unregulated
...
\[
  \left(-\frac{g}{4!}\right)
  \left(4\cdot\frac{3!}{2}\right)
  =
  -\frac{g}{2}.
\]
Here the \(4\) chooses the vertex field contracted with one external endpoint,
the remaining \(3!\) orders the second external contraction together with the
two loop half-edges, and the division by \(2\) removes the interchange of the
two half-edges forming the loop.
...
At fixed regulator, the expansion is an expansion in the formal parameter
\(g\). The coefficient \(C_1(\Lambda,m)\) grows with the cutoff in
\(D\ge2\), so the product \(gC_1(\Lambda,m)\) is not uniformly small as
\(\Lambda\to\infty\). Thus the distinction between a bare Lagrangian parameter
and a mass defined by spectral data is operational in the cutoff-dependent
calculation.
