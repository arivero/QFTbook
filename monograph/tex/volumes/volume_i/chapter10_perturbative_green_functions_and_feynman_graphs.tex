...
The Haag-theorem boundary of
Section~\ref{sec:haag-theorem-interaction-picture-boundary} is the Lorentzian
counterpart of this regulator warning: the formulas below are not claiming that
the continuum interacting field is globally unitarily conjugate to a free field
on the same fixed-time Wightman representation.
...
\begin{definition}[Finite Fourier cutoff Gaussian scalar datum]
\label{def:finite-fourier-cutoff-gaussian-scalar-datum}
...
normalization.  More explicitly, put the theory first on a Euclidean torus of
side \(L\), let \(\mathcal K_{\Lambda,L}\) be a finite symmetric set of
allowed momenta, and set
\[
  E_{\Lambda,L}
  =
  \left\{
    \widetilde\phi:\mathcal K_{\Lambda,L}\to\mathbb C:
    \widetilde\phi(-k)=\overline{\widetilde\phi(k)}
  \right\}.
\]
After choosing real coordinates on \(E_{\Lambda,L}\), the free expectation is
the centered Gaussian probability measure with covariance
\[
  \vev{\widetilde\phi(k)\widetilde\phi(k')}_{0,\Lambda,L}
  =
  L^D\,\frac{\delta_{k+k',0}}{k^2+m^2}.
\]
Equivalently, for a real source \(J\) whose Fourier transform is supported in
\(\mathcal K_{\Lambda,L}\),
\[
  Z_{0,\Lambda,L}[J]
  =
  \exp\left[
    \frac12
    \int_{T_L^D}\dd^Dx\,\dd^Dy\,
    J(x)C_{\Lambda,L}(x-y)J(y)
  \right].
\]
The infinite-volume smooth-cutoff formulas in this chapter are shorthand for
the corresponding limit or for the directly regulated covariance
\(C_\Lambda\).  The symbol \([\dd\phi]\) is not used here as an unregulated
...
\begin{aligned}
  ...
  \vev{\exp[-S_{\mathrm{int},E}[\phi]]}_{0,\Lambda,L}} .
\end{aligned}
\]
In infinite-volume shorthand the subscript \(L\) is suppressed.  It is a
distribution in the insertion points after the regulator is removed, when that
limit exists or has been renormalized. Perturbation theory is the
...
\paragraph{Coefficientwise perturbation at fixed regulator.}
...
This is finite-dimensional formal power-series algebra.  At finite regulator
the Gaussian expectation is an ordinary normalized
...
coefficient.  This algebraic step uses only finite-dimensional Gaussian
moments and the formal inverse of a series with constant term \(1\); regulator
removal and renormalization enter later.

\section{Wick Expansion and Connected Functions}

\paragraph{Wick graph expansion and connected cumulants.}
...
test functions or points.  Substituting the fixed-regulator formal
exponential expansion expresses each coefficient as a finite sum over pairings
of the \(n+4V\) field factors.
...
Figure~\ref{fig:normalized-correlator-component-selection} records this component
selection: vacuum components cancel against \(Z_\Lambda[0]\), and surviving
components contain external insertions.
...
For a connected graph with \(I(\Gamma)\) internal edges and \(V(\Gamma)\)
vertices, the number of independent loop momenta is
\[
  L(\Gamma)=I(\Gamma)-V(\Gamma)+1 .
\]
This is the first Betti number of the graph.  Momentum conservation at all
vertices gives \(V(\Gamma)\) linear constraints, but the sum of those
constraints is the external momentum-conservation delta function already
displayed, so only \(V(\Gamma)-1\) vertex constraints eliminate independent
internal edge momenta.
\end{definition}

Figure~\ref{fig:euclidean-phi-four-feynman-rules} fixes the two momentum-space
rules used below: the regulated scalar propagator on each edge and the quartic
vertex with momentum conservation.
...
\paragraph{Automorphism denominator from Wick pairings.}
...
\begin{definition}[Two-point one-particle irreducibility]
\label{def:two-point-one-particle-irreducibility}
A connected two-point graph is one-particle reducible if it contains an
internal edge whose removal disconnects the two external endpoints.  It is
one-particle irreducible, abbreviated \(1\mathrm{PI}\), if no such edge
exists.  The amputated reduced value of a \(1\mathrm{PI}\) two-point graph is
obtained by omitting the two external propagator factors attached to the
external endpoints while keeping the loop integrals, vertex factors, and graph
automorphism factor.

This is a statement about connected Green-function graphs.  Its role here is
only to organize the two-point function.  The systematic functional definition
of the one-particle-irreducible effective action is developed later in
Chapter~\ref{ch:volume-ii-generating-functionals-1pi}.
\end{definition}
...
\begin{proposition}[Dyson decomposition of the connected two-point function]
\label{prop:dyson-decomposition-two-point-function}
As a formal power series at fixed regulator, every connected two-point graph
decomposes uniquely into a chain of \(1\mathrm{PI}\) two-point blocks joined
by free propagator edges.  If \(\Sigma_E(k)\) denotes the sum of amputated
\(1\mathrm{PI}\) two-point insertions with the sign convention of
Definition~\ref{def:euclidean-self-energy}, then
\[
  \widetilde G_E(k)
  =
  G_{0,E}(k)
  +
  G_{0,E}(k)\Sigma_E(k)G_{0,E}(k)
  +
  G_{0,E}(k)\Sigma_E(k)G_{0,E}(k)\Sigma_E(k)G_{0,E}(k)
  +\cdots .
\]
Equivalently,
\[
  \widetilde G_E(k)=\frac{1}{G_{0,E}(k)^{-1}-\Sigma_E(k)}
  =
  \frac{1}{k^2+m^2-\Sigma_E(k)} .
\]
\end{proposition}
...
\begin{proof}
Work at finite regulator and at a fixed order in \(g\), so only finitely many
Wick graphs occur.  In a connected two-point graph, call an internal edge a
separating edge if deleting it disconnects the two external endpoints.  The
set of separating edges on any path from one external endpoint to the other is
linearly ordered by separation: an edge \(e_1\) precedes \(e_2\) if the
component containing the left external endpoint after deleting \(e_1\) is
contained in the corresponding component after deleting \(e_2\).  Cutting all
separating edges decomposes the graph into maximal subgraphs with two
distinguished attachment half-edges and no further separating internal edge
between them.  These maximal subgraphs are exactly the \(1\mathrm{PI}\)
two-point blocks; the cut separating edges are free propagator factors.

The decomposition is unique because the separating edges are intrinsic to the
incidence graph.  Conversely, any ordered chain of \(1\mathrm{PI}\) two-point
blocks joined by free propagators gives a connected two-point graph, and the
automorphism factors multiply correctly once the two attachment half-edges of
each block are distinguished by their position in the chain.  Summing over
chains therefore gives the displayed geometric series.  Since
\(G_{0,E}(k)=(k^2+m^2)^{-1}\), the formal geometric series is
\[
  G_{0,E}(k)\sum_{r\ge0}\bigl(\Sigma_E(k)G_{0,E}(k)\bigr)^r
  =
  \frac{1}{G_{0,E}(k)^{-1}-\Sigma_E(k)} ,
\]
...
\paragraph{One-loop tadpole self-energy and cutoff growth.}
\label{par:one-loop-tadpole-self-energy-cutoff-growth}
...
The coefficient \(-g/2\) is the symmetry factor from the Wick-pairing
automorphism count above: the two unattached half-edges at the quartic vertex
form one loop and may be interchanged.  No external momentum enters the loop
propagator, so the amputated insertion is momentum independent and equals the
displayed regulated integral.  In spherical coordinates the cutoff integral is
\[
  \frac12
  \frac{\operatorname{vol}(S^{D-1})}{(2\pi)^D}
...
\paragraph{Two-loop sunset insertion.}
...
\paragraph{One-loop pole shift and mass-coordinate prescription.}
\label{par:one-loop-pole-shift-mass-coordinate-prescription}
Using
\[
  \widetilde G_E(k)
  =
...
the tadpole calculation above into the
Dyson denominator:
\[
  k^2+m^2-\Sigma_E(k)
  =
...
\begin{remark}[Four-point moment-cumulant decomposition]
\label{rem:four-point-moment-cumulant-decomposition}
...
\end{aligned}
\]
The identity is not a separate theorem: applying the moment-cumulant identity
to four fields gives it directly.  If the theory is
...
Figure~\ref{fig:four-point-wick-connected-decomposition} displays the three pair
partitions and the connected four-point cumulant that appear in the
moment-cumulant identity.
...
\begin{definition}[Euclidean amputation of connected four-point Green functions]
\label{def:euclidean-amputation-connected-four-point}
Away from the free Euclidean pole hypersurfaces \(k_i^2+m^2=0\), define the
amputated connected Euclidean four-point kernel by the algebraic operation
\[
\begin{aligned}
  \widetilde\Gamma_{E,4}(k_1,k_2,k_3,k_4)
  &:=
  \left(\prod_{i=1}^4(k_i^2+m^2)\right)
  \widetilde G_{E,4}^{\mathrm{conn}}(k_1,k_2,k_3,k_4).
\end{aligned}
\]
This definition removes the external free propagator factors from the
connected Green function.  It is not a definition of a scattering amplitude:
scattering requires the nonperturbative one-particle subspace, asymptotic
states, and the LSZ limiting operation developed later.
\end{definition}

\paragraph{Tree-level four-point kernel.}
With the convention of
Definition~\ref{def:euclidean-amputation-connected-four-point}, the leading
amputated connected four-point kernel is
\[
  \widetilde\Gamma_{E,4}(k_1,k_2,k_3,k_4)
  =
  -(2\pi)^D g\,\delta^{(D)}(k_1+k_2+k_3+k_4)
  +O(g^2).
\]
This is obtained by multiplying the leading connected Green function in the
formula above by \(\prod_i(k_i^2+m^2)\).  The four free external propagators
cancel coefficientwise, leaving the momentum-conserving delta distribution
and the quartic vertex coefficient \(-g\).
