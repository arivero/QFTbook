...
momentum. The field variable is a real scalar function \(\phi\). The
interaction is
\[
  S_{\mathrm{int},E}[\phi]
  =
...
\]
where \(g\in\mathbb R\) is the quartic coupling. The free covariance is the
...
expectations. Formulas written without \(\Lambda\) are formal perturbative
formulas or shorthand for regulator-dependent formulas before the limit is
studied.
...
formal expansion of this expression in powers of \(g\).
...
\(|\operatorname{Aut}\Gamma|=2\), and the coefficient is \(-g/2\). The
single quartic contribution to the connected four-point function with four
labelled external insertions has trivial automorphism group, and its
coefficient is \(-g\). Vacuum components have already been removed by
normalization; the logarithm of the generating functional then selects the
component in which all external labels are connected.
...
\]
The factor \(1/2\) is the symmetry factor of the tadpole graph. With a sharp
momentum cutoff used only to display the degree of divergence,
\[
  C_1(\Lambda,m)
  =
  \frac{1}{2}
...
\[
  ...
  \sim
  \begin{cases}
    \dfrac{1}{4\pi}\log\Lambda, & D=2,\\[2mm]
    \dfrac{1}{2}
...
\(\int_0^\Lambda r(r^2+m^2)^{-1}\dd r=\log\Lambda+O(1)\).  The second line
uses \(r^{D-1}(r^2+m^2)^{-1}=r^{D-3}+O(r^{D-5})\) at large \(r\).  Hence
\[
...
The spectral representation of the Euclidean two-point function has the form
\[
  \widetilde G_E(k)
  =
  \frac{Z}{k^2+m_*^2}
...
\]
The counterterm is treated as part of the interaction. At order \(g\), the
two-valent counterterm vertex contributes
\[
...
The physical statement is made after Green functions are expressed in terms of
the chosen finite parameters, such as the pole mass \(m_*\).
