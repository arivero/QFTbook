...
\(\Phi_A(x)\) is an operator-valued distribution: its primary
operator expression is the smeared field
\[
  \Phi_A(f)
  =
  \int_{\mathbb M^D}\dd^Dx\,f(x)\Phi_A(x),
  \qquad
  f\in C_c^\infty(\mathbb M^D).
\]
The integral sign is distributional notation for evaluating the field on the
test function \(f\).  When \(\Phi_A(f)\) is unbounded, its common dense
...
\begin{definition}[Wightman field presentation]
  ...
  \[
    (\Hilb,\vac,U,\mathcal D,\{\Phi_A\}_{A\in I})
  \]
  ...
  \[
    ...
    \Phi_A(f)\mathcal D\subset\mathcal D\quad(A\in I,\ f\in\mathcal T),
  \]
  and, for every \(n\ge1\),
  \[
