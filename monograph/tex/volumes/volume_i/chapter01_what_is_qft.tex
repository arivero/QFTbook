...
\begin{definition}[Euclidean correlation presentation]
  ...
  \[
    S_n\in\mathcal T_E'(E^n;(V^*)^{\otimes n}),
    \qquad n\ge0,\qquad S_0=1.
  \]
  Here \(\mathcal T_E(E^n;V^{\otimes n})\) denotes the corresponding
  \(V^{\otimes n}\)-valued test-function space on \(E^n\), the prime denotes its
  continuous dual, and \(S_n(F)\) denotes the distributional pairing.
  ...
  The regularity and growth condition is also part of the data.  In the
  tempered presentation it means that, for each \(n\), there are constants
  \(C_n\) and \(N_n\) such that every
  \(F\in\mathcal S(E^n;V^{\otimes n})\) obeys a Schwartz-seminorm bound,
  using any fixed norm on the finite-dimensional space \(V^{\otimes n}\):
  \[
    |S_n(F)|
    \le
    C_n
    \sum_{|\alpha|,|\beta|\le N_n}
    \sup_{X\in E^n}
