...
\begin{definition}[Wightman field presentation]
  ...
  \]
  where \(\mathcal D\subset\Hilb\) is a dense invariant domain and
  \(\Phi_a\) are operator-valued distributions with common domain
  \(\mathcal D\).  Its primary correlation data are the vacuum distributions
  \[
...
\begin{definition}[Local-net presentation]
  A local-net presentation consists of a class of spacetime regions
  \(\mathcal K(\mathbb M^D)\), a quasilocal algebra \(\Obs_{\rm qloc}\), and
  an assignment
  \[
    \mathcal O\longmapsto\Obs(\mathcal O)\subset\Obs_{\rm qloc}
  \]
  satisfying isotony, locality, covariance, and any additional structural
  conditions imposed in the problem under study, such as additivity or the
  time-slice property.  A state \(\omega\) on \(\Obs_{\rm qloc}\) gives a
  Hilbert-space representation by the GNS construction.
\end{definition}
...
\begin{definition}[Euclidean correlation presentation]
  ...
  \]
  with Euclidean covariance, permutation symmetry, regularity and growth
  assumptions, and reflection positivity with respect to a chosen Euclidean
  time.  Under the Osterwalder--Schrader hypotheses these data reconstruct a
  Lorentzian Hilbert-space theory.
\end{definition}
...
This organization keeps the foundational objects explicit.  A later
calculation may use fields, path integrals, local algebras, or Euclidean
correlators, but the framework in which the calculation lives and the
comparison maps needed to interpret it must be stated.
...
\begin{definition}[Regulated theory]
  ...
  with a specified state space, algebra of variables, covariance or symmetry
  action, and expectation functional.  Examples include a finite lattice,
  a finite spatial volume with momentum cutoff, or a finite-dimensional
  time-sliced integral.  All integrals and operator products in a regulated
  theory are ordinary objects in the regulator-dependent category.
\end{definition}
...
object are distinct assertions.  The physics use of perturbation theory is
precise when the observable being expanded, the expansion parameter, the
renormalization prescription, and the error class or asymptotic meaning have
been stated.

This stratification lets the text use strong mathematical tools without
discarding nontrivial physics.  Spectral measures, scattering states,
renormalization-group flows, anomalies, conformal data, and gauge-invariant
observables are different constructions related by theorems, approximations,
