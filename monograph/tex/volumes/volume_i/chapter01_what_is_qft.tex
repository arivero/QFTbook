...
\begin{enumerate}
  ...
  \]
  \item a nonzero Poincare-invariant unit vector \(\vac\in\Hilb\), satisfying
  \(U(a,\Lambda)\vac=\vac\), with vacuum uniqueness in the chosen sector
  expressed by
  \[
    \{\psi\in\Hilb:U(a,\Lambda)\psi=\psi
  ...
  \[
    U(a,\Lambda)^{-1}\Obs(\mathcal O)U(a,\Lambda)
    =
...
The direction of conjugation is part of the convention.  With
...
an observable by \(U(a,\Lambda)^{-1}\) sends its localization region to
\(\Lambda\mathcal O+a\).
...
\begin{definition}[Wightman field presentation]
  ...
  operator-valued distribution on a declared test-function space, such as
  \(\mathcal S(\mathbb M^D)\) or \(C_c^\infty(\mathbb M^D)\), with common
  domain \(\mathcal D\).  Thus \(\Phi_a(f)\mathcal D\subset\mathcal D\), finite
  products of smeared fields are defined on \(\mathcal D\), and the vectors
  obtained by applying such products to \(\vac\) lie in \(\Hilb\).

  Let \(g=(a,\Lambda)\), \(gx=\Lambda x+a\), and
  ...
  Definition~\ref{def:vacuum-minkowski-local-qft}.
  ...
  of compactly supported test tensors
  \[
  ...
  evaluated by the distributions \(W_n\).  Vacuum cyclicity means that the
  linear span of all such vectors \(\Psi_F\) is dense in \(\Hilb\).
\end{definition}
