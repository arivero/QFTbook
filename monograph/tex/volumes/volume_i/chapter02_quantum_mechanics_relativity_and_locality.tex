...
\[
  ...
  \frac{\dd^{D-1}\vec p}{(2\pi)^{D-1}2E_{\vec p}} .
\]
For a one-particle wave packet
\(\psi\in C_c^\infty(\Sigma_m^+)\subset\Hilb_1\), the smeared creation and
annihilation operators on the finite-particle domain are
\[
  a^\dagger(\psi)
  =
...
\]
They extend to all \(\psi\in\Hilb_1\) on that domain, and satisfy
\[
  [a(\psi),a^\dagger(\chi)]
  =
