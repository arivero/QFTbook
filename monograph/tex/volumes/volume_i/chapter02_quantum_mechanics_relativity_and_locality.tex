...
\begin{proposition}[Schwartz triple and weak delta kernels]
\label{prop:schwartz-triple-weak-delta-kernels}
The basic model is
\[
...
above.  Its coordinate multiplication operators are the Stone generators
\(P^\mu\), so their spectral projections commute.  The spectrum condition is
exactly the assertion that this joint spectral measure is supported in
\(\overline V_+\).
\end{proof}
...
\begin{proposition}[Isolated shell as a one-particle subrepresentation]
\label{prop:isolated-shell-one-particle-subrepresentation}
...
\begin{proposition}[Bosonic Fock inner product]
\label{prop:bosonic-fock-inner-product}
...
\begin{proof}
Expand both \(\Pi_{s,n}\) in
Definition~\ref{def:bosonic-creation-operator-tensor-normalization}.  The
factor \(n!\) from the two \(\sqrt{n!}\) normalizations cancels one of the two
averages over \(S_n\), leaving exactly one permutation sum. Orthogonality of
different tensor powers gives the factor \(\delta_{mn}\).
\end{proof}
...
\begin{proposition}[Covariance of the free scalar field]
\label{prop:free-scalar-field-covariance-chapter-two}
...
\begin{proof}
The one-particle scalar representation preserves the mass-shell measure and
maps the dense test space \(\mathcal S_m\) to itself.  Second quantization
therefore maps \(a^\dagger(\psi)\) to \(a^\dagger(U_1(g)\psi)\) on
\(\mathcal D_0\).  Applying this identity to the distributional creation and
annihilation kernels in
Definition~\ref{def:free-scalar-field-mass-shell-kernels} gives the displayed
covariance law after the invariant change of variables on \(\Sigma_m^+\).
\end{proof}
...
Pauli--Jordan commutator on the common domain \(\mathcal D_0\).  Lorentz
invariance of the mass-shell measure makes \(\Delta\) Lorentz invariant.  A
spacelike displacement can therefore be transformed to \((0,\vec r)\); in that
frame the two exponentials in \(\Delta\) are exchanged by
\(\vec p\mapsto-\vec p\), and the integral vanishes.  Smearing with
compactly-supported test functions whose supports are spacelike separated
keeps all differences \(x-y\) spacelike, so the smeared commutator vanishes as
a distribution.
\end{proof}
