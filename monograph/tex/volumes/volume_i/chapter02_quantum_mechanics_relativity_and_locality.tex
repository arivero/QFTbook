...
\]
The label \(\lambda\) indexes fibers over the spectrum. A formal vector
\(|\lambda,a\rangle\), where \(a\) labels a basis in the fiber
\(\Hilb_\lambda\), is a distributional coordinate functional on this direct
integral. Delta-normalization records the kernel of the identity relative to
the measure \(\dd\nu\). Changing \(\dd\nu\) rescales the generalized kets and
therefore changes the displayed delta factors; the Hilbert-space vector
obtained after smearing is unchanged.
...
symbol \(|p,\sigma\rangle\) is an element of \(\Phi_1^\times\), and
\[
...
\[
  ...
  [\mathfrak a^\dagger,\mathfrak a^\dagger]=0.
\]
...
Starting from the free Fock representation, one may write a formal interacting
Hamiltonian
\[
  H=H_0+H_{\mathrm{int}},
\]
...
notation.  The relativistic local-theory datum extends such a formal
Hamiltonian by translation generators \(P_\mu\), Lorentz generators
\(J^{\mu\nu}\), a Poincare-invariant vacuum, and local observables or fields
satisfying covariance and microcausality.  In an interacting theory the boost
generators, the Hamiltonian, and the local field operators are tied together
by the Poincare algebra and locality.
...
The free scalar field is covariant.  Let \(g=(b,\Lambda)\in G_D\), and let
...
\[
  ...
  a^\dagger(U_1(g)\psi)
\]
for every one-particle wave packet \(\psi\).  Applying this identity to the
distributional creation and annihilation kernels, and using invariance of the
mass-shell measure, gives
\[
