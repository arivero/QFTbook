...
\([J^{\rho\sigma},P^\mu]
=\ii(\eta^{\mu\rho}P^\sigma-\eta^{\mu\sigma}P^\rho)\).
This is the Lorentzian Poincare subalgebra convention used again in the
conformal chapters.  There the Hermitian charge \(Q_X\) associated to a
conformal Killing field \(X\) obeys
\([Q_X,Q_Y]=-\ii Q_{[X,Y]}\); see
Chapter~\ref{ch:volume-iii-conformal-killing-vectors},
\eqref{eq:charge-vector-field-commutator}.  Taking
\(X=M^{\rho\sigma}\) and \(Y=P^\mu\), then raising indices with
\(\eta\), gives the displayed \([J^{\rho\sigma},P^\mu]\) bracket.  The
radial-quantization real form is obtained later by the explicit generator map
\eqref{eq:lorentzian-to-radial-generator-map}.
...
domain hypotheses, is Theorem~\ref{thm:wightman-spin-statistics}.
\end{remark}
