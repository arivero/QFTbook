...
\]
in the generalized eigenvector sense. With a spatial regulator understood,
or distributionally after removing the regulator, a state vector
\(\ket{\Psi}\) is represented on this basis by the wave functional
\[
  \Psi_t[\varphi]=\langle\varphi;t|\Psi\rangle.
\]
The generalized vector \(\ket{\varphi_i;t_i}\) is represented in this basis by
a functional delta distribution:
\[
  \langle\varphi;t_i|\varphi_i;t_i\rangle
  =
  \delta[\varphi-\varphi_i],
\]
with the right hand side understood after imposing a finite spatial regulator.
Equivalently, the operator \(\widehat\phi(t,\vec x)\) acts as multiplication by
\(\varphi(\vec x)\) on the regulated wave functional.
...
\[
  ...
  \widehat\phi(x^0=-\ii\tau,\vec x).
\]
For Euclidean times ordered as
\[
