...
The scale dependence in \eqref{eq:zeta-determinant-definition} is local in
the following sense.  The value \(\zeta_A(0)\) is determined by the coefficient
of \(t^0\) in the heat-trace expansion, up to the explicit removal of zero
modes.  Therefore changing \(\mu\) changes a one-loop effective action by the
same local heat-kernel coefficient that is later represented by local
renormalized coordinates.  This is the determinant counterpart of the
regulator/subtraction distinction recorded in
Table~\ref{tab:regulator-integration-status-catalog}.

\begin{proposition}[One-loop determinant from a quadratic fluctuation]
\label{prop:zeta-one-loop-quadratic-fluctuation}
