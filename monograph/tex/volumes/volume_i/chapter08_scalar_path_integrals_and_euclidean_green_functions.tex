...
The following proposition records the precise sense in which the
...
\begin{proposition}[Regulated meaning of the configuration path integral]
\label{prop:regulated-meaning-configuration-path-integral}
...
\begin{proof}
At fixed finite-dimensional \(\Lambda\), the hypotheses of
Proposition~\ref{prop:scalar-regulator-trotter-status} identify the
...
\begin{proposition}[Euclidean scalar action]
\label{prop:euclidean-scalar-action}
...
\begin{proposition}[Euclidean boundary value gives the time-ordered function]
\label{prop:euclidean-boundary-value-time-ordered}
...
\begin{proposition}[Finite-dimensional Gaussian source functional]
\label{prop:finite-dimensional-gaussian-source-functional}
...
\begin{proof}
The completion-of-the-square identity displayed above follows from
...
\begin{proposition}[Scale dependence of a zeta determinant]
\label{prop:zeta-determinant-scale-dependence}
...
\(\zeta_A(0)=a_d(A)\).  The scale-dependence formula follows immediately from
\[
  \log\det\nolimits_{\zeta}\left(\frac{A}{\mu^r}\right)
  =
  -\zeta_A'(0)-\zeta_A(0)\log\mu^r .
...
\begin{proposition}[Feynman boundary value]
\label{prop:feynman-propagator-wick-rotation-boundary-value}
