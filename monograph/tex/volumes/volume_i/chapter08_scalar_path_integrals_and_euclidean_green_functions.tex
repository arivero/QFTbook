...
state.  The Euclidean equation is
\[
...
to the free operator \(M_F\), the exact Pöschl--Teller calculation gives
\begin{equation}
...
mode is still omitted, the corresponding ratio is \(1/12\).  The convention in
...
\end{equation}
The passage from the Euclidean branch to the real-time width therefore uses
the metastable state of
Definition~\ref{def:finite-volume-false-vacuum-datum}, the negative-mode
cycle \eqref{eq:false-vacuum-negative-mode-half-gaussian}, and the
dilute-gas residual in
\eqref{eq:false-vacuum-dilute-bounce-exponentiation}.
...
\]
For an \(O(D)\)-symmetric bounce \(G_{\mu\nu}=(B/D)\delta_{\mu\nu}\) when
the action is \(B\).  Internal symmetry or gauge zero modes require their own
collective coordinates, stabilizer volumes, gauge fixing, ghost determinants,
and orbit metrics.  A leading field-theory rate formula written as
\[
  \frac{\Gamma}{V_D}
  =
  K_{\rm FT}\ee^{-B}
\]
is conditional on the following data: existence of the bounce in the
specified regulator, exactly one negative mode in the physical scalar sector,
separation of all zero modes, a regulated nonzero-mode determinant ratio,
renormalization of local counterterms, a dilute multi-bounce bound, and a
real-time metastable state or resonance interpretation.  Finite-temperature
activation and gravitational Coleman--De Luccia problems
