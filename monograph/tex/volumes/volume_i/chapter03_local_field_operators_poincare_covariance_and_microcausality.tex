...
\label{prop:distributional-matrix-elements}
...
\(C_c^\infty(\mathbb M^D)^{\widehat\otimes n}\) is canonically
\(C_c^\infty((\mathbb M^D)^n)\).  By hypothesis the displayed matrix element
is a continuous \(n\)-linear functional of the test functions, hence it
extends uniquely to a continuous linear functional on
\(C_c^\infty((\mathbb M^D)^n)\).  This is exactly a distribution on
\((\mathbb M^D)^n\).  The formula on decomposable test functions records the
extension on a dense generating set.
\end{proof}

\begin{remark}[Direction of point-field notation]
\label{rem:point-field-notation-direction}
The preceding proposition fixes the direction of logic: point-dependent
...
\begin{corollary}[Covariance of localization regions]
\label{cor:covariance-localization-regions}
...
\begin{proposition}[Covariance of Wightman distributions]
\label{prop:wightman-distribution-covariance-from-fields}
...
\]
with the same distributional meaning as the smeared formula.
\end{proposition}
...
\begin{proposition}[Poincare invariance of spacelike support separation]
\label{prop:poincare-invariance-spacelike-support-separation}
Let \(g=(a,\Lambda)\in\Poinc\). If \(K_1,K_2\subset\mathbb M^D\) are
spacelike separated, then \(gK_1\) and \(gK_2\) are spacelike separated. If
test functions \(f_1,f_2\) have spacelike separated supports, then the
supports of \((f_1)_g\) and \((f_2)_g\) are also spacelike separated.
\end{proposition}
...
Minkowski quadratic form. Hence \(gK_1\) and \(gK_2\) are spacelike separated.
For test functions, \(\operatorname{supp}(f_g)=g\,\operatorname{supp}f\):
...
taking closures. Applying the first part to the two supports proves the
claim.
\end{proof}
...
\begin{proposition}[Adjacent spacelike exchange in Wightman functions]
\label{prop:wightman-adjacent-spacelike-exchange}
...
\begin{proposition}[Isotony and graded locality for polynomial field algebras]
\label{prop:polynomial-field-algebra-isotony-graded-locality}
...
\begin{proposition}[Field axioms inherited by Wightman distributions]
\label{prop:wightman-inherits-covariance-and-locality}
Assume the common-domain and continuity hypotheses of
Proposition~\ref{prop:distributional-matrix-elements}. If the vacuum is
Poincare invariant and the field multiplet is covariant, the Wightman
distributions obey the covariance law of
Proposition~\ref{prop:wightman-distribution-covariance-from-fields}. If the
field multiplet satisfies microcausality, adjacent spacelike separated
insertions obey the graded exchange relation of
Proposition~\ref{prop:wightman-adjacent-spacelike-exchange}.
\end{proposition}

\begin{proof}
Both statements are obtained by applying the corresponding operator identities
to smeared products on the common domain and then using the defining
continuity of the Wightman distributions.  Covariance follows by inserting
\(U(g)U(g)^{-1}\) between neighboring factors and using vacuum invariance.
Locality follows by applying the vanishing graded commutator to the two
adjacent factors with spacelike separated supports.  Since the distributions
are determined by their values on tensor-product test functions, these
smeared identities are exactly the distributional covariance and exchange
relations.
\end{proof}
