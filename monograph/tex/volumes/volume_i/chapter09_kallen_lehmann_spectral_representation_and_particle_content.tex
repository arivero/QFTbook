...
\[
  \rho(B)=\nu_\phi(\pi^{-1}(B)).
\]
This is the pushforward of \(\nu_\phi\) by the invariant-mass map.
...
\begin{proof}
Let \(\pi(p)=-p^2\).  The map \(\pi:\overline V_+\to[0,\infty)\) is Borel and
Lorentz invariant, hence the pushforward \(\pi_*\nu\) is a positive Borel
measure.  Disintegration of locally finite positive measures gives conditional
...
\(\nu_s=c(s)\sigma_s\) for a nonnegative measurable function \(c\).  Absorbing
\(c(s)\) into \(\dd(\pi_*\nu)(s)\) defines the measure \(\dd\rho(s)\), which
