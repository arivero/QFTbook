...
\]
The explicit factor of \(\ii\) is fixed by the adjoint convention:
\[
  (\beta\gamma^\mu)^\dagger
  =
  (\gamma^\mu)^\dagger\beta
  =
  -\beta\gamma^\mu .
\]
Thus, for an ordinary commuting spinor vector \(s\in S\), the finite
matrix test \(s^\dagger\beta\gamma^\mu s\) is anti-Hermitian, while
\(\ii s^\dagger\beta\gamma^\mu s\) is Hermitian.  For Grassmann-odd
classical spinor components we use the unsigned order-reversing
anti-involution
\[
  (uv)^\dagger=v^\dagger u^\dagger
\]
for homogeneous Grassmann variables \(u,v\), with no additional
\((-1)^{|u||v|}\) factor.  Hence, componentwise for
\(A=\beta\gamma^\mu\),
\[
  (\psi^\dagger A\psi)^\dagger
  =
  \sum_{i,j}\psi_j^\dagger\,\overline{A_{ij}}\,\psi_i
  =
  \psi^\dagger A^\dagger\psi
  =
  -\psi^\dagger A\psi .
\]
Thus \((\bar\psi\gamma^\mu\psi)^\dagger=-\bar\psi\gamma^\mu\psi\).  In a
quantum field theory, the local current is a renormalized composite
operator-valued distribution; a normal-ordered free current is one
representation-dependent model of that composite field, not a general
definition of the coincident product.
\end{definition}

\paragraph{Basic Clifford, adjoint, and current identities.}
...
\(\ii\bar\psi\gamma^\mu\psi\) is Hermitian both for the commuting-vector
matrix test and for Grassmann-odd classical spinor fields with the unsigned
order-reversing anti-involution stated above.
...
\(\bar\psi\gamma^\mu\psi\) is anti-Hermitian for an ordinary commuting
spinor vector, and the same conclusion holds for Grassmann-odd classical
spinor fields with the unsigned order-reversing anti-involution.  Multiplication
by \(\ii\) gives the Hermitian current normalization.  The quantum statement
concerns the renormalized composite current as an operator-valued distribution;
a normal-ordered free current is one representation-dependent model of that
composite field.
...
\paragraph{Spin covariance and chirality trace.}
...
the adjoint identities above; the value at the identity is \(\beta\).
...
The two-component sign relation is a convention check rather than an
independent structural theorem.  It is nevertheless worth recording explicitly
because it fixes the minus sign that often changes when formulae are copied
between spinor conventions.  Let
\[
  \epsilon=
...
\paragraph{Lorentz covariance of Majorana conjugation.}
...
\(\gamma_5=-\ii\gamma^0\gamma^1\gamma^2\gamma^3\) is unchanged by the
similarity transformation \eqref{eq:appendix-wb-phase-change}.  The Dirac
adjoint is also the same abstract Hermitian pairing after the basis change:
\[
  \bar s=s^\dagger(\ii\gamma^0)
  =
  s_{\rm WB}^\dagger
...
for the same invariant pairing.  The basis relation in
\eqref{eq:appendix-wb-phase-change} is not just a naming convention: since
\(U_{\rm ch}^{-1}=\operatorname{diag}(I_2,-\ii I_2)\), direct block
...
matrix phase alone is insufficient for translating two-component formulae.
...
\begin{remark}[What the finite chiral trace fixes]
\label{rem:finite-chiral-trace-scope}
Equation~\eqref{eq:appendix-gamma-five-trace} fixes the spinor trace and
orientation slot of later anomaly formulae.  A physical anomaly calculation
also requires the regulator split, preserved Ward identities, local
counterterm representative, gauge-trace normalization, and loop or
heat-kernel normalization supplied in the anomaly chapter.
The two-dimensional axial anomaly also uses its own two-component Clifford
trace with \(\epsilon^{01}=+1\).  Restricting the four-dimensional block to a
\((0,1)\)-plane gives a different finite trace normalization and would
describe a different fermion content.
\end{remark}
...
\begin{remark}[Three-dimensional Clifford and symmetric block identities]
\label{rem:three-dimensional-clifford-symmetric-block-identities}
The matrices in
...
\(a,b\).  This is a finite convention check:
...
\begin{remark}[Two-dimensional chirality trace]
\label{rem:two-dimensional-chirality-trace}
The convention in the preceding definition is realized by
\[
...
\begin{remark}[Compatibility of the two-dimensional spinor bases]
\label{prop:two-dimensional-chiral-component-dirac-compatibility}
The Dirac-anomaly basis of
Remark~\ref{rem:two-dimensional-chirality-trace} and the
Majorana basis of
...
For Euclidean continuum Dirac operators with these Hermitian gamma matrices,
one must still include the factor of \(\ii\) in the operator.  If
\(\nabla_i\) is the metric spin connection tensored with a unitary internal
connection on a closed Riemannian manifold, then \(\nabla_i\) is formally
anti-self-adjoint and the raw operator \(\Gamma^i\nabla_i\) is formally
anti-self-adjoint.  The self-adjoint operator used for heat kernels and
spectral cutoffs is
\[
  D_E=\ii\Gamma^i\nabla_i .
\]
Its closure has domain \(H^1\) on compact manifolds without boundary.  On a
flat torus, \(D_E\) acts on a Fourier mode \(e^{\ii p\cdot x}\) as
\(-\Gamma^i p_i\), so its eigenvalues are real and \(D_E^2=|p|^2\) on that
mode.  This is the convention used by the background-index chapter and by the
Euclidean Fujikawa heat regulator.
