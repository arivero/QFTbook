...
\]
on a suitable dense domain, and satisfy
\[
...
\(\ket{q}\) and \(\ket{p}\). They are distributions, normalized by
\[
...
\[
  ...
  \bra{q_f}U(T)\ket{q_i}.
\]
In Lagrangian notation one often writes this kernel formally as
\[
  \bra{q_f}U(T)\ket{q_i}
  =
...
\]
The rest of this chapter gives this formula its regulated meaning.  The
symbol \([Dq]\) obtains its meaning from the regulator, limiting procedure,
operator symbol, and boundary conditions.
...
\end{aligned}
\]
For \(G_{ab}=m\delta_{ab}\), this reduces to the usual expression with
\[
