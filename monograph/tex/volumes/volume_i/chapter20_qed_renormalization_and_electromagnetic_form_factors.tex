...
\begin{proposition}[Charge normalization of the electron current]
\label{prop:qed-electron-charge-normalization}
With the plane-wave normalization and current normalization used in
Definition~\ref{def:qed-electron-electromagnetic-form-factors}, the charge
operator acts by
\(\widehat Q|\vec p,\sigma\rangle=q|\vec p,\sigma\rangle\) if and only if
\[
  F(0)+G(0)=1 .
\]
\end{proposition}
