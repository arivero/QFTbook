...
\section{Regulated Variables and Ward Organization}

\begin{definition}[Regulated explicit-coupling QED coordinates]
\label{def:qed-regulated-explicit-coupling-coordinates}
Use the electron convention of Chapter~\ref{ch:qed-external-states}.  At a
chosen perturbative regulator, use regulator-dependent representative fields
and parameters
\[
  A_\mu^B,\qquad \psi^B,\qquad \bar\psi^B,\qquad m^B,\qquad e^B,
\]
where the superscript \(B\) labels this cutoff-dependent coordinate system.
It does not denote a second Lagrangian separate from the regulated path
integral.  The regulated QED Lagrangian in these coordinates is
\[
  \mathcal L_B
  =
  -\frac14F^B_{\mu\nu}F_B^{\mu\nu}
...
\[
  ...
  \right]\psi .
\]
Equivalently, write the same regulated Lagrangian in the finite field and
charge coordinates as a finite reference part plus regulator-dependent local
coordinates:
\[
\begin{aligned}
  \mathcal L_B
...
because \(m\bar\psi\psi\) is gauge-invariant.  These counterterm vertices are
not added to a separate bare theory; they are the regulator-dependent part of
the single local Lagrangian \(\mathcal L_B\) after it has been expressed in
the renormalized coordinates \(A_\mu,\psi,\bar\psi,m,e\).
\end{definition}

\begin{proposition}[Ward organization of the renormalized current]
\label{prop:qed-ward-organization-current}
In the coordinate system of
Definition~\ref{def:qed-regulated-explicit-coupling-coordinates}, the
coefficient \(e\) in the renormalized gauge transformation is the same
coefficient fixed by the zero-momentum electromagnetic current matrix element.
If a vertex constant \(Z_1\) is introduced by
\(e^BZ_A^{1/2}Z_\psi=eZ_1\), Abelian gauge covariance gives
\(Z_1=Z_\psi\), and the current normalized by the renormalized field strength is
\[
  \partial_\nu F^{\mu\nu}
  =
...
\begin{proof}
The equality \(e=e^BZ_A^{1/2}\) is the condition that the covariant derivative
written in regulator fields,
\(\partial_\mu+\ii e^BA_\mu^B\), becomes
\(\partial_\mu+\ii eA_\mu\) after the field-residue change
\(A_\mu^B=Z_A^{1/2}A_\mu\).  Therefore the renormalized representative
transformation
\(A_\mu\mapsto A_\mu+\partial_\mu\xi\),
\(\psi\mapsto \exp(-\ii e\xi)\psi\) fixes \(e\) before the spinor residue
normalization is chosen.  Substituting \(e=e^BZ_A^{1/2}\) into
\(e^BZ_A^{1/2}Z_\psi=eZ_1\) gives \(Z_1=Z_\psi\).  Finally vary
\(\mathcal L_B\), expressed in the finite fields of
Definition~\ref{def:qed-regulated-explicit-coupling-coordinates}, with respect
to \(A_\mu\).  The Maxwell term contributes \(Z_A\partial_\nu F^{\mu\nu}\) and
the interaction term contributes \(\ii eZ_\psi\bar\psi\gamma^\mu\psi\), which
gives the displayed operator identity after division by \(Z_A\).
\end{proof}
...
\begin{tikzpicture}[baseline=(current bounding box.center),>=Stealth,scale=0.88]
  \tikzset{
    photon/.style={
      decorate,
      decoration={snake,amplitude=1.15pt,segment length=5pt},
      line width=0.55pt}
  }
...
irreducible photon two-point insertions.

\begin{proposition}[Transversality of the photon self-energy]
\label{prop:qed-photon-self-energy-transversality}
Modulo local contact terms in time-ordered products, Abelian current
conservation implies
\[
  k_\mu\Pi^{\mu\nu}(k)=0,\qquad k_\nu\Pi^{\mu\nu}(k)=0 .
\]
The statement is about the amputated nonlocal 1PI two-point kernel and is
independent of the longitudinal representative chosen for the photon
propagator.
\end{proposition}

\begin{proof}
The Maxwell equation with the renormalized current has the form
\[
  \partial_\nu \widehat F^{\mu\nu}=\widehat j^\mu,
  \qquad
...
\[
  k_\mu\Pi^{\mu\nu}(k)=0 .
\]
This is the two-photon consequence of a linear Ward--Takahashi identity.  If
\(\Gamma_{\rm inv}[A,\psi,\bar\psi]\) denotes the gauge-invariant part of the
QED 1PI functional, the infinitesimal transformation
\[
  \delta_\alpha A_\mu=\partial_\mu\alpha,\qquad
  \delta_\alpha\psi=-\ii e\alpha\psi,\qquad
  \delta_\alpha\bar\psi=\ii e\alpha\bar\psi
\]
gives
\[
  \mathcal W_\alpha\Gamma_{\rm inv}
  :=
  \int
  \left[
    \partial_\mu\alpha\,\frac{\delta\Gamma_{\rm inv}}{\delta A_\mu}
    -
    \ii e\alpha\psi\,\frac{\delta_L\Gamma_{\rm inv}}{\delta\psi}
    +
    \ii e\alpha\bar\psi\,
      \frac{\delta_R\Gamma_{\rm inv}}{\delta\bar\psi}
  \right]\dd^4x
  =
  0 .
\]
The operator \(\mathcal W_\alpha\) is fixed before \(\Gamma_{\rm inv}\) is
known; hence the identity is linear in \(\Gamma_{\rm inv}\).  Differentiating
twice with respect to \(A\) and then setting the fields to zero gives
transversality of the photon 1PI two-point kernel, up to the local contact
terms already separated above.  The nonabelian Slavnov--Taylor identity in
Chapter~\ref{ch:volume-ii-18} has a different mathematical
form: because the BRST variations of \(A_\mu\) and \(c\) are composite local
fields, the corresponding 1PI identity is quadratic in \(\Gamma\), as in
\eqref{eq:slavnov-taylor-identity}.
\end{proof}
...
\begin{proposition}[One-loop QED vacuum polarization in explicit-coupling coordinates]
\label{prop:qed-one-loop-vacuum-polarization}
In the regulator convention of
Definition~\ref{def:qed-regulated-explicit-coupling-coordinates}, dimensional
continuation to \(D=4-\epsilon\) and the pole normalization \(\pi(0)=0\) give
\[
  Z_A=1-\frac{e^2}{6\pi^2}\frac1\epsilon+\textup{finite},
\]
...
\begin{tikzpicture}[baseline=(current bounding box.center),>=Stealth,scale=0.82]
  \tikzset{
    photon/.style={
      decorate,
      decoration={snake,amplitude=1.1pt,segment length=5pt},
      line width=0.55pt}
  }
...
\begin{tikzpicture}[baseline=(current bounding box.center),>=Stealth,scale=0.9]
  \tikzset{
    ...
    fermion/.style={
      postaction={
        decorate,
        decoration={markings,mark=at position .58 with {\arrow{Stealth}}}},
      line width=0.55pt}
  }
...
\begin{definition}[Electron electromagnetic form factors]
\label{def:qed-electron-electromagnetic-form-factors}
...
\begin{equation}
  ...
  \left(\gamma^\mu+\frac{\ii}{2m}(p+p')^\mu\right)
  u(p).
  \label{eq:qed-pauli-gordon-current-convention}
\end{equation}
Thus \(F_2^{\rm DP}\) is the coefficient of the Pauli tensor
\(m^{-1}S^{\mu\nu}k_\nu\), not the coefficient of \((p+p')^\mu\) alone.
Its matrix element vanishes at \(p'=p\).  Comparing coefficients with the
displayed \((F,G)\) decomposition gives
\[
  F_1^{\rm DP}=F+G,
  \qquad
  F_2^{\rm DP}=-G,
...
With the plane-wave normalization and current normalization used in
Definition~\ref{def:qed-electron-electromagnetic-form-factors}, charge
normalization fixes one combination at zero momentum transfer.  The condition
\(\widehat Q|\vec p,\sigma\rangle=q|\vec p,\sigma\rangle\), where
\(\widehat Q=\int\dd^3\vec x\,\widehat j^0(x)\), is equivalent to
\begin{equation}
  F(0)+G(0)=1 .
  \label{eq:qed-electron-charge-normalization}
\end{equation}
Indeed, taking
\(p'\to p\), using
\[
...
\begin{proposition}[Magnetic factor in the \((F,G)\) basis]
\label{prop:qed-magnetic-factor-from-form-factor}
For an electron current matrix element written in the basis of
Definition~\ref{def:qed-electron-electromagnetic-form-factors}, the
spin-dependent coupling to a slowly varying external magnetic field is
\[
  ...
  \qquad
  g_{\mathrm{mag}}=2F(0)=2\bigl(1-G(0)\bigr).
\]
\end{proposition}
...
\begin{proposition}[Schwinger term in the electron magnetic factor]
\label{prop:qed-one-loop-vertex-schwinger}
In the same ultraviolet and infrared hard-kernel convention used for
Proposition~\ref{prop:qed-one-loop-vacuum-polarization}, the one-loop vertex
correction gives
\[
...
the loop integral dimensionally, and combine the three denominators
\[
  D_0=q^2,\qquad
  D_1=(p'-q)^2+m^2,\qquad
  D_2=(p-q)^2+m^2
...
\[
  z_0=1-x,\qquad z_1=y,\qquad z_2=x-y,
  \qquad 0\le y\le x\le1 .
\]
On the external mass shell \(p^2=p'^2=-m^2\), \(k=p-p'\), one has
\[
  z_0D_0+z_1D_1+z_2D_2
  =
  \ell^2+m^2x^2+k^2y(x-y),
  \qquad
  q=\ell+yp'+(x-y)p .
\]
Thus
\[
  \frac1{D_0D_1D_2}
  =
  2\int_0^1\dd x\int_0^x\dd y\,
  \frac1{\bigl(\ell^2+m^2x^2+k^2y(x-y)\bigr)^3}.
\]
The external Dirac equations and the Clifford algebra reduce every parity-even
vector bilinear between the on-shell spinors to the two-basis form
\[
  \bar u(p')\gamma^\mu u(p)\,A(k^2)
  +(p+p')^\mu\bar u(p')u(p)\,B(k^2).
\]
The scalar \(B(k^2)\) is the coefficient of the second basis vector in this
on-shell quotient; for \(k^2\neq0\) it is obtained by solving the resulting
two-by-two Gram system, and the \(k^2=0\) value used below is the regular
limit.  Terms odd in \(\ell\) vanish, and the \(\ell_\alpha\ell_\beta\) terms
are replaced by \(D^{-1}\eta_{\alpha\beta}\ell^2\) before the scalar
integrals are evaluated.  The Pauli-coordinate projection of the finite part
gives
\[
  B(k^2)
  =
  ...
  \frac{x(1-x)}
       {m^2x^2+k^2y(x-y)} .
\]
Since the form-factor definition is
\(B(k^2)=-\ii G(k^2)/(2m)\), this is equivalent to the formula below.
