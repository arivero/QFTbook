...
\(\langle\Omega|\operatorname T\widehat j^\mu(x)\widehat j^\nu(0)|\Omega\rangle\).
Then
\[
  k_\mu M^{\mu\nu}(k)=0,\qquad k_\nu M^{\mu\nu}(k)=0 .
\]
...
\[
  F(0)+G(0)=1.
\]
The equality is understood in the usual delta-normalized sense: after
smearing the external plane waves by a common wave packet and then removing
the smearing, the spatial integral supplies the same
