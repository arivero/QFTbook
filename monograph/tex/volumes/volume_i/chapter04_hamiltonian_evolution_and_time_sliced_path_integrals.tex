...
\paragraph{Mechanism of the product limit.}
The theorem is a statement in \(\Hilb_{\mathcal Q}\), not a pointwise
...
then gives strong semigroup convergence by the spectral theorem.  The
real-time unitary product formula is a separate Stone--Chernoff statement:
one verifies the infinitesimal generator on a core or invokes the corresponding
self-adjoint-sum theorem.  It should not be inferred by simply substituting
\(t\mapsto\ii t\) into an uncontrolled kernel expression.
...
\paragraph{Mechanism of the Faris--Lavine estimate.}
