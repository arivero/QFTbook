...
convention.  There are two different finite objects to keep separate.  The
first is the exact resolution-of-identity formula built from the exact
short-time kernel of \(\exp(-\ii\epsilon\widehat H/\hbar)\).  The second is a
symbol-parametrix oscillatory integral obtained only after replacing that
exact short-time kernel by a declared short-time symbol expansion.  The
Trotter theorem below applies to exact operator products, not automatically to
an arbitrary exponentiated symbol.  We record the exact construction first.
...
\end{aligned}
\]
This identity is exact at finite \(N\).  If one denotes the one-step mixed
kernel by
\[
  M_\epsilon(q_{n+1},p_n;q_n)
  =
  \bra{q_{n+1}}p_n\rangle
  \bra{p_n}\exp\left(-\frac{\ii}{\hbar}\epsilon\widehat H\right)\ket{q_n},
\]
then the exact finite product is the same integral with
\(\prod_n M_\epsilon(q_{n+1},p_n;q_n)\).  No symbol replacement has yet been
made.
...
\]
on the chosen domain.  A short-time symbol expansion may then give a
...
\[
  ...
  \right]
\]
for this convention, with the error understood in a specified regulated symbol
topology.  This display is not a convergence theorem.  In particular, an
\(O(\hbar\epsilon)\) term in the one-step exponent accumulates over
\(N=T/\epsilon\) slices to an \(O(\hbar T)\) contribution; it is an ordering or
measure counterterm unless the chosen construction proves that it cancels.
After a symbol class, quantization convention, endpoint convention, one-step
error estimate, and limiting topology have been supplied, the associated
finite-\(N\) phase-space approximant is
\[
  K_N^{\rm sym}(q_f,T;q_i,0)
  =
  \int
...
procedure.  In the present chapter, the finite-\(N\) exact product is defined
without approximation, while \(K_N^{\rm sym}\) is a symbol-parametrix
approximant whose convergence requires its own estimate.
...
\paragraph{Exact Trotter kernel for the standard Schr\"odinger split.}
For the standard Hamiltonian
\[
  \widehat H=\widehat T+V(\widehat q),
  \qquad
  \widehat T=\frac{\widehat p^2}{2m},
\]
the finite approximant justified by
Theorem~\ref{qthm:trotter-product-formula-qm} is not the arbitrary
symbol-exponentiated expression above.  It is the exact operator product
\[
  U_N^{TV}(T)
  =
  \left[
    \exp\left(-\frac{\ii\epsilon}{\hbar}\widehat T\right)
    \exp\left(-\frac{\ii\epsilon}{\hbar}V(\widehat q)\right)
  \right]^N,
  \qquad \epsilon=\frac{T}{N}.
\]
Its position kernel is obtained from the exact free-particle kernel and the
exact multiplication kernel of \(V(\widehat q)\):
\[
\begin{aligned}
  K_N^{TV}(q_f,T;q_i,0)
  &=
  \int\prod_{n=1}^{N-1}\dd^dq_n
  \prod_{n=0}^{N-1}
  \left(\frac{m}{2\pi\ii\hbar\epsilon}\right)^{d/2}
  \\
  &\quad\times
  \exp\left[
    \frac{\ii}{\hbar}
    \sum_{n=0}^{N-1}
    \left(
      \frac{m}{2\epsilon}|q_{n+1}-q_n|^2
      -\epsilon V(q_n)
    \right)
  \right].
\end{aligned}
\]
When the real-time hypotheses of the theorem hold, \(U_N^{TV}(T)\) converges
strongly to \(\exp(-\ii T\widehat H/\hbar)\).  This is an operator statement.
Pointwise kernel convergence requires additional hypotheses or smearing in a
declared test-function topology.

In Euclidean time the corresponding exact product is
\[
  \left[
    \exp\left(-\frac{\delta}{\hbar}\widehat T\right)
    \exp\left(-\frac{\delta}{\hbar}V(\widehat q)\right)
  \right]^N,
  \qquad \delta=\frac{\tau}{N},
\]
with heat-kernel factors
\[
  \left(\frac{m}{2\pi\hbar\delta}\right)^{d/2}
  \exp\left[-\frac{m|q_{n+1}-q_n|^2}{2\hbar\delta}
            -\frac{\delta}{\hbar}V(q_n)\right].
\]
Under the Kato hypotheses below this converges to the Feynman--Kac kernel.
For the harmonic oscillator \(V(q)=m\omega^2q^2/2\) in one dimension the
Gaussian product can be evaluated explicitly, and its limit is the Mehler
kernel
\[
  K_\tau^{\rm osc}(x,y)
  =
  ...
  \exp\left[
    -\frac{m\omega}{2\hbar\sinh\omega\tau}
    \bigl((x^2+y^2)\cosh\omega\tau-2xy\bigr)
  \right],
\]
the heat kernel of
\(-\hbar^2\partial_x^2/(2m)+m\omega^2x^2/2\).  This example is the model for
what has to be proved for a chosen approximant: exact finite products first,
then the topology of the limiting kernel.
...
above.  For example, on \(L^2(\mathbb R,\ee^{\sigma(q)}\dd q)\) the
Laplace--Beltrami kinetic operator for the metric
\(g(q)=\ee^{2\sigma(q)}\) is
\[
  -\frac{\hbar^2}{2}\Delta_g
  =
  -\frac{\hbar^2}{2}\ee^{-\sigma}\partial_q
  \bigl(\ee^{-\sigma}\partial_q\bigr).
\]
In a right-ordered symbol on this reference measure it is represented by
\[
  h_{\rm right}(q,p)
  =
  \frac12\ee^{-2\sigma(q)}p^2
  +
  \frac{\ii\hbar}{2}\ee^{-2\sigma(q)}\sigma'(q)p .
\]
With the flat reference measure and the symmetric kinetic operator
\(\frac12\,p\,a(q)\,p\), the Weyl symbol is instead
\[
  h_{\rm Weyl}(q,p)
  =
  \frac12a(q)p^2+\frac{\hbar^2}{8}a''(q).
\]
The linear momentum term and the scalar quantum potential are not optional
decorations: omitting them changes the limiting Hamiltonian selected by the
time-sliced construction.  Thus, for the declared quadratic symbol with no
linear momentum term and for the left-endpoint density convention displayed in
the Gaussian integral above, the induced finite-\(N\) configuration-space
approximant is
\[
\begin{aligned}
  K_N^{\rm met}(q_f,T;q_i,0)
  &=
  \int\prod_{n=1}^{N-1}\dd^dq_n
  \prod_{n=0}^{N-1}
...
\end{aligned}
\]
For a target Laplace--Beltrami or Weyl-ordered Hamiltonian, the preceding
reference-measure and ordering counterterms must first be built into the
one-step symbol and density; the bare determinant formula is not by itself a
coordinate-invariant definition of the quantum dynamics.
...
\(-\epsilon V(q_n)\) gives the displayed configuration-space approximant.
