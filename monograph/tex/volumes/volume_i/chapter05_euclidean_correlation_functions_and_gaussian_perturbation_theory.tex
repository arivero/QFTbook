...
\end{aligned}
\]
\begin{definition}[Spectral measure of a vacuum coordinate]
\label{def:vacuum-coordinate-spectral-measure}
Let \(P_{\widehat H}\) be the projection-valued spectral measure of
\(\widehat H\).  For a vector
\[
\eta_q:=\widehat q\ket0,
\]
assumed to lie in \(\Hilb\), define the positive finite Borel measure
\[
  \mu_q(B)=\langle \eta_q,P_{\widehat H}(B)\eta_q\rangle
\]
for Borel sets \(B\subset\mathbb R\).  Its shifted version is the
pushforward of \(\mu_q\) under \(E\mapsto E-E_0\).  This is the
one-dimensional prototype of the spectral measures that appear later in the
K\"all\'en--Lehmann representation.
\end{definition}

\begin{proposition}[Lower-half-plane continuation from the spectral theorem]
\label{prop:qm-spectral-euclidean-boundary-value}
With the hypotheses above, the ordered two-point function admits the
...
\]
The same formula defines a holomorphic function for \(\operatorname{Im}t<0\).
For \(\tau>0\), its Euclidean boundary value at \(t=-\ii\tau\) is the
Laplace transform
\[
  G_E(\tau)
  =
...
\]
If the spectral measure is pure point, the integral reduces to the displayed
sum over energy eigenvectors.
\end{proposition}

\begin{proof}
The spectral theorem gives
\[
  \ee^{-\frac{\ii}{\hbar}t(\widehat H-E_0)}
  =
  \int_{[E_0,\infty)}
  \ee^{-\frac{\ii}{\hbar}t(E-E_0)}\,\dd P_{\widehat H}(E)
\]
in the functional calculus.  Sandwiching this identity between
\(\eta_q=\widehat q\ket0\) and itself gives the stated formula for real
\(t\).  The support is contained in \([E_0,\infty)\) because \(E_0\) is the
bottom of the spectrum, and the total mass is
\(\|\widehat q\ket0\|^2\).

If \(t=x+\ii y\) with \(y<0\), then
\[
  \left|
  \ee^{-\frac{\ii}{\hbar}t(E-E_0)}
  \right|
  =
  \ee^{\frac{1}{\hbar}y(E-E_0)}
  \le 1 .
\]
The finite measure \(\mu_q\) therefore gives a locally dominated integral.
The integrand is entire as a function of \(t\), so differentiation under the
integral is justified on compact subsets of the lower half-plane by the same
estimate with one additional power of \(E-E_0\) after inserting a compact
vertical margin \(y\le -\epsilon<0\).  Hence the integral is holomorphic
there.  Setting \(t=-\ii\tau\) gives the Laplace transform.  In the
pure-point case \(\mu_q=\sum_n |\bra n\widehat q\ket0|^2\delta_{E_n}\), so
the integral is the sum already obtained by inserting the eigenbasis.
\end{proof}
For \(\tau>0\), define
\[
  G_E(\tau)
...
\begin{definition}[Euclidean ordered source functional in quantum mechanics]
\label{def:euclidean-ordered-source-functional-qm}
For a smooth compactly supported function \(J(\tau)\), define the
Euclidean ordered source functional
\[
  Z_E[J]
  =
  \bra 0
  ...
  \ket 0 .
\]
The exponential is understood by its power series in the source, with
Euclidean ordering applied term by term.  Its functional derivatives recover
the Euclidean ordered correlation functions:
\[
  G_E^{(r)}(\tau_1,\ldots,\tau_r)
  =
  \left.
...
\]
\begin{definition}[Euclidean Lagrangian associated with a mechanical action]
\label{def:euclidean-lagrangian-mechanics}
For a Lorentzian mechanical Lagrangian \(L(q,\dot q)\) whose expression is
analytic in \(\dot q\) near the real axis, the Euclidean Lagrangian in the
coordinate chart \(q^a\) is
\[
  L_E(q,q')
  =
  -L(q,\ii q'),
...
\paragraph{Sign of the Wick-rotated weight.}
Under \(t=-\ii\tau\), with the orientation of the integration contour rotated
accordingly, the Lorentzian phase transforms formally as
\[
  \exp\left[
  ...
  \right].
\]
Indeed, along the rotated contour \(\dd t=-\ii\,\dd\tau\) and
\(\dot q=\dd q/\dd t=\ii\,\dd q/\dd\tau=\ii q'\).  Therefore
\[
  \frac{\ii}{\hbar}L(q,\dot q)\,\dd t
  =
  \frac{\ii}{\hbar}L(q,\ii q')(-\ii\,\dd\tau)
  =
  \frac{1}{\hbar}L(q,\ii q')\,\dd\tau
  =
  -\frac{1}{\hbar}L_E(q,q')\,\dd\tau .
\]
Exponentiating the integrated identity gives the stated weight.  This is an
identity of the formal action density after choosing the contour; analytic
existence of the path-integral limit is a separate assertion supplied only in
the regulated cases below.

\begin{proposition}[Euclidean long-time projection onto the vacuum]
\label{prop:euclidean-long-time-vacuum-projection}
Let \(\ket\psi\in\Hilb\) satisfy \(\braket{0}{\psi}\ne0\), and assume that the
ground state is separated from the rest of the spectrum by a gap.  More
precisely, suppose that the spectrum of \(\widehat H-E_0\) on
\(\{\ket0\}^{\perp}\) lies in \([\Delta,\infty)\) with \(\Delta>0\).  Then
\[
  \frac{\exp[-T(\widehat H-E_0)/\hbar]\ket\psi}
       {\left\|\exp[-T(\widehat H-E_0)/\hbar]\ket\psi\right\|}
  \longrightarrow
  \frac{\braket{0}{\psi}}{|\braket{0}{\psi}|}\ket0
\]
in Hilbert norm as \(T\to\infty\).  Consequently Euclidean matrix elements
with endpoints \(\ket\psi\) and fixed insertions separated from the endpoints
converge to vacuum Euclidean ordered correlation functions, after division by
the corresponding endpoint partition function.
\end{proposition}
...
\[
  ...
  \frac{c_0}{|c_0|}\ket0
\]
as \(T\to\infty\).  The same estimate applies independently to the bra
endpoint.  The phase \(c_0/|c_0|\) cancels between bra and ket in normalized
matrix elements.  For \(0<\tau_1<\cdots<\tau_r<T\), inserting the semigroup
factors between consecutive operators writes the endpoint matrix element as
a bounded product of fixed-time operators and exterior factors
\(\exp[-T(\widehat H-E_0)/\hbar]\ket\psi\) and its adjoint.  The norm
convergence of the exterior factors gives the vacuum Euclidean ordered
correlator.  If the inserted operators are unbounded, this last sentence is
read on a common invariant domain or after smearing/truncating the insertions;
the regulated path-integral constructions below implement precisely that
domain control.
\end{proof}
...
\begin{convention}[Euclidean path-integral brackets in this chapter]
\label{conv:euclidean-path-integral-brackets-ch5}
After the Wick rotation of the contour to Euclidean time, the Lagrangian
path-integral notation
\[
...
\]
means the value produced by a specified regulator and limiting procedure, not
an independently given translation-invariant measure on an infinite-dimensional
space.  The Euclidean action is
\[
  S_E[q]=\int_{-T}^{T}L_E(q,q')\,\dd\tau.
\]
For a bosonic Schr\"odinger operator satisfying the hypotheses of
Theorem~\ref{thm:wiener-feynman-kac-qm}, this sentence can be read as a
Wiener-measure statement: finite-interval transition amplitudes are Brownian
bridge expectations with the potential weight, and vacuum correlators are
obtained by the spectral projection limit described above.  In the rest of
this chapter the same bracket notation is also used for finite-dimensional
Gaussian regulators and perturbative asymptotic expansions.  These are
different mathematical objects, and the text will specify which one is meant.
\end{convention}
...
\begin{definition}[Mode cutoff regulator]
\label{def:mode-cutoff-regulator-qm}
A mode cutoff regulator consists of a finite set of functions
\(f_1,\ldots,f_{N_{\rm max}}\), a finite-dimensional coordinate map
\[
  q(\tau)=\sum_{n=1}^{N_{\rm max}}q_nf_n(\tau),
\]
and a regulated measure defined by a finite product of Lebesgue measures in
the coefficients \(q_n\), together with the normalization and boundary
conditions dictated by the chosen time-sliced construction.  The limit
\(N_{\rm max}\to\infty\) is a further analytic operation and is not part of
the symbol \([Dq]\) until its existence has been established for the
observables under discussion.
\end{definition}

The harmonic oscillator provides the basic Gaussian example.

\begin{controlledapproximation}[Removing a mode cutoff]
\label{ctrl:remove-mode-cutoff-ch5}
In this chapter the phrase ``remove the mode cutoff'' has two distinct
meanings.  In the free Gaussian oscillator it denotes an actual limit of
finite-dimensional Gaussian moments; the covariance kernels below converge
after the finite mode projection is removed.  In the interacting examples it
denotes a coefficientwise operation on a perturbative expansion first defined
at finite cutoff.  No nonperturbative continuum measure, and no convergence
of the formal power series in the coupling, is inferred from the notation.
...
\begin{definition}[Dirichlet harmonic-oscillator Gaussian cutoff]
\label{def:dirichlet-harmonic-oscillator-cutoff}
For \(N\in\mathbb N\), let
\[
  q^{(N)}(\tau)=\sum_{n=1}^{N}q_nf_n(\tau),
  \qquad
  S_{E,T,N}(q_1,\ldots,q_N)
  =
  \frac12\sum_{n=1}^{N}
  \left[
    \left(\frac{n\pi}{2T}\right)^2+\omega^2
...
\[
  \langle F\rangle_{T,N}
  =
  \frac{
    \int_{\mathbb R^N}\prod_{n=1}^{N}\dd q_n\,
    \exp[-S_{E,T,N}/\hbar]\,F(q^{(N)})
  }{
    \int_{\mathbb R^N}\prod_{n=1}^{N}\dd q_n\,
    \exp[-S_{E,T,N}/\hbar]
  } .
...
\begin{proposition}[Harmonic-oscillator Euclidean covariance from the cutoff]
\label{prop:harmonic-oscillator-euclidean-covariance-cutoff}
For the cutoff of Definition~\ref{def:dirichlet-harmonic-oscillator-cutoff},
the finite-mode covariance converges at fixed \(T\).  For fixed insertion
times \(\tau_1,\tau_2\) and then \(T\to\infty\), the limiting Euclidean
two-point function on the line is
\[
...
\begin{proof}
This is an ordinary finite-dimensional Gaussian integral.  In this free
subsection, \(\langle F\rangle_T\) denotes the limit
\(\lim_{N\to\infty}\langle F\rangle_{T,N}\) only for those observables for
which that limit is being proved or has already been proved.  The independent
Gaussian integrals give, for \(1\le n,m\le N\),
\[
  \langle q_nq_m\rangle_{T,N}
  =
  \delta_{nm}\,
  \frac{\hbar}{
    \left(\frac{n\pi}{2T}\right)^2+\omega^2
...
\[
  \langle q^{(N)}(\tau_1)q^{(N)}(\tau_2)\rangle_{T,N}
  =
  \sum_{n=1}^{N}
  \frac{\hbar\,f_n(\tau_1)f_n(\tau_2)}
       {\left(\frac{n\pi}{2T}\right)^2+\omega^2}.
\]
Since \(\sum_{n\ge1}((n\pi/2T)^2+\omega^2)^{-1}<\infty\), this sequence has
a pointwise limit for every fixed \(\tau_1,\tau_2\in[-T,T]\), and the
finite-\(T\) Gaussian covariance is
\[
...
\begin{definition}[Finite-dimensional real Gaussian datum]
\label{def:finite-dimensional-real-gaussian-datum}
Let \(A\) be a real symmetric positive-definite \(N\times N\) matrix, let
\(J\in\mathbb R^N\), and write repeated finite indices as summed.  Define
\[
  Z_A[J]
  =
...
\begin{proposition}[Gaussian generating functional and Wick theorem]
\label{prop:finite-dimensional-gaussian-wick-theorem}
For the datum of Definition~\ref{def:finite-dimensional-real-gaussian-datum},
\[
  Z_A[J]
  =
  (2\pi)^{N/2}(\det A)^{-1/2}
...
\begin{proof}
Completing the square gives the displayed formula for \(Z_A[J]\).  Indeed
positive-definiteness of \(A\) permits the shift
\(q\mapsto q-A^{-1}J\), and the remaining Gaussian integral is
\((2\pi)^{N/2}(\det A)^{-1/2}\).  Differentiating with respect to \(J\)
gives
\[
  \langle F(q)\rangle_A
  =
...
only the degree \(r\) term in
\(\exp(\frac12J_aC_{ab}J_b)\) contributes after setting \(J=0\), and each
surviving derivative assignment partitions the \(2r\) labels into \(r\)
unordered pairs.  Thus the Gaussian expectation of \(2r\) variables is the
...
\[
  ...
  \prod_{(a,b)\in P}(A^{-1})_{i_ai_b}.
\]
For an odd number of derivatives no monomial of odd total degree appears in
the source exponential, so the moment vanishes.
\end{proof}

\section{Gaussian Functional Integrals}

\begin{definition}[Gaussian functional integral as a regulated limit]
\label{def:gaussian-functional-integral-regulated-limit}
...
\[
  ...
  \right].
\]
This notation denotes the finite-dimensional Gaussian construction followed
by a limit of covariance kernels in the specified topology; it does not
assert the existence of a translation-invariant Lebesgue measure on the space
of paths.
\end{definition}

\begin{proposition}[Gaussian functional source identity]
\label{prop:gaussian-functional-source-identity}
For the regulated Gaussian construction of
Definition~\ref{def:gaussian-functional-integral-regulated-limit},
the normalization-independent source ratio is
\[
  \frac{Z_0[J]}{Z_0[0]}
  =
  \exp\left[
...
\begin{proof}
At finite cutoff this is exactly
Proposition~\ref{prop:finite-dimensional-gaussian-wick-theorem} with the
matrix \(A_{ij}\) equal to the quadratic form of the cutoff action and with
the source vector \(J_i=(J,f_i)\).  The inverse matrix has kernel equal to
the cutoff Green function.  Passing to the limit gives the displayed kernel
identity whenever the cutoff Green functions converge against the chosen
test sources.
...
\begin{proposition}[Gaussian integration by parts]
\label{prop:gaussian-functional-integration-by-parts}
In the same regulated Gaussian construction,
\[
  \langle q(\tau_1)q(\tau_2)\rangle_0=G(\tau_1,\tau_2)
\]
also follows from finite-dimensional integration by parts.  Equivalently,
the formal identity
\[
...
\]
\begin{definition}[Cutoff perturbative expansion of the anharmonic oscillator]
\label{def:anharmonic-cutoff-perturbative-expansion}
...
the regulated Gaussian integral.  Removing the mode cutoff in this interacting
section means taking \(N_{\rm max}\to\infty\), or equivalently
\(\Lambda\to\infty\), separately for each coefficient after its regulator
dependence has been displayed.  If the coefficient has a finite limit, that
limit is part of the perturbative definition used here; if it does not, one
must specify the local counterterm or subtraction coordinate before a finite
...
At first order in \(g\),
\begin{equation}
  \label{eq:anharmonic-leading-correction}
  \begin{aligned}
  \left(\frac{Z_g}{Z_0}-1\right)_{O(g)}
  &=
  ...
  \end{aligned}
\end{equation}
The factor \(3\) in \(\langle q^4\rangle_0=3G_0(\tau,\tau)^2\) is the Wick
count \((4-1)!!=3\) for pairing four identical fields at the same Euclidean
time among themselves.  The \(4!\) assignment count appears only when four
labeled vertex half-edges are attached to four labeled insertions; that is a
different combinatorial problem from the coincident self-contraction here.
...
propagators.  Figure~\ref{fig:anharmonic-second-order-vacuum-topologies}
records this separation between disconnected vacuum products and the connected
two-vertex topologies that enter the linked-cluster proof.
...
\begin{tikzpicture}[scale=0.82]
  ...
  \draw[thick,blue!65!black]
    (5.05,0) .. controls (5.45,-0.2) and (5.95,-0.2) .. (6.35,0);
\end{tikzpicture}
\caption{Second-order Wick topologies in the anharmonic oscillator expansion.
The left pair is a product of disconnected one-vertex vacuum bubbles.  The two
right diagrams are connected two-vertex vacuum contractions: two inter-vertex
lines with one tadpole at each vertex, and four inter-vertex lines.}
\label{fig:anharmonic-second-order-vacuum-topologies}
\end{figure}

\begin{proposition}[Linked-cluster formula for the cutoff expansion]
\label{prop:linked-cluster-formula-cutoff-anharmonic}
At every fixed cutoff and at the level of formal power series in \(g\),
\(\log(Z_g/Z_0)\) is the sum of connected vacuum diagrams.
\end{proposition}
...
\]
\begin{proposition}[Ground-state energy from the Euclidean kernel]
\label{prop:ground-state-energy-from-euclidean-kernel}
Assume that the anharmonic oscillator Hamiltonian has discrete spectrum
\(E_0(g)<E_1(g)\le\cdots\), with nondegenerate ground state \(\psi_0\).  If
the endpoint data used in the transition kernel have nonzero overlap with
\(\psi_0\), then
\[
  E_0(g)
  =
...
\[
  ...
  \frac{1}{T_{\mathrm{tot}}}\log Z_g.
\]
The endpoint wavefunction factors contribute only \(O(1)\) to
\(\log Z_g\), so they disappear after multiplication by
\(T_{\mathrm{tot}}^{-1}\).  Taking the difference between the interacting and
free kernels gives
\[
  E_0(g)-E_0(0)
  =
  ...
  \log\frac{Z_g}{Z_0}.
\]
The first-order term in \eqref{eq:anharmonic-leading-correction} is
connected, hence it is also the first-order term in
\(\log(Z_g/Z_0)\).  On an interval of length \(T_{\mathrm{tot}}\),
\[
  \frac{1}{T_{\mathrm{tot}}}\log\frac{Z_g}{Z_0}
  =
  -\frac{g}{8}G_0(0)^2+O(g^2).
\]
The sign of the energy shift therefore comes from the minus sign in
\(E_0=-T_{\mathrm{tot}}^{-1}\log Z\) together with the per-vertex factor
\(-g/4!\) in \eqref{eq:anharmonic-leading-correction}. Thus
\[
  \Delta E_0
  =
  \frac{g}{8}G_0(0)^2+O(g^2)
...
\[
  ...
  \right\rangle_0 .
\]
\begin{proposition}[Vacuum components cancel in normalized two-point functions]
\label{prop:vacuum-components-cancel-normalized-two-point}
At fixed cutoff and as a formal power series in \(g\), the normalized
two-point function is the sum of Wick diagrams whose connected component
contains both external insertions.
\end{proposition}

\begin{proof}
At fixed cutoff every coefficient of the formal series is a finite Gaussian
moment, hence Wick's theorem expresses it as a finite sum over pairings.
View a pairing as a graph whose vertices are the two external insertions and
the interaction insertions, with an edge for each Wick contraction.  Every
such graph decomposes uniquely into connected components.  The component
containing the two external vertices carries all dependence on the external
points; the remaining components contain interaction vertices only and are
vacuum graphs.  Conversely, a connected two-point graph together with an
unordered finite collection of vacuum components determines a unique
disconnected graph of the normalized numerator, with the exponential
generating-function factors.  Summing over all vacuum components therefore
produces the same formal power series \(Z_g\) that appears in the denominator.
Division by \(Z_g\) cancels this vacuum factor coefficientwise, leaving exactly
the Wick graphs whose connected component contains both external insertions.
\end{proof}
...
Figure~\ref{fig:anharmonic-normalized-two-point-connected-diagrams} displays
precisely the connected topologies that survive the division by \(Z[0]\), so
the vacuum factors drawn in the preceding bookkeeping figure are no longer
present:
\begin{figure}[!ht]
\centering
...
\end{tikzpicture}
\caption{First connected topologies contributing to the normalized two-point
function.  Red endpoints mark the fixed insertions \(q(\tau)\) and \(q(0)\);
black vertices are interaction insertions; blue arcs are contracted oscillator
fields.  Vacuum components are absent because they cancel against \(Z_g\).}
\label{fig:anharmonic-normalized-two-point-connected-diagrams}
\end{figure}
...
\begin{definition}[Euclidean \(1\mathrm{PI}\) two-point insertion]
\label{def:euclidean-1pi-two-point-insertion-ch5}
...
\(1\mathrm{PI}\) two-point insertions in momentum space, with the external
free propagators removed.
\end{definition}

\begin{proposition}[Dyson resummation of exact two-point insertions]
\label{prop:dyson-resummation-exact-two-point-insertions-qm}
With the sign convention of
Definition~\ref{def:euclidean-1pi-two-point-insertion-ch5}, the full
...
\begin{proof}
Every connected two-point graph is obtained uniquely by arranging a finite
chain of amputated \(1\mathrm{PI}\) two-point insertions along the external
line and inserting one free propagator between consecutive amputated
insertions.  Cutting the propagators between adjacent insertions gives the
unique chain decomposition; conversely gluing such a chain gives a connected
two-point graph.  Translation invariance diagonalizes the convolution in
momentum space, so the chain with \(r\) insertions contributes
\(\widetilde G_0(k)(\widetilde G_0(k)\Sigma(k))^r\).  Summing the formal
geometric series gives the displayed denominator.
\end{proof}
...
\]
The momentum-independent term \(\frac{g^2}{32}\) is the
tadpole-bubble, equivalently the double-bubble insertion after external
amputation.  The term proportional to \((k^2+9)^{-1}\) is the sunset
insertion.  Its denominator is not an arbitrary algebraic remnant: the
three-fold convolution is the Fourier transform of
\(\frac18\ee^{-3|\tau|}\), so its nearest singularities are
\(k=\pm3\ii\), or \(k^2=-9=-(3\omega)^2\) with \(\omega=1\).  In the spectral
language of this oscillator computation, this is the intermediate
three-quantum energy carried by the sunset topology.
...
\]
\begin{proposition}[Pole extraction of the first energy gap]
\label{prop:anharmonic-first-gap-from-two-point-pole}
For the anharmonic oscillator in the convention above, the pole of
\(\widetilde G(k)\) near \(k=\ii\) gives
\[
  E_1-E_0
  =
...
It remains to solve the pole equation near \(k=\ii\).  Write
\(\omega=1+a g+b g^2+O(g^3)\) and set \(k=\ii\omega\).  The denominator of
\(\widetilde G(k)\) is
\[
  -\omega^2+1+\frac{g}{4}
  -\frac{g^2}{8}
  \left(
    \frac14+\frac{1}{9-\omega^2}
  \right)
  +O(g^3).
\]
Since \(9-\omega^2=8+O(g)\), the \(O(g^2)\) part of the last factor is
\(\frac14+\frac18=\frac38\).  Also
\(\omega^2=1+2ag+(a^2+2b)g^2+O(g^3)\).  Vanishing of the denominator gives
\[
  -2a+\frac14=0,
  \qquad
  -(a^2+2b)-\frac{3}{64}=0.
\]
Thus \(a=\frac18\) and \(b=-\frac1{32}\), proving the formula.
\end{proof}
...
\begin{remark}[Classical field redefinition check]
\label{prop:derivative-oscillator-classical-field-redefinition}
Define a new coordinate \(y\), to first order in \(g\), by
\[
  \frac{\dd y}{\dd q}
  =
...
order \(g\), to an ordinary anharmonic oscillator.  This is a classical
coordinate calculation:
The defining equation gives \(\dot y=(1-\frac{g}{4}q^2)^{1/2}\dot q\), hence
\[
  \frac12\dot q^2-\frac{g}{8}q^2\dot q^2
  =
  \frac12\left(1-\frac{g}{4}q^2\right)\dot q^2
  =
  \frac12\dot y^2+O(g^2).
\]
Inverting \(y=q-\frac{g}{24}q^3+O(g^2)\) gives
\(q=y+\frac{g}{24}y^3+O(g^2)\).  Therefore
\[
  -\frac12q^2
  =
  -\frac12y^2-\frac{g}{24}y^4+O(g^2),
\]
which proves the displayed Lagrangian.  The quantum computation below keeps
the \(q\)-coordinate and shows where the ordering or path-integral
prescription enters.
\end{remark}
...
Figure~\ref{fig:volume-i-derivative-interaction-contractions} separates the
derivative contractions according to their ultraviolet role: loop-derivative
contractions produce the local divergent term, and the parity-odd loop
...
\begin{definition}[Frequency cutoff for the derivative interaction]
\label{def:frequency-cutoff-derivative-oscillator}
...
the finite-dimensional real slice has been chosen, \([Dq]_\Lambda\) denotes
the corresponding finite product Lebesgue measure.  The interaction is
expanded before \(\Lambda\to\infty\).
\end{definition}

\begin{proposition}[Local counterterm required by the derivative interaction]
\label{prop:derivative-interaction-local-counterterm-gap}
After restoring \(\hbar\), the first-order self-energy has the form
\[
  \Sigma_\Lambda(k)
  =
  \frac{\hbar g}{4}
...
\begin{proof}
The contraction with two derivatives acting on the loop gives
\[
  \frac{g}{4}
  \int\frac{\dd k}{2\pi}
    \frac{\ee^{\ii k\tau}}{(k^2+1)^2}
...
\[
  \int_{-\Lambda}^{\Lambda}\frac{\dd k'}{2\pi}
  \frac{k'^2}{k'^2+1}
  =
  \frac{1}{\pi}\int_0^\Lambda
  \left(1-\frac{1}{k'^2+1}\right)\dd k'
  =
  \frac{\Lambda-\arctan\Lambda}{\pi}.
\]
The remaining first-order contractions are the finite even momentum term and
the odd momentum term displayed above.  Combining them and expanding
\(\arctan\Lambda=\pi/2+O(\Lambda^{-1})\) gives the stated self-energy.
The counterterm contributes a local two-point insertion \(-Cg\hbar\) in the
denominator convention used for \(\Sigma\).  Hence it shifts
\(\Sigma_\Lambda(k)\) by \(-Cg\hbar\).  Choosing
\(C=\Lambda/(4\pi)+C_{\rm fin}\) removes the linearly divergent local part.
The remaining finite family is the one displayed above.
\end{proof}
...
\]
\paragraph{Finite gap after choosing the finite local term.}
After the counterterm choice of
Proposition~\ref{prop:derivative-interaction-local-counterterm-gap}, the
first energy gap is
\[
  \omega_1
  =
  ...
  +O(g^2).
\]
Setting \(k=\ii\omega_1\) in the denominator of
\(\widetilde G_\Lambda(k)\) and expanding to first
order in \(g\) gives
\[
  \omega_1
  =
