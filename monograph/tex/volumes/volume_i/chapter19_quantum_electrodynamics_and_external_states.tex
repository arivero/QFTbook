...
\(\bar\psi=\psi^\dagger\beta\) is the Dirac adjoint defined in Chapter 16.
...
Charged insertions can be represented by adding dressing data. If \(C_x\) is
an oriented curve beginning at \(x\) and ending at spatial infinity, define
\[
  \Psi_{C_x}(x)
  =
  \exp\left(\ii g\int_{C_x} A_\mu(y)\,\dd y^\mu\right)\psi(x).
\]
For gauge functions that vanish at the endpoint at infinity, the phase
acquired by the line integral cancels the phase of \(\psi(x)\). The dressing
records the electromagnetic field attached to the charged insertion. Its
dependence on the curve is physical dressing data; the pointlike local
observables remain the neutral expressions above.
...
formal Lorentzian generating functional is
\[
\begin{aligned}
  Z[J,\bar\eta,\eta]
  &=
...
\end{aligned}
\]
It is understood perturbatively by expanding the interaction term
\[
  \ii gA_\mu\bar\psi\gamma^\mu\psi
\]
around the Gaussian Maxwell and Dirac functionals already constructed.
...
With the cross-section normalization of Chapter 14, the total cross section
