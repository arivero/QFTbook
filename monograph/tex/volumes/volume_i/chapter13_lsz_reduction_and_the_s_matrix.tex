...
\]
be positive-energy momenta on \(\Sigma_m^+\), and introduce all-incoming
Green-function momenta
\[
...
Under the assumptions above, the connected component of
\(\langle F_{\mathrm{out}},SF_{\mathrm{in}}\rangle_{\mathcal F_s(\Hilb_1)}\) is obtained
from the distributional boundary value
\[
\begin{aligned}
&\left[
  \prod_{a=1}^{m+n}
...
\end{aligned}
\]
by restricting every external \(q_i\) and \(p_j\) to \(\Sigma_m^+\) and
smearing with the wave packets \(f_i^*(\vec q_i)\) and \(g_j(\vec p_j)\)
against the mass-shell measures \(\dd\mu_m\).  The restriction is a
boundary-value operation in the sense of
Chapter~\ref{ch:lorentzian-green-functions}: the expression is first paired
with the wave packets, and the external one-particle residues are then taken.
\end{theorem}
...
The definition of \(Z\) above says that \(\widehat\phi\) is a scalar field
with
\[
