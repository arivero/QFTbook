...
Equivalently, write \(x=e^\tau\Omega_x\) under the Weyl map
\(\R^D\setminus\{0\}\simeq\R_\tau\times S^{D-1}\).  For scalar primaries,
\(\mathcal O_i(x)=|x|^{-\Delta_i}\widetilde{\mathcal O}_i(\tau,\Omega_x)\),
so the product creates the cylinder state
\[
  \widetilde{\mathcal O}_i(\tau,\Omega_x)\ket{\mathcal O_j}
  \in\mathcal H_{S^{D-1}}.
\]
...
generated by local operators.  Schematically,
\[
  V_i\otimes V_j
  \longrightarrow
...
\begin{figure}[H]
  ...
  \caption{The radial variable \(\rho=re^{\ii\alpha}\) puts the
