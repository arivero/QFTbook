...
primary conformal multiplet.  In the \(12\to34\) channel one writes
\[
  \mathcal G(u,v)
  =
  \sum_{\mathcal O}
...
\]
where \(\mathcal O\) has scaling dimension \(\Delta\) and spin \(\ell\).  The
function \(G_{\Delta,\ell}^{(12|34)}\) is the conformal block: it is the sum
of the primary \(\mathcal O\) and all of its descendants, with descendant
