...
\begin{enumerate}
  ...
  \item A Poincare-invariant unit vector \(\Omega\in\Hilb\), so
  \(U(\Lambda,a)\Omega=\Omega\).
...
for this construction.  Let \(g_{\mu\nu}\) be a smooth background metric and
let \(\eta^A(x)\) be smooth compactly supported sources for a chosen local
operator basis \(\mathcal O_A\).  In the source convention used here,
\[
  \exp W_\ast[g,\eta]
  =
  \left\langle
    \exp\left[-\int\dd^D x\,\sqrt g\,\eta^A(x)\mathcal O_A(x)\right]
  \right\rangle_g .
\]
...
\begin{equation}
  ...
  =
  -\frac{2}{\sqrt g(x)}
  \frac{\delta W_\ast[g,\eta]}{\delta g_{\mu\nu}(x)},
  \qquad
  \left\langle \mathcal O_A(x)\right\rangle_{g,\eta}
  =
  -\frac{1}{\sqrt g(x)}
  \frac{\delta W_\ast[g,\eta]}{\delta\eta^A(x)} .
...
\[
  ...
  \xi^\mu\nabla_\mu\eta^A
\]
for scalar sources.  Invariance of \(W_\ast[g,\eta]\) gives
\begin{equation}
  \nabla_\mu
  \left\langle T^{\mu\nu}(x)\right\rangle_{g,\eta}
  =
...
Differentiating this identity with respect to \(\eta\) and then setting
\(\eta=0\) produces the contact terms in stress-tensor Ward identities.
...
\[
  \delta_\sigma g_{\mu\nu}=2\sigma g_{\mu\nu},
  \qquad
  \delta_\sigma\eta^A=(\Delta_A-D)\sigma\eta^A .
\]
