...
Equation \eqref{eq:lorentzian-charge-commutator-by-slices} is therefore not a
new assumption; it is the definition of the commutator after the charge has
been represented on the Hilbert spaces immediately to the future and past of
the insertion.  After Euclidean continuation, the two surfaces close into a
small hypersurface surrounding the insertion, and the commutator becomes the
small-surface action used below.
...
The surface identity can be written locally by leaving the stress tensor at
the point \(x\).  For scalar insertions the diffeomorphism Ward identity has
...
\end{equation}
away from additional contact terms associated with composite-operator
renormalization.  If \(\mathcal O_i\) carries spin, the right side also
contains a local rotation acting on its spin indices.

The trace Ward identity records the response to Weyl rescaling.  For scalar
primary insertions of dimensions \(\Delta_i\), its flat-space form at a
conformal fixed point is
\begin{equation}
  \left\langle
    T^\mu{}_\mu(x)\prod_{i=1}^n\mathcal O_i(x_i)
  \right\rangle
  =
  -\sum_{i=1}^n
...
\end{equation}
The term \(\mathcal A\) is the possible Weyl-anomaly contribution in a curved
background, specialized after variation to flat space.  On flat space with
separated insertions it vanishes except for contact terms.

Equations \eqref{eq:local-translation-ward} and
\eqref{eq:local-trace-ward}, integrated against a conformal Killing vector
\(\epsilon^\nu\), reproduce \eqref{eq:global-conformal-ward-identity}.  The
conformal algebra therefore acts on local operators through stress-tensor
surface integrals, and the same action constrains all correlation functions.
