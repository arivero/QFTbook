...
\end{equation}
Assume the closed operators \(Q\) and \(Q^\dagger\) have a common invariant
dense domain on which the Hodge decomposition holds:
\[
  \Hilb
  =
  \ker\mathcal B
  \oplus
  \overline{\operatorname{im}Q}
  \oplus
  \overline{\operatorname{im}Q^\dagger}.
\]
This condition follows, for example, when zero is an isolated spectral value
of \(\mathcal B\) on the sector being considered.  If \(\ket\psi\in\ker Q\),
write
\[
  \ket\psi=\ket h+Q\ket\alpha+Q^\dagger\ket\beta .
\]
Applying \(Q\) and using \(Q^2=0\) shows
\[
  0=Q\ket\psi=QQ^\dagger\ket\beta .
\]
Taking the inner product with \(\ket\beta\) gives
\(\|Q^\dagger\ket\beta\|^2=0\), so \(\ket\psi\) is cohomologous to the
harmonic vector \(\ket h\).  Orthogonality to
\(\overline{\operatorname{im}Q}\) gives uniqueness of this representative.
Thus
\[
...
when quantum numbers allow a recombination rule of the representation-theoretic
form
\[
  L_{\Delta\to\Delta_*}
  \longrightarrow
