...
\]
The fermion field itself is a semi-local soliton vertex operator.  In the
equal-time canonical chart
\[
...
Clifford generators, and the displayed vertex is understood in the same UV
normal-ordering scheme as the sine-Gordon perturbation.  The line integral
makes \(\psi_\pm\) carry topological charge and gives the semi-local exchange
phase with \(\varphi\).  Computing equal-time commutators from the displayed
canonical relation gives the current relation above, while the short-distance
exchange phase of the two Mandelstam exponentials fixes the Coleman relation
between \(g_{\mathrm T}\) and \(\beta\).  In particular,
\[
  \bar\psi\psi
  \quad\text{is proportional, after the same finite normalization choice, to}\quad
  :\!\cos(\beta\varphi)\!: .
\]
