...
\begin{remark}[\(SU(N)\) nesting origin]
\label{rem:su-n-nested-origin}
...
The full proof is finite-dimensional algebra, but it requires the complete
recursive nested monodromy construction and the corresponding transfer-matrix
eigenvalue at every level.  The paragraph above records the origin of the
displayed equations and the pole-cancellation mechanism; it is not a
completeness theorem for Bethe vectors.  Completeness for every chain length
and every admissible representation is a separate problem; it depends on
boundary conditions, singular roots, and the treatment of coincident solutions.
