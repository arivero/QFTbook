...
\begin{proof}
The trace in \eqref{eq:aba-transfer-matrix} is a partial trace.  If
\[
  X\in\operatorname{End}(V_0\otimes V_{0'}\otimes\mathcal H_L),
\]
then \(\operatorname{tr}_{0,0'}X\) is the unique endomorphism of
\(\mathcal H_L\) satisfying
\[
  \operatorname{tr}_{\mathcal H_L}
  \bigl(Y\,\operatorname{tr}_{0,0'}X\bigr)
  =
  \operatorname{tr}_{0,0',\mathcal H_L}
  \bigl((1_{0,0'}\otimes Y)X\bigr)
\]
for finite-rank test operators \(Y\) on \(\mathcal H_L\).  In components this
definition preserves the order of the quantum-space endomorphisms, so
\[
  t(u)t(v)
  =
...
\]
The proof therefore uses cyclicity only in the finite auxiliary trace, never
cyclicity in the quantum space.
...
\]
Since \(R_{00'}(u-v)\) acts trivially on \(\mathcal H_L\), it may be moved
cyclically inside the finite trace over \(V_0\otimes V_{0'}\):
\[
\begin{aligned}
  t(u)t(v)
  &=
...
\end{aligned}
\]
Both sides are meromorphic \(\operatorname{End}(\mathcal H_L)\)-valued
functions of \(u,v\).  The complement of the auxiliary invertibility locus is
a proper analytic subset, so equality on the nonempty open invertibility
domain extends as a meromorphic identity on the common domain of meromorphy.
\end{proof}
...
\end{align}
The signs in \eqref{eq:aba-ab-relation}--\eqref{eq:aba-db-relation} are tied
to the convention \(T_0(u)=\begin{psmallmatrix}A&B\\ C&D\end{psmallmatrix}\),
\(R(u)=u\,1+\ii P\), and \(L(u)=R(u-\ii/2)\).  In this convention \(B\) lowers
one spin relative to the ferromagnetic vector, \(A(u)\Omega_L=(u+\ii/2)^L
\Omega_L\), and \(D(u)\Omega_L=(u-\ii/2)^L\Omega_L\).  For a single site the
relations reduce to
\[
  A(u)B=(u-\ii/2)B,\qquad
  BA(u)=(u+\ii/2)B,
\]
and similarly \(D(u)B=(u+\ii/2)B\), \(BD(u)=(u-\ii/2)B\); this one-site check
already fixes the displayed signs.  For general \(L\) the same relations are
the component form of RTT, obtained by inserting auxiliary matrix units
\(E_{ab}^{(0)}E_{cd}^{(0')}\) in
\eqref{eq:aba-rtt-relation} and comparing the corresponding
\(\operatorname{End}(\mathcal H_L)\)-valued coefficients.  The apparent poles
at \(u=v\) are not singularities of the operator identity after multiplication
by \(u-v\); they encode the rational \(R\)-matrix normalization.
...
\begin{equation}
\begin{aligned}
  A(u)\prod_{k=1}^M B(u_k)\Omega_L
  &=
  a(u)\prod_{k=1}^M\frac{u-u_k-\ii}{u-u_k}
  \prod_{k=1}^M B(u_k)\Omega_L
  \\
  &\quad
  +\sum_{j=1}^M
  \frac{\ii\,a(u_j)}{u-u_j}
  ...
  \right).
\end{aligned}
\label{eq:aba-a-on-bethe-vector-recursion}
\end{equation}
\begin{equation}
\begin{aligned}
  D(u)\prod_{k=1}^M B(u_k)\Omega_L
  &=
  d(u)\prod_{k=1}^M\frac{u-u_k+\ii}{u-u_k}
  \prod_{k=1}^M B(u_k)\Omega_L
  \\
  &\quad
  -\sum_{j=1}^M
  ...
  \right).
\end{aligned}
\label{eq:aba-d-on-bethe-vector-recursion}
\end{equation}
For \(M=1\) these are exactly \eqref{eq:aba-ab-relation} and
\eqref{eq:aba-db-relation} applied to \(\Omega_L\).  The induction step moves
\(A(u)\), respectively \(D(u)\), through the leftmost \(B\)-factor.  The
wanted part gives the first displayed product factor.  The unwanted part
changes the moving diagonal entry to \(A(u_j)\), respectively \(D(u_j)\), and
the remaining \(B\)-factors are then passed only by wanted moves; if two
unwanted moves are chosen in opposite orders, the two normally ordered terms
cancel by the rational identity obtained from the same RTT component relation
that gives \(B(u)B(v)=B(v)B(u)\).  Thus the induction produces exactly one
replaced rapidity in the final normal form and no terms with two replaced
rapidities.

Adding \eqref{eq:aba-a-on-bethe-vector-recursion} and
\eqref{eq:aba-d-on-bethe-vector-recursion}, the wanted terms give the
eigenvalue in \eqref{eq:aba-xxx-transfer-eigenvalue}.  The coefficient of
\[
