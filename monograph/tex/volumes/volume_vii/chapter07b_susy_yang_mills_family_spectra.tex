...
\item a common gauge-field regulator with lattice spacing or cutoff \(a\), a
  ...
  line-operator lattice;
\item a renormalized one-parameter path \(H_{a,L}(m)\) of finite-volume
  Hamiltonians along a fixed ray \(m=\operatorname{Re}m_\lambda\) in the soft
  source plane, including the counterterm and tuning prescriptions that keep
  the gauge coupling, vacuum energy, and relevant fermion-mass coordinate in
  the declared scheme;
\item the renormalized perturbing operator
  \begin{equation}
    \mathcal V_{a,L}(m)=\partial_m H_{a,L}(m),
  ...
  contact and counterterm derivatives;
...
Assume the datum of
Definition~\ref{def:soft-mass-spectral-bridge-datum}.  Let \(I_n(0)\) be an
isolated local-channel spectral cluster of \(H_{a,L}(0)\), separated from the
rest of the finite-volume spectrum by distance \(g_n>0\), and let
\(\mathcal C_n\) be a contour enclosing only that cluster.  If
\(\|(z-H_{a,L}(m))^{-1}\mathcal V_{a,L}(m)\|\le K_n/g_n\) on
\(\mathcal C_n\) throughout the interval, then the Riesz projection
\begin{equation}
  P_n(m)=\frac{1}{2\pi\ii}
  \oint_{\mathcal C_n}(z-H_{a,L}(m))^{-1}\,\dd z
...
\begin{equation}
  ...
  \frac{1}{2\pi\ii}
  \oint_{\mathcal C_n}
  (z-H_{a,L}(m))^{-1}\mathcal V_{a,L}(m)
  (z-H_{a,L}(m))^{-1}\,\dd z .
  \label{eq:soft-mass-projection-derivative}
\end{equation}
For the cluster centroid
\[
  \overline E_n(m)=\frac{1}{d_n}\operatorname{Tr}\bigl(P_n(m)H_{a,L}(m)\bigr),
...
\begin{equation}
  ...
  =
  \frac{1}{d_n}\operatorname{Tr}\bigl(P_n(m)\mathcal V_{a,L}(m)P_n(m)\bigr)
  -
  \langle\Omega_m,\mathcal V_{a,L}(m)\Omega_m\rangle .
...
\begin{equation}
  ...
  =
  \frac1L\left(
    \langle k,L;m|\mathcal V_{a,L}(m)|k,L;m\rangle
    -
    \langle\Omega_m,\mathcal V_{a,L}(m)\Omega_m\rangle
  \right),
...
The proof is the standard finite-volume perturbation argument, included here
to expose the physics input.  The resolvent identity gives
\[
  \partial_m(z-H_{a,L}(m))^{-1}
  =
  (z-H_{a,L}(m))^{-1}\mathcal V_{a,L}(m)(z-H_{a,L}(m))^{-1},
\]
which is \eqref{eq:soft-mass-projection-derivative} after contour
integration.  Differentiating
\(\operatorname{Tr}(P_nH_{a,L})\) gives two terms:
\[
  \operatorname{Tr}((\partial_mP_n)H_{a,L})
  +
  \operatorname{Tr}(P_n\mathcal V_{a,L}).
\]
The first vanishes because \(P_n\) commutes with \(H_{a,L}\) and
...
contour intersects another level, if center flux can break by dynamical
matter in the chosen global form, if the fermion regulator changes Pfaffian
or reflection-positivity status, or if the counterterm/tuning trajectory
needed to define \(\mathcal V_{a,L}\) is not uniform in the continuum and
thermodynamic limits.  These are the phase-boundary tests for the soft-mass
path.  What the bridge supplies is a controlled derivative and stability
criterion while the tests pass; it does not turn the large-\(|m_\lambda|\)
decoupling limit into a theorem.
