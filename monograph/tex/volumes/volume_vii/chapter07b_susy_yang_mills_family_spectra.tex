...
energies may be estimated by applying the finite-volume correlator-matrix
method of Section~\ref{sec:lattice-glueball-correlator-gevp} separately to
the three channel matrices.  In particular, the corresponding generalized
...
\end{equation}
This is only the general finite-matrix estimator applied to the pure-SYM
source channels.  It is not a supersymmetric definition of the spectrum, and
it supplies no multiplet statement unless the source basis, residual
higher-state contamination, finite-volume effects, and regulator limits have
all been controlled in the sense of
Section~\ref{sec:lattice-glueball-correlator-gevp}.  The underlying object is
the spectral measure of \(\ee^{-tH_{\rm reg}}\) in the corresponding channel.
...
If a matrix GEVP is used instead of projected correlators, its justification is
the general finite-regulator statement of
Section~\ref{sec:lattice-glueball-correlator-gevp}, applied to
\(C_+(t),C_-(t),C_F(t)\) channel by channel with a published residual bound.
Any persistent nonzero splitting among the continuum-extrapolated leading pole
positions is therefore evidence that at least one hypothesis has failed: the
sources have isolated different states, the regulator limit has not restored
supersymmetry in that window, or the assumed one-particle atom is not
...
The matrix GEVP is a useful way of choosing the coefficient vector \(u_a\)
from a source family, but its finite-state diagonalization and residual
estimates are the general linear-algebra analysis of
Section~\ref{sec:lattice-glueball-correlator-gevp}.  The present proposition
uses only the channel spectral gap and overlap hypotheses.  The final
diagnostic statement is the contrapositive of the common-pole hypothesis
together with these extraction formulae.
\end{proof}
