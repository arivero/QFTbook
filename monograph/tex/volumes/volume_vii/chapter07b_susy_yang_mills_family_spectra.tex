...
\paragraph{Application of the general spectral estimator.}
The common-pole analysis below is not a theorem special to pure
\(\mathcal N=1\) Yang--Mills theory.  It is the general finite-volume
spectral estimate of Section~\ref{sec:lattice-glueball-correlator-gevp}
applied to the three source channels singled out by the expected massive
supermultiplet structure.  The supersymmetric content is the statement of
which channels must agree after the regulator, volume, and gluino-mass limits
are taken; the linear-algebra extraction itself belongs to the general
finite-regulator methodology.

Assume, for a specified finite-volume window and source basis, that the
regulated extraction contains one isolated massive candidate multiplet of
mass \(m>0\), with a gap \(\Delta>0\) to the next spectral contribution in
each of the three channel matrices of
Definition~\ref{def:pure-sym-channel-correlator-matrices}.  Assume also that
the chosen source vectors have nonzero overlap with the corresponding atom.
Then the channel correlators can be written in the controlled form
\begin{equation}
  C_a(t)
  =
...
\end{equation}
where \(K_a\) is a bound on the residual spectral measure in that channel
after multiplication by \(\ee^{(m+\Delta)t}\).  For every coefficient vector
\(u_a\) whose overlap
...
\end{equation}
The displayed formula is obtained by contracting
\eqref{eq:pure-sym-common-pole-channel-form} with \(u_a^\dagger\) and
\(u_a\), writing the residual term as
\(|z_{a,u}|^2\ee^{-mt}\rho_{a,u}(t)\), and using the spectral gap bound.  If
the right-hand side bound at \(s=t_0\) is less than one, then
\begin{equation}
  \left|
  \ee^{m(t-t_0)}
  \frac{G_{a,u}(t)}{G_{a,u}(t_0)}
...
\[
  ...
  \frac{\varepsilon(t)+\varepsilon(t_0)}
       {1-\varepsilon(t_0)} .
\]
If a matrix GEVP is used instead of a hand-chosen projected correlator, its
justification is precisely the general finite-regulator statement of
...
positions is therefore not a new theorem about supersymmetry; it is a
diagnostic that at least one stated input has failed: the sources have
isolated different states, the regulator limit has not restored supersymmetry
in that window, or the assumed one-particle atom is not separated from the
continuum.
