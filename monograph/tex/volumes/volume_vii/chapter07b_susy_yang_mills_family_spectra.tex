...
Fix the regulated extraction datum of
Definition~\ref{def:regulated-pure-sym-spectral-extraction-datum}.  Choose
...
Definition~\ref{def:pure-sym-channel-source-ledger}, \(B_-\) has the
...
Assume the continuum limit of the regulated datum contains an isolated massive
...
Definition~\ref{def:regulated-pure-sym-spectral-extraction-datum}.
...
precise low-energy spectral datum; comparison with the non-Abelian
...
\begin{definition}[Abelianized \(A_{N_c-1}\) string datum]
\label{def:abelianized-a-string-datum}
An Abelianized \(A_{N_c-1}\) string datum consists of:
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
This datum defines an Abelianized string spectrum in the local effective
...
Assume the Abelianized datum of
Definition~\ref{def:abelianized-a-string-datum} and suppose that, in the
...
\]
Consequently, if an Abelianized \(SU(N_c)\) string datum has aligned FI
magnitudes
\[
...
Assume a comparison datum as in
...
\mathcal N=2\text{ Abelianized }k\text{-strings} &
  \text{\(A\)-type sine profile; comparison datum}\\
