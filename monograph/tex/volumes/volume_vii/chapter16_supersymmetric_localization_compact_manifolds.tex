...
Assume, in addition to Proposition~\ref{prop:susy-localization-deformation-identity},
that \(\operatorname{Re}(QV)\ge0\) on the bosonic body of \(C\), that \(F\) is
compact and clean, and that the large-\(t\) integral is dominated by a tubular
neighborhood of \(F\).  In local coordinates \(y\) along \(F\), bosonic normal
coordinates \(x^i\), and fermionic normal coordinates \((\psi^a,\chi^r)\), the
quadratic part may be written after choosing splittings as
\[
  (QV)_{\rm quad}
  =
  \frac12 x^i A_{ij}(y)x^j
  +\frac12\psi^a B_{ab}(y)\psi^b
  +\chi^r C_{ri}(y)x^i .
\]
Here \(A\) is symmetric on the declared real normal cycle, \(B\) is
antisymmetric, and \(C\) records the mixed fermion-boson differential in the
normal \(Q\)-complex.  With the conventional finite-dimensional Gaussian
normalization absorbed into the definition of \(\mu\), the local normal factor
is
\[
  Z_{\rm normal}(y)
  =
  \frac{\operatorname{Pf}B(y)\,\det C(y)}
       {\sqrt{\det A(y)}} .
  \label{eq:finite-normal-localization-factor}
\]
Unpaired zero modes are not included in this determinant: they are coordinates
on \(F\), insertions to be saturated, or obstructions showing that the proposed
localized observable vanishes.  Thus a determinant formula without a zero-mode
statement is not a localization theorem.  Equation
\eqref{eq:finite-normal-localization-factor} is the finite model for the
one-loop determinants in the compact-space QFT examples below.
