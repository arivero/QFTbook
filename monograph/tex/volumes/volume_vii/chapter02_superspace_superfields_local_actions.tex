...
We use
\[
  \theta^2=\frac12\theta^\alpha\theta_\alpha,\qquad
  \bar\theta^2=\frac12\bar\theta_{\dot\alpha}\bar\theta^{\dot\alpha},
  \qquad
...
\(\partial^L\theta^2/\partial\theta^\alpha=\theta_\alpha\), the
\(\epsilon Q\) part contributes \(\sqrt2\,\epsilon_\alpha F\), and the
...
\[
  ...
  +\partial_\mu(\cdots).
\]
The total derivative is discarded in the action.  For the \(F\)-term, use
Taylor expansion of the holomorphic function in the nilpotent increment
...
\[
  ...
  W(\phi)
  +\partial_iW(\phi)
  \bigl(\sqrt2\,\theta\psi^i+\theta^2F^i\bigr)
  +\frac12\partial_i\partial_jW(\phi)
  \bigl(\sqrt2\,\theta\psi^i\bigr)
  \bigl(\sqrt2\,\theta\psi^j\bigr).
\]
Since \(\theta^\alpha\theta^\beta\psi^i_\alpha\psi^j_\beta
=-\frac12\theta^\rho\theta_\rho\,\psi^{i\alpha}\psi^j_\alpha\), the
