\renewcommand{\bibname}{Bibliographic Guide}

\begin{thebibliography}{99}
\setlength{\emergencystretch}{3em}
\raggedright
\addcontentsline{toc}{chapter}{Bibliographic Guide}

\item[]
\textbf{How this bibliography is used.}
The references below are source traces for theorem-boundary statements,
historical origin of constructions, and convention checks.  A citation is not
a proof in this monograph.  Every result used in the logical development must
still be derived in the local text, stated with its hypotheses as a
quoted theorem, or marked as a hypothesis, controlled approximation,
conjecture, or open problem.

\item[]
This first seed covers the most frequently quoted foundational lineages:
Wightman and OS reconstruction
\cite{Wightman1956,StreaterWightman1964,Jost1965,OS1973,OS1975,
GubinelliHofmanova2021,KravchukQiaoRychkov2021}, scattering theory
\cite{Haag1955,HallWightman1957,Haag1958,Ruelle1962,
Steinmann1960I,Steinmann1960II,Stapp1971,CahillStapp1973,CahillStapp1975,
LSZ1955}, perturbative and
Wilsonian renormalization \cite{BPH1957,Hepp1966,Zimmermann1969,Polchinski1984},
loop-amplitude reconstruction and integral reduction
\cite{BDDK1994Unitarity,BDK1996OneLoopQCD,
BrittoCachazoFeng2005GeneralizedUnitarity,OPP2007IntegrandReduction,
Forde2007OneLoopCoefficients,BernMorgan1996MassiveUnitarity,
BrandhuberEtAl2005PureYMUnitarity,PassarinoVeltman1979,
ChetyrkinTkachov1981IBP,Tkachov1981AnalyticalCalculability,
Kotikov1991DifferentialEquations,GehrmannRemiddi2000DifferentialEquations,
GehrmannDeRidderGehrmannGlover2005Antenna,
LaportaRemiddi2005Sunrise,AdamsBognerWeinzierl2013Sunrise,
UssyukinaDavydychev1993Ladders},
AQFT superselection and modular theory
\cite{HaagKastler1964,ReehSchlieder1961,DHR1971,DHR1974,DR1989,DR1990,
GelfandNaimark1943,Segal1947,Takesaki1970,Connes1973,DAntoniLongo1983,
Borchers1992},
constructive and stochastic routes \cite{GlimmJaffe1987,Hairer2014},
low-dimensional continuous-symmetry obstructions
\cite{Coleman1973NoGoldstone,MerminWagner1966,Hohenberg1967,
Berezinskii1971,KosterlitzThouless1973}, non-Lorentz
Nambu--Goldstone counting
\cite{NielsenChadha1976GoldstoneCounting,
WatanabeBrauner2011NGBChargeDensity,WatanabeMurayama2012UnifiedNGB,
WatanabeMurayama2014NonrelativisticEL,Leutwyler1994NonrelativisticEL},
EFT positivity constraints
\cite{AdamsEtAl2006IRObstruction},
massless-particle no-go boundaries
\cite{WeinbergWitten1980,Loebbert2008WeinbergWitten,
BenedettiCasiniMagan2022Noether},
false-vacuum decay
\cite{Coleman1977FalseVacuum,CallanColeman1977Fate,ColemanDeLuccia1980},
gauge dependence of effective potentials
\cite{Nielsen1975GaugeDependence,FukudaKugo1976GaugeEffectiveAction,
AitchisonFraser1984GaugeEffectivePotential,MetaxasWeinberg1996BubbleGauge},
high-energy longitudinal-vector amplitudes
\cite{CornwallLevinTiktopoulos1974,LeeQuiggThacker1977,
ChanowitzGaillard1985,Veltman1990EquivalenceTheorem,BaggerSchmidt1990},
curved-spacetime local covariance and pAQFT
\cite{BrunettiFredenhagenVerch2003,BrunettiFredenhagenKoehler1996,
Radzikowski1996,KayWald1991,
Moretti2003StressEnergy,HollandsWald2001LocalWick,
HollandsWald2005Conservation,HollandsWald2015QFCS},
CFT and
VOA structure
\cite{Segal2004,KontsevichSegal2021,Zhu1996,Huang2005,DO1994,ZZ1996},
sigma-model Weyl consistency
\cite{CurciPaffuti1987,SchnittgerEllwanger1992}, anomalies and index
theory
\cite{AdlerBardeen1969,Bardeen1969AnomalousWard,PiguetSorella1995,
BarnichBrandtHenneaux2000,Anselmi2014AdlerBardeen,Seeley1967,Gilkey1984,
AtiyahSinger1968,APS1975I,APS1975II,APS1976III,
BismutFreed1986,DaiFreed1994,Freed1995DeterminantRevisited,Witten1982}, instantons
\cite{tHooft1976Pseudoparticle}, supersymmetric dynamics
\cite{SeibergWitten1994I,SeibergWitten1994II,Seiberg1994}, lattice BRST
obstructions \cite{Neuberger1986NonperturbativeBRS,Neuberger1987BRSGribov},
and locality laboratories and lattice filling constraints
\cite{LiebRobinson1972,LiebSchultzMattis1961,Oshikawa2000Commensurability,
BachmannMichalakisNachtergaeleSims2012Automorphic,Hastings2004LSMHigher}.
Later passes should attach additional
citations at the precise theorem-boundary sites and expand this guide.

\bibitem{Wightman1956}
A.~S. Wightman.
Quantum field theory in terms of vacuum expectation values.
\emph{Physical Review} \textbf{101} (1956), 860--866.

\bibitem{StreaterWightman1964}
R.~F. Streater and A.~S. Wightman.
\emph{PCT, Spin and Statistics, and All That}.
Benjamin, 1964.

\bibitem{Jost1965}
R.~Jost.
\emph{The General Theory of Quantized Fields}.
American Mathematical Society, 1965.

\bibitem{Haag1955}
R.~Haag.
On quantum field theories.
\emph{Det Kongelige Danske Videnskabernes Selskab, Matematisk-fysiske
Meddelelser} \textbf{29} (1955), no.~12, 1--37.

\bibitem{HallWightman1957}
D.~Hall and A.~S.~Wightman.
A theorem on invariant analytic functions with applications to relativistic
quantum field theory.
\emph{Det Kongelige Danske Videnskabernes Selskab, Matematisk-fysiske
Meddelelser} \textbf{31} (1957), no.~5, 1--41.

\bibitem{Haag1958}
R.~Haag.
Quantum field theories with composite particles and asymptotic conditions.
\emph{Physical Review} \textbf{112} (1958), 669--673.

\bibitem{Ruelle1962}
D.~Ruelle.
On the asymptotic condition in quantum field theory.
\emph{Helvetica Physica Acta} \textbf{35} (1962), 147--163.

\bibitem{Steinmann1960I}
O.~Steinmann.
{\"U}ber den Zusammenhang zwischen den Wightmanfunktionen und den retardierten
Kommutatoren.
\emph{Helvetica Physica Acta} \textbf{33} (1960), 257--298.

\bibitem{Steinmann1960II}
O.~Steinmann.
Wightman-Funktionen und retardierte Kommutatoren. II.
\emph{Helvetica Physica Acta} \textbf{33} (1960), 347--362.

\bibitem{Stapp1971}
H.~P. Stapp.
Inclusive cross sections are discontinuities.
\emph{Physical Review D} \textbf{3} (1971), 3177--3184.

\bibitem{CahillStapp1973}
K.~E. Cahill and H.~P. Stapp.
Generalized optical theorems and Steinmann relations.
\emph{Physical Review D} \textbf{8} (1973), 2714--2720.

\bibitem{CahillStapp1975}
K.~E. Cahill and H.~P. Stapp.
Optical theorems and Steinmann relations.
\emph{Annals of Physics} \textbf{90} (1975), 438--514.

\bibitem{LSZ1955}
H.~Lehmann, K.~Symanzik, and W.~Zimmermann.
On the formulation of quantized field theories.
\emph{Il Nuovo Cimento} \textbf{1} (1955), 205--225.

\bibitem{CornwallLevinTiktopoulos1974}
J.~M. Cornwall, D.~N. Levin, and G.~Tiktopoulos.
Derivation of gauge invariance from high-energy unitarity bounds on the
\(S\)-matrix.
\emph{Physical Review D} \textbf{10} (1974), 1145--1167.

\bibitem{LeeQuiggThacker1977}
B.~W. Lee, C.~Quigg, and H.~B. Thacker.
Weak interactions at very high energies: the role of the Higgs-boson mass.
\emph{Physical Review D} \textbf{16} (1977), 1519--1531.

\bibitem{ChanowitzGaillard1985}
M.~S. Chanowitz and M.~K. Gaillard.
The TeV physics of strongly interacting \(W\)'s and \(Z\)'s.
\emph{Nuclear Physics B} \textbf{261} (1985), 379--431.

\bibitem{Veltman1990EquivalenceTheorem}
M.~J.~G. Veltman.
The equivalence theorem.
\emph{Physical Review D} \textbf{41} (1990), 2294--2302.

\bibitem{BaggerSchmidt1990}
J.~Bagger and C.~Schmidt.
Equivalence theorem redux.
\emph{Physical Review D} \textbf{41} (1990), 264--270.

\bibitem{AnselmiFreedmanGrisaruJohansen1998}
D.~Anselmi, D.~Z. Freedman, M.~T. Grisaru, and A.~A. Johansen.
Nonperturbative formulas for central functions of supersymmetric gauge
theories.
\emph{Nuclear Physics B} \textbf{526} (1998), 543--571.
arXiv:hep-th/9708042.

\bibitem{OsbornPetkou1994}
H.~Osborn and A.~C. Petkou.
Implications of conformal invariance in field theories for general dimensions.
\emph{Annals of Physics} \textbf{231} (1994), 311--362.
arXiv:hep-th/9307010.

\bibitem{ArkaniHamedMurayama1997}
N.~Arkani-Hamed and H.~Murayama.
Holomorphy, rescaling anomalies and exact beta functions in supersymmetric
gauge theories.
\emph{Journal of High Energy Physics} \textbf{06} (2000), 030.
arXiv:hep-th/9707133.

\bibitem{IntriligatorWecht2003}
K.~A. Intriligator and B.~Wecht.
The exact superconformal \(R\)-symmetry maximizes \(a\).
\emph{Nuclear Physics B} \textbf{667} (2003), 183--200.
arXiv:hep-th/0304128.

\bibitem{BenArousGuionnet1997}
G.~Ben Arous and A.~Guionnet.
Large deviations for Wigner's law and Voiculescu's non-commutative entropy.
\emph{Probability Theory and Related Fields} \textbf{108} (1997), 517--542.

\bibitem{AndersonGuionnetZeitouni2010}
G.~W. Anderson, A.~Guionnet, and O.~Zeitouni.
\emph{An Introduction to Random Matrices}.
Cambridge University Press, 2010.

\bibitem{Coleman1973NoGoldstone}
S.~Coleman.
There are no Goldstone bosons in two dimensions.
\emph{Communications in Mathematical Physics} \textbf{31} (1973), 259--264.

\bibitem{Coleman1977FalseVacuum}
S.~Coleman.
The fate of the false vacuum. 1. Semiclassical theory.
\emph{Physical Review D} \textbf{15} (1977), 2929--2936.

\bibitem{CallanColeman1977Fate}
C.~G. Callan, Jr. and S.~Coleman.
The fate of the false vacuum. 2. First quantum corrections.
\emph{Physical Review D} \textbf{16} (1977), 1762--1768.

\bibitem{ColemanDeLuccia1980}
S.~Coleman and F.~De Luccia.
Gravitational effects on and of vacuum decay.
\emph{Physical Review D} \textbf{21} (1980), 3305--3315.

\bibitem{Nielsen1975GaugeDependence}
N.~K. Nielsen.
On the gauge dependence of spontaneous symmetry breaking in gauge theories.
\emph{Nuclear Physics B} \textbf{101} (1975), 173--188.
doi:10.1016/0550-3213(75)90301-6.

\bibitem{FukudaKugo1976GaugeEffectiveAction}
R.~Fukuda and T.~Kugo.
Gauge invariance in the effective action and potential.
\emph{Physical Review D} \textbf{13} (1976), 3469--3484.
doi:10.1103/PhysRevD.13.3469.

\bibitem{AitchisonFraser1984GaugeEffectivePotential}
I.~J.~R. Aitchison and C.~M. Fraser.
Gauge invariance and the effective potential.
\emph{Annals of Physics} \textbf{156} (1984), 1--40.
doi:10.1016/0003-4916(84)90209-4.

\bibitem{MetaxasWeinberg1996BubbleGauge}
D.~Metaxas and E.~J. Weinberg.
Gauge independence of the bubble nucleation rate in theories with radiative
symmetry breaking.
\emph{Physical Review D} \textbf{53} (1996), 836--843.
arXiv:hep-ph/9507381.
doi:10.1103/PhysRevD.53.836.

\bibitem{MerminWagner1966}
N.~D. Mermin and H.~Wagner.
Absence of ferromagnetism or antiferromagnetism in one- or two-dimensional
isotropic Heisenberg models.
\emph{Physical Review Letters} \textbf{17} (1966), 1133--1136.

\bibitem{Hohenberg1967}
P.~C. Hohenberg.
Existence of long-range order in one and two dimensions.
\emph{Physical Review} \textbf{158} (1967), 383--386.

\bibitem{Berezinskii1971}
V.~L. Berezinskii.
Destruction of long-range order in one-dimensional and two-dimensional systems
having a continuous symmetry group. I. Classical systems.
\emph{Soviet Physics JETP} \textbf{32} (1971), 493--500.

\bibitem{KosterlitzThouless1973}
J.~M. Kosterlitz and D.~J. Thouless.
Ordering, metastability and phase transitions in two-dimensional systems.
\emph{Journal of Physics C: Solid State Physics} \textbf{6} (1973),
1181--1203.

\bibitem{NielsenChadha1976GoldstoneCounting}
H.~B. Nielsen and S.~Chadha.
On how to count Goldstone bosons.
\emph{Nuclear Physics B} \textbf{105} (1976), 445--453.
doi:10.1016/0550-3213(76)90025-0.

\bibitem{WatanabeBrauner2011NGBChargeDensity}
H.~Watanabe and T.~Brauner.
On the number of Nambu--Goldstone bosons and its relation to charge densities.
\emph{Physical Review D} \textbf{84} (2011), 125013.
arXiv:1109.6327.
doi:10.1103/PhysRevD.84.125013.

\bibitem{WatanabeMurayama2012UnifiedNGB}
H.~Watanabe and H.~Murayama.
Unified description of Nambu--Goldstone bosons without Lorentz invariance.
\emph{Physical Review Letters} \textbf{108} (2012), 251602.
arXiv:1203.0609.
doi:10.1103/PhysRevLett.108.251602.

\bibitem{WatanabeMurayama2014NonrelativisticEL}
H.~Watanabe and H.~Murayama.
Effective Lagrangian for nonrelativistic systems.
\emph{Physical Review X} \textbf{4} (2014), 031057.
arXiv:1402.7066.
doi:10.1103/PhysRevX.4.031057.

\bibitem{Leutwyler1994NonrelativisticEL}
H.~Leutwyler.
Nonrelativistic effective Lagrangians.
\emph{Physical Review D} \textbf{49} (1994), 3033--3043.
arXiv:hep-ph/9311264.
doi:10.1103/PhysRevD.49.3033.

\bibitem{OS1973}
K.~Osterwalder and R.~Schrader.
Axioms for Euclidean Green's functions.
\emph{Communications in Mathematical Physics} \textbf{31} (1973), 83--112.

\bibitem{OS1975}
K.~Osterwalder and R.~Schrader.
Axioms for Euclidean Green's functions II.
\emph{Communications in Mathematical Physics} \textbf{42} (1975), 281--305.

\bibitem{GubinelliHofmanova2021}
M.~Gubinelli and M.~Hofmanov\'a.
A PDE construction of the Euclidean \(\Phi^4_3\) quantum field theory.
\emph{Communications in Mathematical Physics} \textbf{384} (2021), 1--75.

\bibitem{HollandsWald2001LocalWick}
S.~Hollands and R.~M. Wald.
Local Wick polynomials and time ordered products of quantum fields in curved
spacetime.
\emph{Communications in Mathematical Physics} \textbf{223} (2001), 289--326.

\bibitem{HollandsWald2005Conservation}
S.~Hollands and R.~M. Wald.
Conservation of the stress tensor in perturbative interacting quantum field
theory in curved spacetimes.
\emph{Reviews in Mathematical Physics} \textbf{17} (2005), 227--312.

\bibitem{HollandsWald2015QFCS}
S.~Hollands and R.~M. Wald.
Quantum fields in curved spacetime.
\emph{Physics Reports} \textbf{574} (2015), 1--35.

\bibitem{Moretti2003StressEnergy}
V.~Moretti.
Comments on the stress-energy tensor operator in curved spacetime.
\emph{Communications in Mathematical Physics} \textbf{232} (2003), 189--221.
arXiv:gr-qc/0109048.

\bibitem{FredenhagenRejzner2015PerturbativeConstruction}
K.~Fredenhagen and K.~Rejzner.
Perturbative construction of models of algebraic quantum field theory.
In \emph{Advances in Algebraic Quantum Field Theory}, Springer (2015).
arXiv:1503.07814 [math-ph].

\bibitem{FredenhagenRejzner2013BVPerturbativeAQFT}
K.~Fredenhagen and K.~Rejzner.
Batalin--Vilkovisky formalism in perturbative algebraic quantum field theory.
\emph{Communications in Mathematical Physics} \textbf{317} (2013), 697--725.
arXiv:1110.5232 [math-ph].

\bibitem{Rejzner2011BVLocallyCovariant}
K.~Rejzner.
Batalin--Vilkovisky formalism in locally covariant field theory.
arXiv:1111.5130 [math-ph].

\bibitem{BrunettiDuetschFredenhagenRejzner2022UAMWI}
R.~Brunetti, M.~D\"utsch, K.~Fredenhagen, and K.~Rejzner.
Unitary, anomalous Master Ward Identity and its connections to the
Wess--Zumino condition, BV formalism and \(L_\infty\)-algebras.
arXiv:2210.05908 [math-ph].

\bibitem{HawkinsRejznerVisser2024Equicausal}
E.~Hawkins, K.~Rejzner, and B.~Visser.
A novel class of functionals for perturbative algebraic quantum field theory.
arXiv:2312.15203v2 [math-ph] (2024).

\bibitem{KravchukQiaoRychkov2021}
P.~Kravchuk, J.~Qiao, and S.~Rychkov.
Distributions in conformal field theory II: Minkowski space.
\emph{Journal of High Energy Physics} \textbf{08} (2021), 094.

\bibitem{Kato1966Perturbation}
T.~Kato.
\emph{Perturbation Theory for Linear Operators}.
Springer, 1966.

\bibitem{ReedSimon1975II}
M.~Reed and B.~Simon.
\emph{Methods of Modern Mathematical Physics II: Fourier Analysis,
Self-Adjointness}.
Academic Press, 1975.

\bibitem{Strocchi2013NonPerturbative}
F.~Strocchi.
\emph{An Introduction to Non-Perturbative Foundations of Quantum Field Theory}.
Oxford University Press, 2013.

\bibitem{BPH1957}
N.~N. Bogoliubov and O.~S. Parasiuk.
On the multiplication of causal functions in the quantum theory of fields.
\emph{Acta Mathematica} \textbf{97} (1957), 227--266.

\bibitem{Hepp1966}
K.~Hepp.
Proof of the Bogoliubov--Parasiuk theorem on renormalization.
\emph{Communications in Mathematical Physics} \textbf{2} (1966), 301--326.

\bibitem{Zimmermann1969}
W.~Zimmermann.
Convergence of Bogoliubov's method of renormalization in momentum space.
\emph{Communications in Mathematical Physics} \textbf{15} (1969), 208--234.

\bibitem{Polchinski1984}
J.~Polchinski.
Renormalization and effective Lagrangians.
\emph{Nuclear Physics B} \textbf{231} (1984), 269--295.

\bibitem{HaagKastler1964}
R.~Haag and D.~Kastler.
An algebraic approach to quantum field theory.
\emph{Journal of Mathematical Physics} \textbf{5} (1964), 848--861.

\bibitem{ReehSchlieder1961}
H.~Reeh and S.~Schlieder.
Bemerkungen zur unit\"ar\"aquivalenz von Lorentz-invarianten Feldern.
\emph{Nuovo Cimento} \textbf{22} (1961), 1051--1068.

\bibitem{GelfandNaimark1943}
I.~M. Gelfand and M.~A. Naimark.
On the imbedding of normed rings into the ring of operators in Hilbert space.
\emph{Matematicheskii Sbornik} \textbf{12(54)} (1943), 197--217.

\bibitem{Segal1947}
I.~E. Segal.
Irreducible representations of operator algebras.
\emph{Bulletin of the American Mathematical Society} \textbf{53} (1947),
73--88.

\bibitem{DHR1971}
S.~Doplicher, R.~Haag, and J.~E.~Roberts.
Local observables and particle statistics I.
\emph{Communications in Mathematical Physics} \textbf{23} (1971), 199--230.

\bibitem{DHR1974}
S.~Doplicher, R.~Haag, and J.~E.~Roberts.
Local observables and particle statistics II.
\emph{Communications in Mathematical Physics} \textbf{35} (1974), 49--85.

\bibitem{DR1989}
S.~Doplicher and J.~E.~Roberts.
A new duality theory for compact groups.
\emph{Inventiones Mathematicae} \textbf{98} (1989), 157--218.

\bibitem{DR1990}
S.~Doplicher and J.~E.~Roberts.
Why there is a field algebra with a compact gauge group describing the
superselection structure in particle physics.
\emph{Communications in Mathematical Physics} \textbf{131} (1990), 51--107.

\bibitem{Takesaki1970}
M.~Takesaki.
\emph{Tomita's Theory of Modular Hilbert Algebras and Its Applications}.
Lecture Notes in Mathematics \textbf{128}, Springer, 1970.

\bibitem{Connes1973}
A.~Connes.
Une classification des facteurs de type \(\mathrm{III}\).
\emph{Annales scientifiques de l'Ecole Normale Superieure} \textbf{6}
(1973), 133--252.

\bibitem{DAntoniLongo1983}
C.~D'Antoni and R.~Longo.
Interpolation by type I factors and the flip automorphism.
\emph{Journal of Functional Analysis} \textbf{51} (1983), 361--371.

\bibitem{BuchholzWichmann1986}
D.~Buchholz and E.~H. Wichmann.
Causal independence and the energy-level density of states in local quantum
field theory.
\emph{Communications in Mathematical Physics} \textbf{106} (1986), 321--344.

\bibitem{Borchers1992}
H.-J. Borchers.
The CPT theorem in two-dimensional theories of local observables.
\emph{Communications in Mathematical Physics} \textbf{143} (1992), 315--332.

\bibitem{Borchers2000}
H.-J. Borchers.
On revolutionizing quantum field theory with Tomita's modular theory.
\emph{Journal of Mathematical Physics} \textbf{41} (2000), 3604--3673.

\bibitem{Wiesbrock1993}
H.-W. Wiesbrock.
Half-sided modular inclusions of von Neumann algebras.
\emph{Communications in Mathematical Physics} \textbf{157} (1993), 83--92.

\bibitem{GlimmJaffe1987}
J.~Glimm and A.~Jaffe.
\emph{Quantum Physics: A Functional Integral Point of View}.
Second edition, Springer, 1987.

\bibitem{Hairer2014}
M.~Hairer.
A theory of regularity structures.
\emph{Inventiones Mathematicae} \textbf{198} (2014), 269--504.

\bibitem{BrunettiFredenhagenVerch2003}
R.~Brunetti, K.~Fredenhagen, and R.~Verch.
The generally covariant locality principle: A new paradigm for local quantum
physics.
\emph{Communications in Mathematical Physics} \textbf{237} (2003), 31--68.

\bibitem{Radzikowski1996}
M.~J. Radzikowski.
Micro-local approach to the Hadamard condition in quantum field theory on
curved space-time.
\emph{Communications in Mathematical Physics} \textbf{179} (1996), 529--553.

\bibitem{BrunettiFredenhagenKoehler1996}
R.~Brunetti, K.~Fredenhagen, and M.~Koehler.
The microlocal spectrum condition and Wick polynomials of free fields on
curved spacetimes.
\emph{Communications in Mathematical Physics} \textbf{180} (1996), 633--652.
arXiv:gr-qc/9510056.

\bibitem{JunkerSchrohe2002AdiabaticVacua}
W.~Junker and E.~Schrohe.
Adiabatic vacuum states on general spacetime manifolds: Definition,
construction, and physical properties.
\emph{Annales Henri Poincare} \textbf{3} (2002), 1113--1182.
arXiv:math-ph/0109010.
doi:10.1007/s000230200001.

\bibitem{Hawking1975ParticleCreation}
S.~W. Hawking.
Particle creation by black holes.
\emph{Communications in Mathematical Physics} \textbf{43} (1975), 199--220.

\bibitem{FredenhagenHaag1990HawkingRadiation}
K.~Fredenhagen and R.~Haag.
On the derivation of Hawking radiation associated with the formation of a
black hole.
\emph{Communications in Mathematical Physics} \textbf{127} (1990), 273--284.

\bibitem{BardeenCarterHawking1973FourLaws}
J.~M. Bardeen, B.~Carter, and S.~W. Hawking.
The four laws of black hole mechanics.
\emph{Communications in Mathematical Physics} \textbf{31} (1973), 161--170.

\bibitem{KayWald1991}
B.~S. Kay and R.~M. Wald.
Theorems on the uniqueness and thermal properties of stationary, nonsingular,
quasifree states on spacetimes with a bifurcate Killing horizon.
\emph{Physics Reports} \textbf{207} (1991), 49--136.

\bibitem{Segal2004}
G.~Segal.
The definition of conformal field theory.
In U.~Tillmann (ed.),
\emph{Topology, Geometry and Quantum Field Theory},
London Mathematical Society Lecture Note Series \textbf{308},
Cambridge University Press, 2004, 421--577.

\bibitem{KontsevichSegal2021}
M.~Kontsevich and G.~Segal.
Wick rotation and the positivity of energy in quantum field theory.
\emph{Quarterly Journal of Mathematics} \textbf{72} (2021), 673--699.

\bibitem{Zhu1996}
Y.~Zhu.
Modular invariance of characters of vertex operator algebras.
\emph{Journal of the American Mathematical Society} \textbf{9} (1996),
237--302.

\bibitem{Huang2005}
Y.-Z. Huang.
Vertex operator algebras and the Verlinde conjecture.
\emph{Communications in Contemporary Mathematics} \textbf{10} (2008),
103--154.

\bibitem{FriedanQiuShenker1984}
D.~Friedan, Z.~Qiu, and S.~Shenker.
Conformal invariance, unitarity, and critical exponents in two dimensions.
\emph{Physical Review Letters} \textbf{52} (1984), 1575--1578.

\bibitem{GoddardKentOlive1986}
P.~Goddard, A.~Kent, and D.~Olive.
Unitary representations of the Virasoro and super-Virasoro algebras.
\emph{Communications in Mathematical Physics} \textbf{103} (1986), 105--119.

\bibitem{CurciPaffuti1987}
G.~Curci and G.~Paffuti.
Consistency between the string background field equation of motion and the
vanishing of the conformal anomaly.
\emph{Nuclear Physics B} \textbf{286} (1987), 399--408.

\bibitem{SchnittgerEllwanger1992}
J.~Schnittger and U.~Ellwanger.
Nonperturbative conditions for local Weyl invariance on a curved world sheet.
arXiv:hep-th/9211139.

\bibitem{DashenHasslacherNeveu1975}
R.~F. Dashen, B.~Hasslacher, and A.~Neveu.
Particle spectrum in model field theories from semiclassical functional
integral techniques.
\emph{Physical Review D} \textbf{11} (1975), 3424--3450.

\bibitem{FaddeevKorepin1978}
L.~D. Faddeev and V.~E. Korepin.
Quantum theory of solitons.
\emph{Physics Reports} \textbf{42} (1978), 1--87.

\bibitem{KawahigashiLongoMuger2001}
Y.~Kawahigashi, R.~Longo, and M.~M\"uger.
Multi-interval subfactors and modularity of representations in conformal
field theory.
\emph{Communications in Mathematical Physics} \textbf{219} (2001), 631--669.

\bibitem{CarpiKawahigashiLongoWeiner2018}
S.~Carpi, Y.~Kawahigashi, R.~Longo, and M.~Weiner.
From vertex operator algebras to conformal nets and back.
\emph{Memoirs of the American Mathematical Society} \textbf{254} (2018),
no.~1213.

\bibitem{BinghamGoldieTeugels1987}
N.~H. Bingham, C.~M. Goldie, and J.~L. Teugels.
\emph{Regular Variation}.
Encyclopedia of Mathematics and its Applications \textbf{27},
Cambridge University Press, 1987.

\bibitem{DKRV2016}
F.~David, A.~Kupiainen, R.~Rhodes, and V.~Vargas.
Liouville quantum gravity on the Riemann sphere.
\emph{Communications in Mathematical Physics} \textbf{342} (2016),
869--907.

\bibitem{KRV2020}
A.~Kupiainen, R.~Rhodes, and V.~Vargas.
Integrability of Liouville theory: proof of the DOZZ formula.
\emph{Annals of Mathematics} \textbf{191} (2020), 81--166.

\bibitem{GKRV2024}
C.~Guillarmou, A.~Kupiainen, R.~Rhodes, and V.~Vargas.
Conformal bootstrap in Liouville theory.
\emph{Acta Mathematica} \textbf{233} (2024), no.~1, 33--194.

\bibitem{Zamolodchikov1984}
Al.~B. Zamolodchikov.
Conformal symmetry in two dimensions: an explicit recurrence formula for the
conformal partial wave amplitude.
\emph{Communications in Mathematical Physics} \textbf{96} (1984), 419--422.

\bibitem{Zamolodchikov1987}
Al.~B. Zamolodchikov.
Conformal symmetry in two-dimensional space: recursion representation of
conformal block.
\emph{Theoretical and Mathematical Physics} \textbf{73} (1987), 1088--1093.

\bibitem{DO1994}
H.~Dorn and H.-J. Otto.
Two- and three-point functions in Liouville theory.
\emph{Nuclear Physics B} \textbf{429} (1994), 375--388.

\bibitem{ZZ1996}
A.~B. Zamolodchikov and Al.~B. Zamolodchikov.
Structure constants and conformal bootstrap in Liouville field theory.
\emph{Nuclear Physics B} \textbf{477} (1996), 577--605.

\bibitem{FZZ2000}
V.~Fateev, A.~Zamolodchikov, and Al.~Zamolodchikov.
Boundary Liouville field theory I: boundary state and boundary two-point
function.
\emph{arXiv:hep-th/0001012}, 2000.

\bibitem{Teschner2000}
J.~Teschner.
Remarks on Liouville theory with boundary.
\emph{Proceedings of Science} \textbf{tmr2000} (2000), 041.

\bibitem{AngRemySun2025}
M.~Ang, G.~Remy, and X.~Sun.
FZZ formula of boundary Liouville CFT via conformal welding.
\emph{Journal of the European Mathematical Society} \textbf{27} (2025),
1209--1266.

\bibitem{AdlerBardeen1969}
S.~L. Adler and W.~A. Bardeen.
Absence of higher-order corrections in the anomalous axial-vector divergence
equation.
\emph{Physical Review} \textbf{182} (1969), 1517--1536.
doi:10.1103/PhysRev.182.1517.

\bibitem{Bardeen1969AnomalousWard}
W.~A. Bardeen.
Anomalous Ward identities in spinor field theories.
\emph{Physical Review} \textbf{184} (1969), 1848--1859.
doi:10.1103/PhysRev.184.1848.

\bibitem{PiguetSorella1995}
O.~Piguet and S.~P. Sorella.
\emph{Algebraic Renormalization: Perturbative Renormalization, Symmetries and
Anomalies}.
Lecture Notes in Physics Monographs, m28.
Springer, 1995.
ISBN 978-3-540-59115-3.

\bibitem{BarnichBrandtHenneaux2000}
G.~Barnich, F.~Brandt, and M.~Henneaux.
Local BRST cohomology in gauge theories.
\emph{Physics Reports} \textbf{338} (2000), 439--569.
arXiv:hep-th/0002245.
doi:10.1016/S0370-1573(00)00049-1.

\bibitem{Anselmi2014AdlerBardeen}
D.~Anselmi.
Adler--Bardeen theorem and manifest anomaly cancellation to all orders in
gauge theories.
\emph{European Physical Journal C} \textbf{74} (2014), 3083.
arXiv:1402.6453.
doi:10.1140/epjc/s10052-014-3083-0.

\bibitem{Seeley1967}
R.~T. Seeley.
Complex powers of an elliptic operator.
In \emph{Singular Integrals}, Proceedings of Symposia in Pure Mathematics
\textbf{10}, American Mathematical Society, 1967, 288--307.

\bibitem{Gilkey1984}
P.~B. Gilkey.
\emph{Invariance Theory, the Heat Equation, and the Atiyah--Singer Index
Theorem}.
Publish or Perish, 1984.

\bibitem{AtiyahSinger1968}
M.~F. Atiyah and I.~M. Singer.
The index of elliptic operators I.
\emph{Annals of Mathematics} \textbf{87} (1968), 484--530.

\bibitem{AtiyahBott1984MomentMap}
M.~F. Atiyah and R.~Bott.
The moment map and equivariant cohomology.
\emph{Topology} \textbf{23} (1984), 1--28.

\bibitem{BerlineVergne1982Localization}
N.~Berline and M.~Vergne.
Classes caracteristiques equivariantes. Formule de localisation en
cohomologie equivariante.
\emph{Comptes Rendus de l'Academie des Sciences de Paris, Serie I}
\textbf{295} (1982), 539--541.

\bibitem{APS1975I}
M.~F. Atiyah, V.~K. Patodi, and I.~M. Singer.
Spectral asymmetry and Riemannian geometry I.
\emph{Mathematical Proceedings of the Cambridge Philosophical Society}
\textbf{77} (1975), 43--69.

\bibitem{APS1975II}
M.~F. Atiyah, V.~K. Patodi, and I.~M. Singer.
Spectral asymmetry and Riemannian geometry II.
\emph{Mathematical Proceedings of the Cambridge Philosophical Society}
\textbf{78} (1975), 405--432.

\bibitem{APS1976III}
M.~F. Atiyah, V.~K. Patodi, and I.~M. Singer.
Spectral asymmetry and Riemannian geometry III.
\emph{Mathematical Proceedings of the Cambridge Philosophical Society}
\textbf{79} (1976), 71--99.

\bibitem{BismutFreed1986}
J.-M. Bismut and D.~S. Freed.
The analysis of elliptic families II: Dirac operators, eta invariants, and
the holonomy theorem.
\emph{Communications in Mathematical Physics} \textbf{107} (1986), 103--163.

\bibitem{DaiFreed1994}
X.~Dai and D.~S. Freed.
Eta invariants and determinant lines.
\emph{Journal of Mathematical Physics} \textbf{35} (1994), 5155--5194.

\bibitem{Freed1995DeterminantRevisited}
D.~S. Freed.
Determinant line bundles revisited.
\emph{arXiv:dg-ga/9505002}.

\bibitem{Witten1982}
E.~Witten.
An \(SU(2)\) anomaly.
\emph{Physics Letters B} \textbf{117} (1982), 324--328.

\bibitem{tHooft1976Pseudoparticle}
G.~'t Hooft.
Computation of the quantum effects due to a four-dimensional pseudoparticle.
\emph{Physical Review D} \textbf{14} (1976), 3432--3450.

\bibitem{LeutwylerSmilga1992}
H.~Leutwyler and A.~Smilga.
Spectrum of Dirac operator and role of winding number in QCD.
\emph{Physical Review D} \textbf{46} (1992), 5607--5632.

\bibitem{Bernard1979InstantonDeterminants}
C.~W. Bernard.
Gauge zero modes, instanton determinants, and quantum-chromodynamic calculations.
\emph{Physical Review D} \textbf{19} (1979), 3013--3019.

\bibitem{DiakonovPetrov1986InstantonVacuum}
D.~Diakonov and V.~Yu. Petrov.
A theory of light quarks in the instanton vacuum.
\emph{Nuclear Physics B} \textbf{272} (1986), 457--489.

\bibitem{BelitskyHoheneggerKorchemskySokatchevZhiboedov2013EventShapesN4}
A.~V. Belitsky, S.~Hohenegger, G.~P. Korchemsky, E.~Sokatchev, and
A.~Zhiboedov.
Event shapes in \(\mathcal N=4\) super-Yang--Mills theory.
arXiv:1309.1424.

\bibitem{BelitskyHoheneggerKorchemskySokatchevZhiboedov2013EECN4}
A.~V. Belitsky, S.~Hohenegger, G.~P. Korchemsky, E.~Sokatchev, and
A.~Zhiboedov.
Energy-energy correlations in \(\mathcal N=4\) SYM.
arXiv:1311.6800.

\bibitem{HennSokatchevYanZhiboedov2019EECNNLO}
J.~M. Henn, E.~Sokatchev, K.~Yan, and A.~Zhiboedov.
Energy-energy correlations at next-to-next-to-leading order.
arXiv:1903.05314.

\bibitem{KologluKravchukSimmonsDuffinZhiboedov2019LightRayOPE}
M.~Kologlu, P.~Kravchuk, D.~Simmons-Duffin, and A.~Zhiboedov.
The light-ray OPE and conformal colliders.
arXiv:1905.01311.

\bibitem{DixonMoultZhu2019CollinearEEC}
L.~J. Dixon, I.~Moult, and H.~X. Zhu.
The collinear limit of the energy-energy correlator.
arXiv:1905.01310.

\bibitem{EbertMistlbergerVita2020EECBackToBackN3LO}
M.~A. Ebert, B.~Mistlberger, and G.~Vita.
The energy-energy correlation in the back-to-back limit at
\(N^3\)LO and \(N^3\)LL\({}'\).
arXiv:2012.07859.

\bibitem{JaarsmaLiMoultWaalewijnZhu2023TrackEEC}
M.~Jaarsma, Y.~Li, I.~Moult, W.~J. Waalewijn, and H.~X. Zhu.
Energy correlators on tracks: resummation and non-perturbative effects.
arXiv:2307.15739.

\bibitem{BauerFlemingLuke2000SCET}
C.~W. Bauer, S.~Fleming, and M.~E. Luke.
Summing Sudakov logarithms in \(B\to X_s\gamma\) in effective field theory.
\emph{Physical Review D} \textbf{63} (2000), 014006.
arXiv:hep-ph/0005275.

\bibitem{BauerFlemingPirjolStewart2001SCET}
C.~W. Bauer, S.~Fleming, D.~Pirjol, and I.~W. Stewart.
An effective field theory for collinear and soft gluons: heavy to light decays.
\emph{Physical Review D} \textbf{63} (2001), 114020.
arXiv:hep-ph/0011336.

\bibitem{CollinsSoperSterman1989Factorization}
J.~C. Collins, D.~E. Soper, and G.~Sterman.
Factorization of hard processes in QCD.
\emph{Advanced Series on Directions in High Energy Physics} \textbf{5}
(1989), 1--91.
arXiv:hep-ph/0409313.

\bibitem{RogersMulders2010NoGeneralizedTMD}
T.~C. Rogers and P.~J. Mulders.
No generalized TMD-factorization in the hadro-production of high transverse
momentum hadrons.
\emph{Physical Review D} \textbf{81} (2010), 094006.
arXiv:1001.2977.

\bibitem{DasguptaSalam2001NonGlobal}
M.~Dasgupta and G.~P. Salam.
Resummation of non-global QCD observables.
\emph{Physics Letters B} \textbf{512} (2001), 323--330.
arXiv:hep-ph/0104277.

\bibitem{BanfiMarchesiniSmye2002NonGlobal}
A.~Banfi, G.~Marchesini, and G.~Smye.
Away-from-jet energy flow.
\emph{Journal of High Energy Physics} \textbf{08} (2002), 006.
arXiv:hep-ph/0206076.

\bibitem{LarkoskiMarzaniSoyezThaler2014SoftDrop}
A.~J. Larkoski, S.~Marzani, G.~Soyez, and J.~Thaler.
Soft drop.
\emph{Journal of High Energy Physics} \textbf{05} (2014), 146.
arXiv:1402.2657.

\bibitem{BrodskyLepage1980Exclusive}
S.~J. Brodsky and G.~P. Lepage.
Exclusive processes in perturbative quantum chromodynamics.
\emph{Physical Review D} \textbf{22} (1980), 2157--2198.

\bibitem{EfremovRadyushkin1980PionFormFactor}
A.~V. Efremov and A.~V. Radyushkin.
Factorization and asymptotical behavior of pion form factor in QCD.
\emph{Physics Letters B} \textbf{94} (1980), 245--250.

\bibitem{KuraevFadinLipatov1977}
E.~A. Kuraev, L.~N. Lipatov, and V.~S. Fadin.
The Pomeranchuk singularity in nonabelian gauge theories.
\emph{Soviet Physics JETP} \textbf{45} (1977), 199--204.

\bibitem{BalitskyLipatov1978}
I.~I. Balitsky and L.~N. Lipatov.
The Pomeranchuk singularity in quantum chromodynamics.
\emph{Soviet Journal of Nuclear Physics} \textbf{28} (1978), 822--829.

\bibitem{NikolaevZakharov1991}
N.~N. Nikolaev and B.~G. Zakharov.
Colour transparency and scaling properties of nuclear shadowing in deep
inelastic scattering.
\emph{Zeitschrift f\"ur Physik C} \textbf{49} (1991), 607--618.

\bibitem{Mueller1994SoftGluons}
A.~H. Mueller.
Soft gluons in the infinite-momentum wave function and the BFKL pomeron.
\emph{Nuclear Physics B} \textbf{415} (1994), 373--385.

\bibitem{Balitsky1996HighEnergy}
I.~Balitsky.
Operator expansion for high-energy scattering.
\emph{Nuclear Physics B} \textbf{463} (1996), 99--160.

\bibitem{JalilianMarianKovnerLeonidovWeigert1998HighDensity}
J.~Jalilian-Marian, A.~Kovner, A.~Leonidov, and H.~Weigert.
The Wilson renormalization group for low \(x\) physics: towards the high
density regime.
\emph{Physical Review D} \textbf{59} (1998), 014014.

\bibitem{JalilianMarianKovnerWeigert1998FiniteDensity}
J.~Jalilian-Marian, A.~Kovner, and H.~Weigert.
The Wilson renormalization group for low \(x\) physics: gluon evolution at
finite parton density.
\emph{Physical Review D} \textbf{59} (1998), 014015.

\bibitem{Kovchegov1999SmallXF2}
Y.~V. Kovchegov.
Small-\(x\) \(F_2\) structure function of a nucleus including multiple pomeron
exchanges.
\emph{Physical Review D} \textbf{60} (1999), 034008.

\bibitem{IancuLeonidovMcLerran2001CGC}
E.~Iancu, A.~Leonidov, and L.~D. McLerran.
The renormalization group equation for the color glass condensate.
\emph{Physics Letters B} \textbf{510} (2001), 133--144.

\bibitem{Weigert2002Unitarity}
H.~Weigert.
Unitarity at small Bjorken \(x\).
\emph{Nuclear Physics A} \textbf{703} (2002), 823--860.

\bibitem{SeibergWitten1994I}
N.~Seiberg and E.~Witten.
Electric-magnetic duality, monopole condensation, and confinement in
\(\mathcal N=2\) supersymmetric Yang--Mills theory.
\emph{Nuclear Physics B} \textbf{426} (1994), 19--52.

\bibitem{SeibergWitten1994II}
N.~Seiberg and E.~Witten.
Monopoles, duality and chiral symmetry breaking in
\(\mathcal N=2\) supersymmetric QCD.
\emph{Nuclear Physics B} \textbf{431} (1994), 484--550.

\bibitem{Witten1994MonopolesFourManifolds}
E.~Witten.
Monopoles and four-manifolds.
\emph{Mathematical Research Letters} \textbf{1} (1994), 769--796.
arXiv:hep-th/9411102.

\bibitem{FeehanLeness2002SO3MonopoleCobordism}
P.~M.~N. Feehan and T.~G. Leness.
An \(SO(3)\)-monopole cobordism formula relating Donaldson and
Seiberg-Witten invariants.
arXiv:math/0203047.

\bibitem{FeehanLeness2015WittenConjecture}
{\raggedright
P.~M.~N. Feehan and T.~G. Leness.
Witten's conjecture for many four-manifolds of simple type.
\emph{J. Eur. Math. Soc.} \textbf{17} (2015), 899--923.
arXiv:math/0609530.
\par}

\bibitem{Seiberg1994}
N.~Seiberg.
Electric-magnetic duality in supersymmetric non-Abelian gauge theories.
\emph{Nuclear Physics B} \textbf{435} (1995), 129--146.

\bibitem{BraatenNieto1995EFTThermo}
E.~Braaten and A.~Nieto.
Effective field theory approach to high-temperature thermodynamics.
\emph{Physical Review D} \textbf{51} (1995), 6990--7006.
arXiv:hep-ph/9501375.

\bibitem{KajantieEtAl1996DimensionalReduction}
K.~Kajantie, M.~Laine, K.~Rummukainen, and M.~Shaposhnikov.
Generic rules for high temperature dimensional reduction and their application
to the Standard Model.
\emph{Nuclear Physics B} \textbf{458} (1996), 90--136.
arXiv:hep-ph/9508379.

\bibitem{MaldacenaZhiboedov2012}
J.~Maldacena and A.~Zhiboedov.
Constraining conformal field theories with a higher spin symmetry.
\emph{Journal of Physics A} \textbf{46} (2013), 214011.

\bibitem{AGY2012}
O.~Aharony, G.~Gur-Ari, and R.~Yacoby.
Correlation functions of large \(N\) Chern--Simons--matter theories and
bosonization in three dimensions.
\emph{Journal of High Energy Physics} \textbf{12} (2012), 028.

\bibitem{JainMinwallaYokoyama2013}
S.~Jain, S.~Minwalla, and S.~Yokoyama.
Chern--Simons duality with a fundamental boson and fermion.
\emph{Journal of High Energy Physics} \textbf{11} (2013), 037.

\bibitem{AppelquistCarazzone1975}
T.~Appelquist and J.~Carazzone.
Infrared singularities and massive fields.
\emph{Physical Review D} \textbf{11} (1975), 2856--2861.

\bibitem{Weinberg1979PhenomenologicalLagrangians}
S.~Weinberg.
Phenomenological Lagrangians.
\emph{Physica A} \textbf{96} (1979), 327--340.

\bibitem{Georgi1993EffectiveFieldTheory}
H.~Georgi.
Effective field theory.
\emph{Annual Review of Nuclear and Particle Science} \textbf{43} (1993),
209--252.

\bibitem{AdamsEtAl2006IRObstruction}
A.~Adams, N.~Arkani-Hamed, S.~Dubovsky, A.~Nicolis, and R.~Rattazzi.
Causality, analyticity and an IR obstruction to UV completion.
\emph{Journal of High Energy Physics} \textbf{10} (2006), 014.
arXiv:hep-th/0602178.

\bibitem{WeinbergWitten1980}
S.~Weinberg and E.~Witten.
Limits on massless particles.
\emph{Physics Letters B} \textbf{96} (1980), 59--62.
doi:10.1016/0370-2693(80)90212-9.

\bibitem{Loebbert2008WeinbergWitten}
F.~Loebbert.
The Weinberg--Witten theorem on massless particles: an essay.
\emph{Annalen der Physik} \textbf{17} (2008), 803--829.
doi:10.1002/andp.200810305.

\bibitem{BenedettiCasiniMagan2022Noether}
V.~Benedetti, H.~Casini, and J.~M. Magan.
Generalized symmetries and Noether's theorem in QFT.
\emph{Journal of High Energy Physics} \textbf{08} (2022), 304.
arXiv:2205.03412.
doi:10.1007/JHEP08(2022)304.

\bibitem{Polchinski1992FermiSurface}
J.~Polchinski.
Effective field theory and the Fermi surface.
In \emph{Proceedings of TASI 1992}, arXiv:hep-th/9210046.

\bibitem{Shankar1994RGFermiLiquid}
R.~Shankar.
Renormalization-group approach to interacting fermions.
\emph{Reviews of Modern Physics} \textbf{66} (1994), 129--192.

\bibitem{LuttingerWard1960}
J.~M. Luttinger and J.~C. Ward.
Ground-state energy of a many-fermion system. II.
\emph{Physical Review} \textbf{118} (1960), 1417--1427.

\bibitem{Luttinger1960}
J.~M. Luttinger.
Fermi surface and some simple equilibrium properties of a system of interacting
fermions.
\emph{Physical Review} \textbf{119} (1960), 1153--1163.

\bibitem{Oshikawa2000Luttinger}
M.~Oshikawa.
Topological approach to Luttinger's theorem and the Fermi surface of a Kondo
lattice.
\emph{Physical Review Letters} \textbf{84} (2000), 3370--3373.

\bibitem{LiebSchultzMattis1961}
E.~H. Lieb, T.~Schultz, and D.~C. Mattis.
Two soluble models of an antiferromagnetic chain.
\emph{Annals of Physics} \textbf{16} (1961), 407--466.
doi:10.1016/0003-4916(61)90115-4.

\bibitem{Oshikawa2000Commensurability}
M.~Oshikawa.
Commensurability, excitation gap, and topology in quantum many-particle systems
on a periodic lattice.
\emph{Physical Review Letters} \textbf{84} (2000), 1535--1538.
doi:10.1103/PhysRevLett.84.1535.

\bibitem{BachmannMichalakisNachtergaeleSims2012Automorphic}
S.~Bachmann, S.~Michalakis, B.~Nachtergaele, and R.~Sims.
Automorphic equivalence within gapped phases of quantum lattice systems.
\emph{Communications in Mathematical Physics} \textbf{309} (2012), 835--871.
arXiv:1102.0842.
doi:10.1007/s00220-011-1380-0.

\bibitem{Hastings2004LSMHigher}
M.~B. Hastings.
Lieb--Schultz--Mattis in higher dimensions.
\emph{Physical Review B} \textbf{69} (2004), 104431.
arXiv:cond-mat/0305505.
doi:10.1103/PhysRevB.69.104431.

\bibitem{MetlitskiThorngren2018Anomalies}
M.~A. Metlitski and R.~Thorngren.
Intrinsic and emergent anomalies at deconfined critical points.
\emph{Physical Review B} \textbf{98} (2018), 085140.
arXiv:1707.07686.
doi:10.1103/PhysRevB.98.085140.

\bibitem{ElseThorngren2020LSMTopological}
D.~V. Else and R.~Thorngren.
Topological theory of Lieb--Schultz--Mattis theorems in quantum spin systems.
\emph{Physical Review B} \textbf{101} (2020), 224437.
arXiv:1907.08204.
doi:10.1103/PhysRevB.101.224437.

\bibitem{ChengSeiberg2023LSMLuttingerAnomaly}
M.~Cheng and N.~Seiberg.
Lieb--Schultz--Mattis, Luttinger, and 't Hooft: anomaly matching in lattice
systems.
\emph{SciPost Physics} \textbf{15} (2023), 051.
arXiv:2211.12543.
doi:10.21468/SciPostPhys.15.2.051.

\bibitem{OsterwalderSeiler1978GaugeLattice}
K.~Osterwalder and E.~Seiler.
Gauge field theories on a lattice.
\emph{Annals of Physics} \textbf{110} (1978), 440--471.
doi:10.1016/0003-4916(78)90039-8.

\bibitem{FradkinShenker1979GaugeHiggs}
E.~Fradkin and S.~H. Shenker.
Phase diagrams of lattice gauge theories with Higgs fields.
\emph{Physical Review D} \textbf{19} (1979), 3682--3697.
doi:10.1103/PhysRevD.19.3682.

\bibitem{FredenhagenMarcu1983ChargedZ2}
K.~Fredenhagen and M.~Marcu.
Charged states in \(\mathbb Z_2\) gauge theories.
\emph{Communications in Mathematical Physics} \textbf{92} (1983), 81--119.
doi:10.1007/BF01206315.

\bibitem{CaudyGreensite2008GaugeAmbiguity}
W.~Caudy and J.~Greensite.
On the ambiguity of spontaneously broken gauge symmetry.
\emph{Physical Review D} \textbf{78} (2008), 025018.
arXiv:0712.0999.
doi:10.1103/PhysRevD.78.025018.

\bibitem{Neuberger1986NonperturbativeBRS}
H.~Neuberger.
Nonperturbative BRS invariance.
\emph{Physics Letters B} \textbf{175} (1986), 69--72.
doi:10.1016/0370-2693(86)90333-3.

\bibitem{Neuberger1987BRSGribov}
H.~Neuberger.
Nonperturbative BRS invariance and the Gribov problem.
\emph{Physics Letters B} \textbf{183} (1987), 337--340.
doi:10.1016/0370-2693(87)90974-9.

\bibitem{HerrlichNierste1995Evanescent}
S.~Herrlich and U.~Nierste.
Evanescent operators, scheme dependences and double insertions.
\emph{Nuclear Physics B} \textbf{455} (1995), 39--58.
arXiv:hep-ph/9412375.

\bibitem{FuentesMartinEtAl2023EvanescentMatching}
J.~Fuentes-Mart\'in, M.~K\"onig, J.~Pag\`es, A.~Eller Thomsen, and
F.~Wilsch.
Evanescent operators in one-loop matching computations.
\emph{Journal of High Energy Physics} \textbf{02} (2023), 031.
arXiv:2211.09144.

\bibitem{MTV2006BetheEigenvectors}
E.~Mukhin, V.~Tarasov, and A.~Varchenko.
Bethe eigenvectors of higher transfer matrices.
\emph{Journal of Statistical Mechanics: Theory and Experiment} \textbf{2006},
no.~8, P08002.

\bibitem{MTV2009XXXSimpleSpectrum}
E.~Mukhin, V.~Tarasov, and A.~Varchenko.
Bethe algebra of homogeneous XXX Heisenberg model has simple spectrum.
\emph{Communications in Mathematical Physics} \textbf{288} (2009), 1--42.

\bibitem{Tarasov2018Completeness}
V.~Tarasov.
Completeness of the Bethe ansatz for the periodic isotropic Heisenberg model.
\emph{Reviews in Mathematical Physics} \textbf{30} (2018), 1840018.

\bibitem{LiebRobinson1972}
E.~H. Lieb and D.~W. Robinson.
The finite group velocity of quantum spin systems.
\emph{Communications in Mathematical Physics} \textbf{28} (1972), 251--257.

\bibitem{NachtergaeleSims2006LiebRobinsonClustering}
B.~Nachtergaele and R.~Sims.
Lieb--Robinson bounds and the exponential clustering theorem.
\emph{Communications in Mathematical Physics} \textbf{265} (2006), 119--130.
arXiv:math-ph/0506030.
doi:10.1007/s00220-006-1556-1.

\bibitem{NachtergaeleSimsYoung2019QuasiLocality}
B.~Nachtergaele, R.~Sims, and A.~Young.
Quasi-locality bounds for quantum lattice systems. Part I. Lieb--Robinson
bounds, quasi-local maps, and spectral flow automorphisms.
\emph{Journal of Mathematical Physics} \textbf{60} (2019), 061101.
arXiv:1810.02428.
doi:10.1063/1.5095769.

\bibitem{BDDK1994Unitarity}
Z.~Bern, L.~Dixon, D.~C. Dunbar, and D.~A. Kosower.
One-loop \(n\)-point gauge theory amplitudes, unitarity and collinear limits.
\emph{Nuclear Physics B} \textbf{425} (1994), 217--260.
arXiv:hep-ph/9403226.
doi:10.1016/0550-3213(94)90179-1.

\bibitem{BDK1996OneLoopQCD}
Z.~Bern, L.~Dixon, and D.~A. Kosower.
Progress in one-loop QCD computations.
\emph{Annual Review of Nuclear and Particle Science} \textbf{46} (1996),
109--148.
arXiv:hep-ph/9602280.
doi:10.1146/annurev.nucl.46.1.109.

\bibitem{Catani1998TwoLoopIR}
S.~Catani.
The singular behaviour of QCD amplitudes at two-loop order.
\emph{Physics Letters B} \textbf{427} (1998), 161--171.
arXiv:hep-ph/9802439.
doi:10.1016/S0370-2693(98)00332-3.

\bibitem{BrittoCachazoFeng2005GeneralizedUnitarity}
R.~Britto, F.~Cachazo, and B.~Feng.
Generalized unitarity and one-loop amplitudes in \(N=4\) super-Yang--Mills.
\emph{Nuclear Physics B} \textbf{725} (2005), 275--305.
arXiv:hep-th/0412103.
doi:10.1016/j.nuclphysb.2005.07.014.

\bibitem{OPP2007IntegrandReduction}
G.~Ossola, C.~G. Papadopoulos, and R.~Pittau.
Reducing full one-loop amplitudes to scalar integrals at the integrand level.
\emph{Nuclear Physics B} \textbf{763} (2007), 147--169.
arXiv:hep-ph/0609007.
doi:10.1016/j.nuclphysb.2006.11.012.

\bibitem{Forde2007OneLoopCoefficients}
D.~Forde.
Direct extraction of one-loop integral coefficients.
\emph{Physical Review D} \textbf{75} (2007), 125019.
arXiv:0704.1835.
doi:10.1103/PhysRevD.75.125019.

\bibitem{BernMorgan1996MassiveUnitarity}
Z.~Bern and A.~G. Morgan.
Massive loop amplitudes from unitarity.
\emph{Nuclear Physics B} \textbf{467} (1996), 479--509.
arXiv:hep-ph/9511336.
doi:10.1016/0550-3213(96)00078-8.

\bibitem{BrandhuberEtAl2005PureYMUnitarity}
A.~Brandhuber, S.~McNamara, B.~Spence, and G.~Travaglini.
Loop amplitudes in pure Yang--Mills from generalised unitarity.
\emph{Journal of High Energy Physics} \textbf{10} (2005), 011.
arXiv:hep-th/0506068.
doi:10.1088/1126-6708/2005/10/011.

\bibitem{BernCarrascoJohansson2008NewRelations}
Z.~Bern, J.~J.~M. Carrasco, and H.~Johansson.
New relations for gauge-theory amplitudes.
\emph{Physical Review D} \textbf{78} (2008), 085011.
arXiv:0805.3993.
doi:10.1103/PhysRevD.78.085011.

\bibitem{BernCarrascoJohansson2010PerturbativeQuantumGravity}
Z.~Bern, J.~J.~M. Carrasco, and H.~Johansson.
Perturbative quantum gravity as a double copy of gauge theory.
\emph{Physical Review Letters} \textbf{105} (2010), 061602.
arXiv:1004.0476.
doi:10.1103/PhysRevLett.105.061602.

\bibitem{PassarinoVeltman1979}
G.~Passarino and M.~Veltman.
One-loop corrections for \(e^+e^-\) annihilation into \(\mu^+\mu^-\) in the
Weinberg model.
\emph{Nuclear Physics B} \textbf{160} (1979), 151--207.
doi:10.1016/0550-3213(79)90234-7.

\bibitem{ChetyrkinTkachov1981IBP}
K.~G. Chetyrkin and F.~V. Tkachov.
Integration by parts: the algorithm to calculate beta-functions in 4 loops.
\emph{Nuclear Physics B} \textbf{192} (1981), 159--204.
doi:10.1016/0550-3213(81)90199-1.

\bibitem{Tkachov1981AnalyticalCalculability}
F.~V. Tkachov.
A theorem on analytical calculability of 4-loop renormalization group
functions.
\emph{Physics Letters B} \textbf{100} (1981), 65--68.
doi:10.1016/0370-2693(81)90288-4.

\bibitem{Kotikov1991DifferentialEquations}
A.~V. Kotikov.
Differential equations method: new technique for massive Feynman diagram
calculation.
\emph{Physics Letters B} \textbf{254} (1991), 158--164.
doi:10.1016/0370-2693(91)90413-K.

\bibitem{GehrmannRemiddi2000DifferentialEquations}
T.~Gehrmann and E.~Remiddi.
Differential equations for two-loop four-point functions.
\emph{Nuclear Physics B} \textbf{580} (2000), 485--518.
arXiv:hep-ph/9912329.
doi:10.1016/S0550-3213(00)00223-6.

\bibitem{GehrmannDeRidderGehrmannGlover2004TwoJet}
A.~Gehrmann-De Ridder, T.~Gehrmann, and E.~W.~N. Glover.
Infrared structure of \(e^+e^-\to 2\) jets at NNLO.
\emph{Nuclear Physics B} \textbf{691} (2004), 195--222.
arXiv:hep-ph/0403057.
doi:10.1016/j.nuclphysb.2004.05.017.

\bibitem{GehrmannDeRidderGehrmannGlover2005Antenna}
A.~Gehrmann-De Ridder, T.~Gehrmann, and E.~W.~N. Glover.
Antenna subtraction at NNLO.
\emph{Journal of High Energy Physics} \textbf{09} (2005), 056.
arXiv:hep-ph/0505111.
doi:10.1088/1126-6708/2005/09/056.

\bibitem{LaportaRemiddi2005Sunrise}
S.~Laporta and E.~Remiddi.
Analytic treatment of the two loop equal mass sunrise graph.
\emph{Nuclear Physics B} \textbf{704} (2005), 349--386.
arXiv:hep-ph/0406160.
doi:10.1016/j.nuclphysb.2004.10.044.

\bibitem{AdamsBognerWeinzierl2013Sunrise}
L.~Adams, C.~Bogner, and S.~Weinzierl.
The two-loop sunrise graph with arbitrary masses.
\emph{Journal of Mathematical Physics} \textbf{54} (2013), 052303.
arXiv:1302.7004.
doi:10.1063/1.4804996.

\bibitem{UssyukinaDavydychev1993Ladders}
N.~I. Ussyukina and A.~I. Davydychev.
Exact results for three- and four-point ladder diagrams with an arbitrary
number of rungs.
\emph{Physics Letters B} \textbf{298} (1993), 363--370.
doi:10.1016/0370-2693(93)91118-7.

\bibitem{DharKunduSaito2021MazurSuzuki}
A.~Dhar, A.~Kundu, and K.~Saito.
Revisiting the Mazur bound and the Suzuki equality.
\emph{Chaos, Solitons and Fractals} \textbf{144} (2021), 110618.
arXiv:2007.04562.

\bibitem{RigolShastry2008OpenDrude}
M.~Rigol and B.~S. Shastry.
Drude weight in systems with open boundary conditions.
\emph{Physical Review B} \textbf{77} (2008), 161101(R).
arXiv:0801.0592.
doi:10.1103/PhysRevB.77.161101.

\bibitem{BellomiaResta2020OpenDrude}
G.~Bellomia and R.~Resta.
Drude weight in systems with open boundary conditions.
\emph{Physical Review B} \textbf{102} (2020), 205123.
arXiv:2007.04143.
doi:10.1103/PhysRevB.102.205123.

\bibitem{IlievskiProsen2013DrudeBounds}
E.~Ilievski and T.~Prosen.
Thermodynamic bounds on Drude weights in terms of almost-conserved
quantities.
\emph{Communications in Mathematical Physics} \textbf{318} (2013), 809--830.
arXiv:1111.3830.

\end{thebibliography}
