...
| pp. 80--99, Green functions, Kallen--Lehmann representation, particle content, \(Z\) | mapped | `volume_i/chapter09_kallen_lehmann_spectral_representation_and_particle_content.tex` | This must remain early; check positivity and one-particle pole normalization. |
| pp. 100--112, \(\phi^4\) perturbation theory for Green functions | mapped | `volume_i/chapter10_perturbative_green_functions_and_feynman_graphs.tex`; `volume_i/chapter11_lorentzian_green_functions_and_analytic_continuation.tex` | Verify that these are Green-function diagrams, not premature S-matrix diagrams. |
...
| pp. 239--248, chiral symmetry breaking and pion EFT | mapped | `volume_ii/chapter21_global_anomalies_spontaneous_symmetry_breaking_and_pions.tex` | Check \(SU(N_f)_L\times SU(N_f)_R/SU(N_f)_V\), WZW normalization, and \(N_f=2\) coordinates. |
| pp. 249--257, chiral perturbation theory, pion scattering, masses, \(\pi^0\to2\gamma\) | mapped | `volume_ii/chapter21_global_anomalies_spontaneous_symmetry_breaking_and_pions.tex` | Pion scattering and chiral-log structure were patched in this audit pass; still needs PDF figure-level check before certification. |
