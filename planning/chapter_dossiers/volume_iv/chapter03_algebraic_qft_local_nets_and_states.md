...
- `\mathcal R_{\rm coord}(\mathcal O)`: coordinate-generated subnet of a
  represented observable net, generated by spectral projections of affiliated
  field coordinates.
...
- Defines complete observable Wightman coordinatization as the additional
  datum needed before field exponentials can be said to generate the
  represented observable net: agreement of the symmetry/vacuum representation,
  operator-theoretic additivity through closure and Trotter functional
  calculus, equality of the generated observable-coordinate net with
  \(\mathcal R(\mathcal O)\), or equality after a specified fixed-point or
  gauge-invariant observable operation.
- Adds the bounded orbit criterion for equality of represented local
  von Neumann algebras: if \(\Omega\) is separating for
  \(\mathcal M\vee\mathcal N\) and every \(A\in\mathcal M\) has uniformly
  bounded approximants \(B_i\in\mathcal N\) with \(B_i\Omega\to A\Omega\),
  then \(A\in\mathcal N\).  This makes clear why cyclic Wightman matrix
  elements alone do not identify local algebras.
- Separates affiliation from generation in the converse comparison: affiliated
  field coordinates give a coordinate-generated subnet
  \(\mathcal R_{\rm coord}(\mathcal O)\subset\mathcal R(\mathcal O)\), and
  equality with the full represented local algebra is an additional
  coordinatization-completeness hypothesis.
...
- `calculation-checks/wightman_net_bridge_checks.py`: exact rational
  ...
  proper spectral subalgebra unless a second coordinate refines the spectral
  partition.  It also checks the finite additivity/covariance shadow: a
  noncommuting sum \(3Z+4X\) has exact rational spectral projections inside
  the algebra generated by \(Z\) and \(X\), and symmetry conjugation sends
  those projections to the transformed summed generator.  It also checks the
  finite orbit-equality boundary: separating orbit equality detects operator
  equality inside the generated diagonal algebra, while a nonseparating vector
  lets a proper diagonal operator and a scalar operator have the same orbit
  vector.
...
- 2026-06-01 generated-additivity bridge pass: inserted the finite-cover
  ...
  conjugation.
- 2026-06-02 affiliated-coordinate generation pass: inserted the converse
  comparison that affiliated unbounded fields define only a
  coordinate-generated subnet until a generation/completeness theorem is
...
- 2026-06-02 complete observable coordinatization pass: added
  Definition `def:complete-observable-wightman-coordinatization`, tying the
  Wightman-to-net comparison datum to equality with a represented observable
  net only after bounded additivity, equality of von Neumann algebras, and the
  appropriate observable-subnet operation have been verified.
...
- 2026-06-02 bounded-orbit algebra-equality pass: inserted the bounded orbit
  criterion immediately before the Wightman-to-net comparison structure.
  This pass prevents equality of cyclic Wightman data, or equality of a dense
  set of cyclic vectors, from being misread as equality of represented local
  von Neumann algebras; the finite companion check records both the separating
  mechanism and the nonseparating failure mode.
