...
- Role in the monograph: derive the Yang--Baxter consistency condition from
...
1. The Yang--Baxter equation is derived from equality of the two reduced
   three-particle scattering decompositions.
2. Internal symmetry places \(R_{ab}(\theta)\) in
   \(\operatorname{End}_G(V_a\otimes V_b)\).
3. Multiplicity-free tensor-product channels reduce the matrix equation to
   scalar meromorphic functions constrained by projectors and the
   Yang--Baxter equation.
4. The \(R\)-matrix datum is on-shell data and does not construct local
   algebras by itself.
