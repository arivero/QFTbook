...
slow-variable data beyond Model A/B by separating dissipative mobility,
reversible Gibbs-preserving drift, exact conserved covectors, and the
additional scaling hypotheses needed before Hohenberg--Halperin labels become
continuum claims.  It also derives Doi--Peliti variables
...
- `y`, `R`, `mathsf J`, `c_I`: coupled slow variables, reversible drift,
  antisymmetric finite bracket matrix, and conserved covectors for dynamic
  critical data beyond Model A/B.
...
12. Coupled slow-variable data are finite Ito processes
    ...
    \(\partial_a(R^ae^{-\mathcal F/T})=0\), and exact conserved linear
...
- `calculation-checks/nonequilibrium_open_system_checks.py` verifies the
  ...
  current.  It also verifies coupled slow-variable data beyond Model A/B:
  antisymmetric reversible drift preserves the Gibbs density, and a Model
  C-style block mobility has a conserved-density covector killing both
  mobility and noise covariance.  It also verifies the
...
- 2026-06-01 statmech crosswalk/#703 slow-variable pass: added the finite
  coupled slow-variable datum beyond Model A/B, including reversible
