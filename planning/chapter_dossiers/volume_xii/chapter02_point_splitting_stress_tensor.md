...
- Construction: explicit scalar bidifferential operator with parallel
  ...
  improvement \(\eta_4=1/3\) that cancels the Hadamard-parametrix
  conservation defect.
...
5. Under those conditions, scalar stress-tensor renormalization has finite
   ...
   \(I_{\mu\nu}\), and \(J_{\mu\nu}\); dimension counting alone would not
   exclude nonpolynomial local functionals.
...
9. Conservation is obtained by the \(\eta_4=1/3\) equation-of-motion
   improvement that cancels the local Hadamard-parametrix defect, and is then
   preserved by conserved residual finite terms.
...
- `calculation-checks/point_splitting_stress_checks.py` verifies the flat
  ...
  massless stress-tensor computation, the equation-of-motion conservation
  repair, the regularity filter in the finite-local-freedom theorem, the
