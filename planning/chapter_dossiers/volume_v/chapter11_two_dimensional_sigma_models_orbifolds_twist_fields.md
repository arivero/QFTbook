...
11. The torsionful connection `Gamma^- = Gamma - H/2` has Ricci tensor with
...
37. For a connected genus-zero twist correlator, the active monodromies must
   ...
   are genus zero.
38. The primitive joining cover with branch indices `K,L,K+L-1` is
...
39. The local inverse-branch Schwarzian at ramification index `r` gives the
    ...
    OPE coefficient.
40. For total copy number `M`, normalized single-cycle class sums have class
