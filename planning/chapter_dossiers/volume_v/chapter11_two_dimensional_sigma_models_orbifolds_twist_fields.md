...
4. The dilaton-gradient vector \(W=\alpha'\nabla\Phi\) produces
   \(2\alpha'\nabla_i\nabla_j\Phi\) in \(\widehat\beta^G\) and the
   non-gauge \(\nabla\Phi\cdot H\) contribution in \(\widehat\beta^B\).
5. Varying `-H^2/12` with respect to the metric gives the coefficient
...
23. For a cycle type
   ...
   Hilbert spaces.
24. The symmetric-product partition-function generating series is the
...
25. Twist OPEs multiply monodromies before conjugacy-class averaging; a
...
26. Twist-field deformations are conformal perturbations by projected local
