...
| \(S_{\mathrm{sub}}\) | in/out comparison operator \(\Omega_{\mathrm{out}}^*\Omega_{\mathrm{in}}\), defined once the two wave operators are isometries |
| \(S\) | unitary closed-sector scattering operator, equal to \(S_{\mathrm{sub}}\) only after incoming and outgoing ranges coincide |
...
- The in/out comparison operator
  \(S_{\mathrm{sub}}=\Omega_{\mathrm{out}}^*\Omega_{\mathrm{in}}\) is defined
  as soon as the incoming and outgoing Haag--Ruelle wave operators are
  isometries.  It is always a contraction on the asymptotic Fock space.
- Proposition `prop:hr-comparison-contraction-range-defects` proves the
  matrix-element formula and the projection identities
  \(S_{\mathrm{sub}}^*S_{\mathrm{sub}}
  =\Omega_{\mathrm{in}}^*P_{\mathrm{out}}\Omega_{\mathrm{in}}\) and
  \(S_{\mathrm{sub}}S_{\mathrm{sub}}^*
  =\Omega_{\mathrm{out}}^*P_{\mathrm{in}}\Omega_{\mathrm{out}}\).  Equality of
  the ranges is the condition that upgrades \(S_{\mathrm{sub}}\) to a unitary
  closed-sector \(S\)-operator.
- The chapter separates three range statements: equality of incoming and
  ...
  Hilbert space.
- Later cluster, cross-section, optical-theorem, and partial-wave unitarity
  arguments require a closed massive sector for the channels under analysis,
  while full asymptotic completeness of the entire Hilbert space remains a
  separate global hypothesis.
...
- 2026-06-09 issue #969 pass: separated the always-defined in/out comparison
  operator \(S_{\mathrm{sub}}\) from the unitary closed-sector \(S\)-operator.
  The chapter now proves the contraction bound and projection-defect
  identities before stating range coincidence as the unitarity condition.
  `calculation-checks/haag_ruelle_fock_inner_product_checks.py` now includes a
  finite-dimensional noncoincident-range example.
