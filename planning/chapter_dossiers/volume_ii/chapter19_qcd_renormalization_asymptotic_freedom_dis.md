...
- Euclidean current sum rules as color-singlet current-correlator identities,
  ...
  plus a two-scale extraction window that propagates moment residuals through
  the mass quotient and rejects plateaus generated by retuning the continuum
  threshold.
...
24. QCD current sum rules are linear functionals of color-singlet current
    ...
    not a controlled QCD statement.  A quoted SVZ mass must also survive a
    two-scale window test: aggregate zeroth- and first-moment residuals are
    propagated through the quotient, and any \(s_0(\tau)\) motion used to flatten
    the curve is counted as threshold sensitivity rather than as independent
    evidence for a pole.
...
- 2026-06-06 issue #630 quasi-PDF inverse-matching pass: added
...
- 2026-06-06 issue #630 SVZ duality/stability pass: added
  ...
  remainder.
- 2026-06-06 issue #630 SVZ two-scale extraction pass: added
  `ca:qcd-svz-two-scale-extraction-window` to make the Borel-window criterion
  an extraction architecture rather than a collection of local diagnostics.
  The new block propagates aggregate OPE/duality/regulator residuals through
  the mass quotient, defines a two-direction window score in \(\tau\) and
  \(s_0\), and identifies retuned-threshold plateaus as a cancellation that
  must be counted in the threshold-sensitivity budget.
