...
| \(\Delta H,\delta_{\theta\theta},\delta_{\theta\eta},\delta_{\eta\eta}\) | residual Hessian entries measuring finite-\(N_c\), chiral, branch-selection, and local theta-counterterm errors around the rank-one Witten--Veneziano matching matrix |
| \(\chi_{\rm QCD}^{m=0,{\rm loc}}\) | local full-QCD theta curvature after minimizing the singlet field in the massless residual Witten--Veneziano window |
...
- The same matching is now expressed as the local \((\theta,\eta_0)\)
  ...
  alignment.
- The Witten--Veneziano matching is also given a residual Hessian budget:
  \(H=\chi_{\rm YM}\begin{psmallmatrix}1&a\\a&a^2\end{psmallmatrix}
  +\Delta H\).  The manuscript derives the fixed-\(\theta\) singlet mass
  shift, the full massless-QCD theta curvature as the Schur complement after
  minimizing \(\eta_0\), the expanded residual numerator, and a bound in terms
  of \(r_{\theta\theta},r_{\theta\eta},r_{\eta\eta}\).  This is local
...
- 2026-06-03 issue #630 theta/Witten--Veneziano pass: added the local
  ...
  identities.
- 2026-06-04 issue #630 residual Witten--Veneziano pass: added the
  first-residual Hessian budget around the rank-one large-\(N_c\) matching
  matrix, deriving the singlet mass shift, the full massless-QCD Schur
  curvature, the exact numerator, and a residual bound.  Extended
  `qcd_theta_witten_veneziano_checks.py` with symbolic and exact-rational
  checks of the residual formulas.
