...
- `SRC-INTERNAL`: Volume II Chapter 20 instanton section and
  ...
  projection, finite-activity dilute-gas theta cumulants, and the large-size
  obstruction that prevents using isolated BPST instantons as a zero-temperature
  QCD eta-prime derivation.
...
| \(H_{\theta,\eta}^{\rm inst}\) | rank-one \((\theta,\eta_0)\) curvature matrix generated by the finite-activity instanton determinant potential |
| \(\chi_{\rm inst},\epsilon_\chi\) | instanton-branch curvature \(2\zeta_\chi\) and same-scheme curvature-distance tolerance used when comparing a finite-activity instanton branch with \(\chi_{\rm YM}\) |
| \(\epsilon_{\rm br},\epsilon_{\rm mix},R_{\rm chiral},R_{\rm kin},R_{\rm mix}\) | branch, kinetic, and observable-channel residuals needed before an instanton activity is used as an \(\eta'\) or \(\eta,\eta'\) trace approximant |
...
| \(a_0^0,a_0^2\) | \(S\)-wave \(\pi\pi\) threshold scattering-length coordinates obtained by isospin and partial-wave projection |
| \(\mathcal L_0^I(\mu),R_6^I\) | one-loop/mass-spurion \(O(p^4)\) threshold projection and first omitted \(O(p^6)\) threshold residual in ChPT |
...
- The same matching is now expressed as the local \((\theta,\eta_0)\)
  ...
  alignment.
- The Witten--Veneziano matching now has a Ward-compatible residual budget:
  in the exactly massless, symmetry-preserving full-QCD setting the local
...
- Under the separate finite-activity dilute-instanton hypothesis inherited
  ...
  states that this is valid only in declared semiclassical finite-activity
  regimes and is not a derivation of the physical eta-prime mass in ordinary
  zero-temperature confining QCD.
- The instanton-to-Witten--Veneziano comparison is now a controlled window
  rather than an implicit substitution.  The chapter defines
  \(\chi_{\rm inst}=2\zeta_\chi\), requires a same-scheme estimate
  \(|\chi_{\rm YM}-\chi_{\rm inst}|\le\epsilon_\chi\), propagates this through
  the singlet mass and \(\eta,\eta'\) trace residual budgets, and repeats the
  Schur-complement point for the instanton branch: after minimizing the
  dynamical massless singlet field, the full-QCD local theta susceptibility is
  still zero.  The text therefore distinguishes the source-saturated
  instanton amplitude, the conditional instanton theta curvature, the pure-YM
  Witten--Veneziano datum, and the physical pseudoscalar pole/trace
  observables.
...
- The local chiral amplitude is now projected to a physical pion-threshold
  observable.  The mass-spurion leading amplitude
  \(A_2=(s-m_\pi^2)/f_\pi^2\), not the chiral-limit \(A=s/f_\pi^2\), gives
  the Weinberg threshold coordinates
  \(a_{0,\mathrm{LO}}^0=7m_\pi^2/(32\pi f_\pi^2)\) and
  \(a_{0,\mathrm{LO}}^2=-m_\pi^2/(16\pi f_\pi^2)\).  At \(O(p^4)\) the
  projected \(C_4,C'_4\) local matrix is displayed separately from the
  loop/mass-spurion projection and the \(O(p^6)\) residual, so fitting
  threshold scattering lengths cannot be mistaken for independently computing
  the local constants from QCD.
...
- 2026-06-06 issue #630/#844 ChPT threshold-observable pass: added
  `ca:chpt-pion-threshold-observable-projection`, which turns the local
  \(SU(2)\) pion amplitude into the physical \(S\)-wave threshold coordinates
  \(a_0^0,a_0^2\).  The pass derives the leading massive-spurion threshold
  scattering lengths, displays the local \(C_4,C'_4\) projection matrix,
  separates loop and \(O(p^6)\) residual entries, and records the finite shift
  in fitted local constants when loop/remainder data are silently absorbed.
  `chpt_nlo_checks.py` now verifies these projections and negative controls.
- 2026-06-03 issue #630 theta/Witten--Veneziano pass: added the local
  ...
  identities.
- 2026-06-04 issue #738 Ward-identity repair: replaced the three-independent
  entry residual Hessian by a Ward-compatible scalar residual
  \(\delta\chi\) preserving the massless screening vector.  The text now keeps
  \(\chi_{\rm QCD}^{m=0,{\rm loc}}\equiv0\) in the exactly massless,
  symmetry-preserving theory, cites the Leutwyler--Smilga small-mass
  constraint at point of use, and demotes a nonzero Schur complement to a
  finite-mass or explicit Ward-breaking diagnostic.  Extended
  `qcd_theta_witten_veneziano_checks.py` with a negative control rejecting an
  unconstrained residual Hessian as a massless-QCD residual.
...
- 2026-06-03 issue #597/#630 instanton-to-chiral-spurion pass: added a
  controlled finite-activity bridge from the Chapter 20 instanton
  determinant to the chiral singlet potential, including susceptibility,
  curvature, screening direction, quartic sign, and singlet mass contribution
  checks in `qcd_theta_witten_veneziano_checks.py`.  The text separates this
...
- 2026-06-05 issue #597 instanton/Witten--Veneziano comparison pass: added the
  same-scheme curvature-distance and observable residual budgets needed before
  replacing the pure-YM Witten--Veneziano curvature by a finite-activity
  instanton branch.  The text now explicitly separates source kernels,
  conditional instanton theta curvature, full massless-QCD theta screening,
  and physical \(\eta'\) or \(\eta,\eta'\) trace observables.  Extended
  `qcd_theta_witten_veneziano_checks.py` with exact checks of the comparison
  mass gap, trace budget, Schur complement, and negative controls against
  under-budgeted instanton substitution or using the full massless-QCD
  susceptibility in the WV formula.
