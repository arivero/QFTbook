...
- the mass gap \(m\) of the scaling limit and the critical \(m=0\) Ising CFT;
- the operator dictionary \(\sigma\leftrightarrow\phi\) and
  \(\varepsilon\leftrightarrow\phi^2\);
...
- universality as attraction to the same fixed point after tuning relevant
  directions;
...
6. At criticality the mass gap vanishes and the limiting theory is the Ising
   CFT.
...
- RG universality diagram showing Ising lattice actions and scalar
  \(\phi^4\)-type actions flowing to the same critical fixed point after
  tuning relevant directions.
