# 2026-06-07 Issue 597 Mass-Assisted Interference Audit

## Scope

- Targeted Volume II, Chapter 20D, the instantons-as-physical-amplitudes
  chapter.
- Added `ca:instanton-mass-assisted-interference-channel` after the crossed
  chiral channel block.
- The pass develops the two-source determinant coordinate \(m_dB_{uu}\) as a
  physical \(u_R\to u_L\) interference channel, not as another moduli-space or
  ADHM calculation.

## Quality Audit

- The block keeps zero-mode saturation, source degree, external-state basis,
  mass-phase orientation, and residual propagation separate.
- The theta-linear observable is not identified with the massless
  one-instanton amplitude by itself.  It requires a same-basis
  chirality-breaking reference amplitude and gives
  \(\theta+\arg m_u+\arg m_d\), up to the channel phase.
