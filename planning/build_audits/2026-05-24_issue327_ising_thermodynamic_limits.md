...
- Added
  `Quoted Theorem~\ref{qthm:ising-ferromagnetic-thermodynamic-limits}` before
  the spin-correlator phase discussion.
- The quoted theorem records the finite-volume ferromagnetic inequalities:
  ...
  - GHS concavity of the magnetization as a function of a uniform positive
    field.
- The manuscript gives the finite-lattice FKG derivation from
  log-supermodularity of the ferromagnetic edge weight, while marking GKS and
  GHS as nontrivial quoted finite-volume correlation inequalities rather than
  pretending to prove them in a few lines.
- The quoted theorem states the existence of plus and minus infinite-volume
  limits for local observables along cofinal rectangular boxes.
- The quoted theorem states the one-sided \(h\downarrow0\) limits and distinguishes
  boundary-condition independence when plus and minus states coincide from
  phase coexistence when they differ.
- The quoted theorem defines the extra clustering condition needed for plus/minus
  boundary-selected Gibbs states to be pure/extremal.
- The subsequent phase discussion now refers back to this quoted theorem before
...
background assumption.  After the proof-substance audit, it also no longer
packages unproved GKS/GHS inputs as a self-contained proof.  It separates:
