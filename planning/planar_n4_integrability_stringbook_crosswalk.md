...
| Crossing equation and crossing-symmetric dressing factor | `The crossing-symmetric dressing factor` | `Expanded/Boundary`: Chapter 13 derives scalar unitarity, shifted-cut crossing-path monodromy for `x^pm`, DHM weak coefficients, the local DHM residue-continuation rules from the stringbook notebook convention, the induced crossed-first-particle BES-phase residue package, the DHM Gamma-kernel pole lattice and admissibility conditions, reciprocal convention algebra, the finite matrix-channel origin of the Janik rational multiplier, the scalar crossing monodromy cocycle, and the explicit double-crossing branch displacement; it states BES/crossing as a theorem | Still needs the genuinely global scalar-branch construction: minimal BES branch uniqueness, homotopy classes separated by scalar poles/zeros or CDD factors, global `chi` continuation, and a systematic comparison with any remaining stringbook notebook crossing experiments not yet ported. |
...
3. Expand the global dressing/crossing analytic continuation beyond the
   ...
   Chapter 13: construct or sharply axiomatize the minimal BES scalar branch,
   classify homotopy-class dependence of scalar branches through poles/zeros
   or CDD factors, and port any remaining stringbook notebook crossing
   experiments.
