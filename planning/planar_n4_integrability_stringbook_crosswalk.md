...
| Crossing equation and crossing-symmetric dressing factor | `The crossing-symmetric dressing factor` | `Partial/Boundary`: Chapter 13 derives scalar unitarity, shifted-cut crossing-path monodromy for `x^pm`, DHM weak coefficients, the local DHM residue-continuation rules from the stringbook notebook convention, reciprocal convention algebra, and states BES/crossing as a theorem | Needs deeper global analytic-continuation development: scalar-factor monodromy along the crossing path, Gamma-kernel pole avoidance hypotheses, and a systematic comparison with the remaining stringbook notebook crossing experiments. |
...
3. Expand the global dressing/crossing analytic continuation beyond the
   shifted-cut `x^pm` monodromy and local DHM residue chamber rules now in
   Chapter 13: state the scalar-factor monodromy, BES/DHM kernel hypotheses,
   pole-avoidance data, and convention transformations.
