...
Tracks **unresolved / actionable** items only. The GitHub issue tracker on `xiyin137/QFT` is the primary backlog. **Current state (2026-05-31)**: **27 open issues** after the Liouville pole-residue synthesis pass for #702. End-to-end per-figure audit complete (114 figures across all 12 volumes; 43 issues filed #649–#690, all closed). Audit-meta issue #579 is closed after the proof-substance ledgers; #691 remains open until surviving trivial theorem-family wrappers are demoted or strengthened after reading the actual proof substance. Issue #701 remains open as the comprehensive-audit meta issue; its source-vs-print mapping item A3 is addressed in the frontmatter source assembly map, C3 is addressed by subsectioning the giant Vol XI ch09 DPD/regularity-structure sections, A1 has a first bibliographic guide seed, and A2 has a first notation/convention guide seed, while complete citation/index expansion/C2-monolith/depth follow-ons remain. Depth-pass-B work on previously-closed lanes and substantive chapter additions remain. Issue #693 is resolved by converting every genuinely labelled `\[...\]` display to a numbered equation environment and adding a build-gate audit against recurrence.
