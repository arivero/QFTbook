...
Tracks **unresolved / actionable** items only. The GitHub issue tracker on `xiyin137/QFT` is the primary backlog. **Current state (2026-05-31)**: the live GitHub tracker is authoritative; this file records the active cross-cutting and selected unresolved lanes. End-to-end per-figure audit complete (114 figures across all 12 volumes; 43 issues filed #649–#690, all closed). Audit-meta issue #579 is closed after the proof-substance ledgers; #691 remains open until surviving trivial theorem-family wrappers are demoted or strengthened after reading the actual proof substance. Issue #701 remains open as the comprehensive-audit meta issue; its source-vs-print mapping item A3 is addressed in the frontmatter source assembly map, C3 is addressed by subsectioning the giant Vol XI ch09 DPD/regularity-structure sections, A1 has a first bibliographic guide seed, A2 has a first notation/convention guide seed, and the Vol II ch19 QCD monolith has a first structural subsection pass, while complete citation/index expansion/depth follow-ons remain. Depth-pass-B work on previously-closed lanes and substantive chapter additions remain. Issue #693 is resolved by converting every genuinely labelled `\[...\]` display to a numbered equation environment and adding a build-gate audit against recurrence.
...
| [#519](https://github.com/xiyin137/QFT/issues/519) | Energy-correlator section — partial: nonperturbative detector definition, outgoing direct-integral multiplication model for smeared detector products, EEC contact/moment sum rules, tree-level collinear EEC coefficient, renormalized small-angle light-ray-OPE bookkeeping/contact convention/RG-sign derivation, and leading back-to-back Sudakov factor added (Vol II ch19, calc-checks pass). Remaining: full modern program (Moult/Zhiboedov/Korchemsky/Kravchuk/Simmons-Duffin); actual all-order renormalized light-ray-OPE/mixing theorem; complete endpoint matching; analytic high-loop frontier | IV |
...
| [#630](https://github.com/xiyin137/QFT/issues/630) | QCD rigor uplift — partial: SCET, HQET/NRQCD, Drell-Yan/Glauber, BFKL, finite Wilson-line CGC/JIMWLK theorem-boundary datum, GPD/TMD, lattice quasi-/pseudo-PDF matching, QCD current sum rules, Eguchi--Kawai volume-reduction hypotheses/center-selection, and Gribov--Zwanziger horizon-functional/BRST-status treatment now have operator-level rigorous starts with calculation checks. Remaining subclusters: refined jet factorization, continuum JIMWLK limit beyond the finite Wilson-line datum, stronger SCET/Glauber proofs, exact CS-matter solution layer, hydrodynamics-from-QCD. (θ/WV/CFL/HTL/NFL done in Vol X ch12 + Vol II ch20/21) |
...
| Issue | Remaining work |
|---|---|
| [#561](https://github.com/xiyin137/QFT/issues/561) | Per-chapter calc-check companions: 206 Python scripts + 2 Wolfram Language scripts are now visible through `tools/run_calculation_checks.sh --list`, with `calculation-checks/INDEX.md` and a README ledger covering every active script. Remaining: close true chapter-coverage gaps, not merely add infrastructure |
...
| [#701](https://github.com/xiyin137/QFT/issues/701) | Comprehensive audit meta. Addressed: A3 source-vs-print volume confusion now has a frontmatter source assembly map; C3 Vol XI ch09 structural balance has subsection navigation; A1 has a first reader-facing bibliographic guide and citation seed, with the harness updated so citations remain source traces rather than proof substitutes; A2 has a first reader-facing notation/convention guide seed, with the harness updated so the guide cannot replace local first-use definitions; Vol II ch19 now has subsection navigation through its large QCD blocks. Remaining: complete bibliography/citation expansion at precise theorem-boundary sites, full index/glossary/nomenclature expansion, and any new findings from continued full audit |
| [#703](https://github.com/xiyin137/QFT/issues/703) | Statmech-to-QFT absorption crosswalk. Partial: nonequilibrium finite path-measure/MSRJD/Doi-Peliti/large-deviation layers, finite Model A/B mobility-gradient data, hydrodynamic Kubo/Mori closure datum, finite-locality/TQFT laboratories, numerical-evidence certificates, HMC/RHMC diagnostics, and constructive polymer source-cumulant bridge. Remaining: dynamic critical models with additional slow variables and model-by-model RG, deeper hydrodynamic theorem-status/GHD bridge, tensor-network/neural continuum-control examples, constructive model-specific polymer bounds, and further finite-locality/Hall/TQFT stability work |
---
