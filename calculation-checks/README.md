...
- `susy_instanton_nekr_checks.py`: exact rational checks for the
  ...
  and Higgs-patch collective-coordinate ledger, the Yukawa-lifting Berezin
  determinant saturation and coefficient-factorization ledger, the
  maximal-rank meson determinant-invariant uniqueness step, holomorphic
  decoupling exponent shift, ADS decoupling-recursion coefficient and
  one-variable \(F\)-term algebra, the pure \(SU(2)\)
