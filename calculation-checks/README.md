...
- `ising_form_factor_checks.py`: finite checks for the Volume VI
  ...
  Euclidean reconstruction estimate.
...
- `thermal_screening_checks.py`: finite checks for the Volume X thermal
  ...
  normalizations, plus the HTL angular-kernel transversality and static-limit
  algebra behind the Debye coefficient.  The same script checks the
  Polyakov-line self-energy cancellation in connected static-source pair
  ratios and source-pair forces, with an explicit guard that the pair-excess
  ratio is undefined when center symmetry makes the one-line expectations
  vanish.
