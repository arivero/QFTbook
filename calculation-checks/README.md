...
- `anomaly_polynomial_descent_checks.py`: exact rational checks for the
  ...
  counterterm shifts, the source-Ward contact-term polarization and raw-trace
  normalization map showing that individual current contacts move under local
...
- `lattice_locality_flow_checks.py`: finite-regulator locality checks for the
  ...
  nearest-neighbor Pauli-chain commutator comparison, a support-weighted
  derivative negative control, two-level spectral-flow transport, an isolated
  excited-band control, the time-window split behind quasi-local generator
  tails, and a receding-boundary Cauchy-tail arithmetic check.  It also
  includes negative controls showing that fixed-distance correlator continuity
  can coexist with a closing gap and that a value-only topology can miss a
  divergent susceptibility, together with a growing-support negative control
  where finite-volume automorphisms converge on every fixed local algebra but
  do not control a moving boundary or logical observable sequence, and an
  explicit finite Ising Hamiltonian selector control showing that a gapped
  parameter value with a multi-branch ground-state band does not select a
  unique phase face, that the full ground-state set can transport setwise, and
  that plus/minus selected faces transport only with a covariant boundary
  selector.
...
- `paqft_algebra_checks.py`: finite polynomial checks for the curved pAQFT
  ...
  kernel.  It also checks the renormalized BV anomaly-map replacement for the
  formal local BV Laplacian: transported time-ordered BV brackets, local
  Wess-Zumino anomaly consistency, exact-anomaly removal by counterterm/scheme
  shift, nonexact anomaly obstruction, and finite-cutoff Laplacian shortcut
  negative controls.
...
- `susy_2d_lg_glsm_checks.py`: exact finite checks for the
  ...
  adding a gauge-covariant vortex-core disorder-insertion gate that checks
  large-gauge invariance of the covariant winding, separates smooth core
  extension from singular holonomy cancellation, rejects the single-valued
  logarithmic chart across the core, derives the primitive `exp(-Y_i)`
  orientation from the excised-disk first-order boundary term, and budgets the
  core-domain residual in the operator comparison,
  ...
  swaps, and unsaturated zero modes, the degree-one representative-independence
  gate checking that the primary stable-map invariant is unchanged by
  non-transverse representatives after deformation or refined/excess
  intersection, rejecting the old Boolean boundary-indicator arithmetic, and
  separating collision-sensitive source contacts from the primary invariant,
  the finite
...
- `generalized_unitarity_reduction_checks.py`: exact finite checks for the
  ...
  and cut-only boundary negative controls, the Steinmann projection test for
  ...
  overlapping-word plus symbol-only non-symbolic-data negative controls, the production master-lane gate
